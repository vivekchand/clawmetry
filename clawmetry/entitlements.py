...
def _lock_row(ent, key: str, kind: str) -> dict:
    try:
        ...
        logger.warning("entitlements: _lock_row failed for %r/%r: %s", key, kind, exc)
        return {
...
def bulk_lock_check(
    ...
    """Multi-axis lock-status check in one round-trip.

    Each kwarg accepts a comma-separated string or an iterable of the
    items to check on that axis (features / runtimes -> name strings;
    channels / retention_days / nodes -> integer strings or ints).
    Omitted axes return empty lists in the response.

    Returns a dict with one key per axis, each holding a list of
    ``_lock_row`` dicts, plus a ``summary`` key (``{locked: N,
    allowed: N}``) and the full live ``entitlement`` dict so the caller
    doesn't need a second request to render the upgrade CTA.
    ...
    try:
        ...
        for feat in _normalise_csv(features):
            result["features"].append(_lock_row(ent, feat, "feature"))
        for rt in _normalise_csv(runtimes):
            result["runtimes"].append(_lock_row(ent, rt, "runtime"))
        for ch in _normalise_csv(channels):
            result["channels"].append(_lock_row(ent, ch, "channels"))
        for rd in _normalise_csv(retention_days):
            result["retention_days"].append(_lock_row(ent, rd, "retention_days"))
        for nd in _normalise_csv(nodes):
            result["nodes"].append(_lock_row(ent, nd, "nodes"))
        all_rows = (
            result["features"]
            + result["runtimes"]
            + result["channels"]
        ...
        result["summary"] = {
            "locked": sum(1 for r in all_rows if r.get("locked")),
            "allowed": sum(1 for r in all_rows if r.get("allowed")),
        }
        result["entitlement"] = ent.to_dict()
        return result
    except Exception as exc:
        logger.warning("entitlements: bulk_lock_check failed: %s", exc)
        return {
            "features": [],
            "runtimes": [],
            "channels": [],
            "retention_days": [],
            "nodes": [],
            "summary": {"locked": 0, "allowed": 0},
            "entitlement": _oss_free().to_dict(),
        }


def tier_matrix() -> list[dict]:
    """Full feature x tier matrix for a pricing-page table.

    Each row in the returned list describes one feature (or runtime)
    and its availability across every purchasable tier::

        [
          {
            "key": "fleet",
            "label": "Multi-node fleet",
            "kind": "feature",
            "tiers": {
              "oss": False,
              "cloud_free": False,
    ...
    The ordering is canonical: FREE_FEATURES first (alphabetical), then
    STARTER_FEATURES, then PRO_ONLY_FEATURES, then ENTERPRISE_FEATURES,
    then FREE_RUNTIMES (alphabetical), then PAID_RUNTIMES. Callers
    wanting a different order should sort client-side -- the shape is
    stable so sorting by (kind, tier_rank, key) is safe.

    Never raises: a resolver failure returns ``[]``.
    """
    try:
        rows: list[dict] = []
        ...
        def _row(key: str, kind: str) -> dict:
            label = (
                FEATURE_LABELS.get(key, key)
                if kind == "feature"
                else RUNTIME_LABELS.get(key, key)
            )
            ...
            for t in ordered_tiers:
                if kind == "feature":
                    ent = Entitlement(
                        ...
                        source="preview",
                        node_limit=1,
                        features=FREE_FEATURES | _TIER_FEATURES.get(t, frozenset()),
                        runtimes=FREE_RUNTIMES,
                        grace=False,
                    )
                    tier_map[t] = ent.allows_feature(key)
                else:
                    ent = Entitlement(
                        tier=t,
                        source="preview",
                    ...
                    tier_map[t] = ent.allows_runtime(key)
            return {"key": key, "label": label, "kind": kind, "tiers": tier_map}

        for key in sorted(FREE_FEATURES):
            rows.append(_row(key, "feature"))
        for key in sorted(STARTER_FEATURES):
            rows.append(_row(key, "feature"))
        for key in sorted(PRO_ONLY_FEATURES):
            rows.append(_row(key, "feature"))
        for key in sorted(ENTERPRISE_FEATURES):
            rows.append(_row(key, "feature"))
        for key in sorted(FREE_RUNTIMES):
            rows.append(_row(key, "runtime"))
        for key in sorted(PAID_RUNTIMES):
            rows.append(_row(key, "runtime"))
        return rows
    except Exception as exc:
        logger.warning("entitlements: tier_matrix failed: %s", exc)
        return []


def tier_matrix_for_tiers(tiers) -> list[dict]:
    """Subset of :func:`tier_matrix` restricted to the supplied ``tiers``.

    Convenience for upgrade-CTA cards that want the feature-x-tier matrix
    for only two or three tiers (e.g. the current tier and the next one)
    rather than the full six-column pricing table. The row shape and
    ordering are identical to :func:`tier_matrix`; each row's ``tiers``
    dict only contains the requested tier ids.

    ``tiers`` may be a comma-separated string or an iterable of tier id
    strings. Unknown / empty tier ids are silently dropped. If the
    resulting set is empty after normalisation, :func:`tier_matrix` is
    called with the full purchasable set ("I want all" is the natural
    fallback for an empty filter).

    Never raises.
    """
    try:
        requested = _normalise_csv(tiers)
        valid = [t for t in requested if t in _PURCHASABLE_TIERS]
        if not valid:
            return tier_matrix()
        ordered_tiers = sorted(valid, key=lambda t: (_TIER_RANK.get(t, -1), t))
        ...
        logger.warning("entitlements: tier_matrix_for_tiers failed: %s", exc)
        return []


def pricing_page_payload() -> dict:
    """One-shot pricing-page bundle: tier previews + matrix + tier_unlocks rows.

    Combines three separate API calls (``/api/entitlement/preview?tier=X`` x N,
    ``/api/entitlement/tier-matrix``, ``/api/entitlement/tier-unlocks-batch``)
    into a single structured payload so a pricing page can render the full
    "what do I get at each tier" table off **one** round-trip.
    ...
    Shape::

        {
          "tiers": [
            {
              "tier": "oss",
              "preview": {...},          # Entitlement.to_dict() for that tier
              "unlocks": {...},          # tier_unlocks() row
            },
            ...
          ],
          "matrix": [...],              # tier_matrix() rows
        }

    Never raises: any sub-call failure is swallowed and the entry is
    omitted / set to ``None``.
    """
    try:
        ordered = sorted(
        ...
        for tid in ordered:
            try:
                ...
                prev = None
            try:
                unlocks = tier_unlocks(tid)
            except Exception:
                unlocks = None
            tier_rows.append({"tier": tid, "preview": prev, "unlocks": unlocks})
        try:
            matrix = tier_matrix()
        except Exception:
            matrix = []
        return {"tiers": tier_rows, "matrix": matrix}
    except Exception as exc:
        logger.warning("entitlements: pricing_page_payload failed: %s", exc)
        return {"tiers": [], "matrix": []}


def _emit_unlock_events(prev_tier: str, new_ent: Entitlement) -> None:
    """Fire ``entitlement.unlocked`` extension events for each newly-accessible
    runtime / feature after an upgrade.

    Compares what the previous tier allowed (synthesised as a grace-off
    Entitlement snapshot so the comparison is concrete) against the new
    entitlement, then emits one event per newly-unlocked item. Each event
    carries ``{kind, key, label, tier, source}`` so listeners can react
    without having to re-query the entitlement layer.

    Called by the sync daemon's heartbeat handler after it writes a fresh
    cloud_plan.json and calls :func:`invalidate`. Never raises.
    """
    try:
        from clawmetry import extensions as _ext
    except Exception:
        return
    try:
        prev_paid_feats = _TIER_FEATURES.get(prev_tier, frozenset())
        prev_runtimes = (
            FREE_RUNTIMES | PAID_RUNTIMES
            if prev_tier in _TIER_PAID_RUNTIMES
            else FREE_RUNTIMES
        )
        prev_ent = Entitlement(
            tier=prev_tier,
            source="preview",
            node_limit=1,
            features=FREE_FEATURES | prev_paid_feats,
            runtimes=prev_runtimes,
            grace=False,
        )
        for rt in sorted(new_ent.runtimes - prev_ent.runtimes):
            try:
                _ext.emit(
                    "entitlement.unlocked",
                    {
                        ...
                        "tier": new_ent.tier,
                        "source": new_ent.source,
                    },
                )
            except Exception as exc:
                logger.debug("entitlements: unlock-event emit failed for %r: %s", rt, exc)
        for feat in sorted(new_ent.features - prev_ent.features):
            try:
                _ext.emit(
                    "entitlement.unlocked",
                    {
                ...
                logger.debug(
                    "entitlements: unlock-event emit failed for %r: %s", feat, exc
                )
    except Exception as exc:
        logger.warning("entitlements: _emit_unlock_events failed: %s", exc)


def tier_unlocks_for_purchasable_tiers(
    tiers,
    *,
    include_paid_runtimes: bool = True,
) -> list[dict]:
    """Marginal-unlock rows for an explicit list of tiers.

    Caller-supplied subset of :func:`tier_unlocks_batch` -- where the
    batch always walks *all* purchasable tiers, this helper accepts an
    explicit ``tiers`` argument (comma-separated string or iterable of
    tier id strings) so a caller that already knows which tiers to show
    avoids the full-ladder walk.

    ``include_paid_runtimes`` (default ``True``) controls whether
    ``paid_runtimes`` appears on each row.  The field is a static list
    of all paid runtimes -- it is not specific to the tier, and it is
    the same on every row -- but including it lets a single-endpoint
    caller build the upgrade card without a second request to
    ``/api/entitlement/runtimes`` or similar.

    Returns the same row shape as :func:`tier_unlocks` (plus an optional
    ``paid_runtimes`` key), sorted by ``(tier_rank, tier_id)``.  Unknown
    or non-purchasable tier ids are silently dropped.  If the resulting
    set is empty, returns ``[]``.  Never raises.
    """
    try:
        requested = _normalise_csv(tiers)
        valid = [
            t for t in requested
            if t in _PURCHASABLE_TIERS
        ]
        if not valid:
            return []
        ordered = sorted(valid, key=lambda t: (_TIER_RANK.get(t, -1), t))
        out: list[dict] = []
        ...
        for tid in ordered:
            ...
            if row is None:
                continue
            if include_paid_runtimes:
                row = dict(row)
                row["paid_runtimes"] = paid_runtimes_sorted
            out.append(row)
        return out
    except Exception as exc:
        logger.warning(
            "entitlements: tier_unlocks_for_purchasable_tiers failed: %s", exc
        )
        return []


def entitlement_summary() -> dict:
    """Compact entitlement snapshot for telemetry / diagnostics / healthcheck.

    Returns a minimal dict -- tier, source, grace, enforced, is_paid,
    expired, days_until_expiry -- that is safe to log or embed in a
    healthcheck response without leaking the full feature / runtime list.
    Useful when the caller only needs to know "am I on a paid plan and
    am I within the grace period" without pulling the entire
    ``to_dict()`` shape.

    Never raises: a resolver failure returns the OSS-free shape.
    """
    try:
        ent = get_entitlement()
        return {
            "tier": ent.tier,
            "tier_label": tier_label(ent.tier),
            "tier_rank": tier_rank(ent.tier),
            "source": ent.source,
            "grace": ent.grace,
            "enforced": not ent.grace,
            "is_paid": ent.is_paid,
            "expired": ent.expired,
            "days_until_expiry": ent.days_until_expiry(),
            "days_until_enforce": ent.grace_remaining_days(),
        }
    except Exception as exc:
        logger.warning("entitlements: entitlement_summary failed: %s", exc)
        oss = _oss_free()
        return {
            "tier": oss.tier,
            "tier_label": tier_label(oss.tier),
            "tier_rank": tier_rank(oss.tier),
            "source": oss.source,
            "grace": oss.grace,
            "enforced": not oss.grace,
            "is_paid": oss.is_paid,
            "expired": oss.expired,
            "days_until_expiry": oss.days_until_expiry(),
            "days_until_enforce": oss.grace_remaining_days(),
        }
