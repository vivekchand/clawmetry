...
def _cmd_onboard(args) -> None:
    ...
    print()
    print(f"    {BOLD('[1] We host it')}   {DIM('The dashboard is run for you: app.clawmetry.com from')}")
    print(f"                     {DIM('anywhere PLUS http://localhost:8900 on this machine.')}")
    print(f"                     {DIM('The easy way to watch a fleet of nodes; desk device at')}")
    print(f"                     {DIM('clawmetry.com/device. E2E-encrypted: snapshots are')}")
    print(f"                     {DIM('sealed here and only you hold the key.')}")
    print(f"    {BOLD('[2] You host it')}  {DIM('Everything runs and stays on your devices; dashboard')}")
    print(f"                     {DIM('at http://localhost:8900. Watching a fleet across')}")
    print(f"                     {DIM('machines is yours to wire up.')}")
    print()
