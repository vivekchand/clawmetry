...
def _cmd_onboard(args) -> None:
    ...
    print()
    print(f"    {BOLD('[1] Cloud')}        {DIM('Your whole fleet in one dashboard: app.clawmetry.com')}")
    print(f"                     {DIM('from anywhere PLUS http://localhost:8900 on this machine,')}")
    print(f"                     {DIM('and the desk device (clawmetry.com/device). E2E-encrypted:')}")
    print(f"                     {DIM('snapshots are sealed here and only you hold the key.')}")
    print(f"    {BOLD('[2] Self-Hosted')}  {DIM('Everything runs and stays on your devices.')}")
