...
def license_features() -> list[str] | None:
    ...
    Mirrors the "refuse expired keys" posture already used by other
    ``license_*`` scalars: a lapsed key must not keep rendering as
    "features unlocked". A caller wanting to surface the CLAIM even on
    an expired key (support: "what features was this key SUPPOSED to
    grant?") should re-verify the token directly with
    :func:`verify_token`.
