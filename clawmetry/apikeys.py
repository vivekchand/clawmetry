...
#: Public aliases so callers that generate their own material (e.g. the CLI,
#: which must do so in its own scope to keep CodeQL's taint tracker satisfied)
#: can match the exact byte counts without importing the private names.
ID_BYTES = _ID_BYTES
...
def create_from_material(
    ...
    *,
    ...
    """Store a key whose material was generated by the caller.

    ``key_id`` and ``wire_secret`` must already be SHA-256 hex digests --
    the caller is responsible for generating the raw entropy and hashing it
    before passing it here.  The returned record has ``"hash"`` set to
    ``sha256(wire_secret)`` and does NOT include the secret itself.

    This split lets the CLI generate key material inside its own function
    scope so CodeQL's intra-procedural taint tracker sees the sanitizing
    ``hashlib.sha256(...).hexdigest()`` calls before any sink, without
    relying on inter-procedural sanitizer propagation across ``create()``.
    ``create()`` itself is unchanged and remains the correct path for every
    caller that does not have that constraint (e.g. the HTTP endpoint).
    """
    label = (name or "").strip()
    if not label:
        raise ApiKeyError(
