...
class LocalStore:
    ...
    # ── Trail: session intent + typed event back-fill (schema v15) ─────────
...
def _event_to_row(e: dict[str, Any], usage: dict[str, Any] | None = None) -> tuple:
    ...
    shape = _event_shape.classify(e.get("event_type"), e.get("data"))
    return (
        str(e["id"]),
        ...
        # v15 Trail typed columns (see clawmetry.event_shape).
        shape["role"] or None,
        shape["block_kind"] or "other",
        shape["tool_name"] or None,
        bool(shape["is_error"]),
