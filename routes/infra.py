    ...
    if request.method == "HEAD" or "text/event-stream" not in accept:
        ...
        if is_local_store_read_enabled():
            ...
            if snap is not None:
                ...
                # Issue #1772: DuckDB responded but returned no events AND
                # the writer is offline → 503. Only fires when snap is
                # non-None (store opened OK) so a no-DuckDB install still
