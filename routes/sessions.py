...
@bp_sessions.route("/api/sessions/export")
def api_sessions_export():
    """Export session data as JSON or CSV for offline analysis and reporting.

    Params:
      format:     json (default) or csv
      session_id: single session; omit for bulk (most-recent first)
      limit:      max sessions for bulk export (default 50, max 200)
      from:       ISO datetime lower bound (inclusive), e.g. 2026-01-01T00:00:00Z
    ...
    if raw_from:
        try:
            from_ts = datetime.fromisoformat(raw_from.replace("Z", "+00:00")).timestamp()
        except Exception:
            pass
    if raw_to:
        try:
            to_ts = datetime.fromisoformat(raw_to.replace("Z", "+00:00")).timestamp()
        except Exception:
            pass
    ...
    def _parse_session(fpath):
        ...
        try:
            with open(fpath, "r", errors="replace") as fh:
                for raw in fh:
                    ...
                    if ts_raw:
                        try:
                            if isinstance(ts_raw, (int, float)):
                                ts_epoch = float(ts_raw) if float(ts_raw) > 1e10 else float(ts_raw) * 1000
                    ...
                    if ts_epoch:
                        if from_ts and ts_epoch < from_ts:
                            continue
                        if to_ts and ts_epoch > to_ts:
                            continue
                        if first_ts_epoch is None or ts_epoch < first_ts_epoch:
                            first_ts_epoch = ts_epoch
                        if last_ts_epoch is None or ts_epoch > last_ts_epoch:
                    ...
                    if isinstance(usage, dict):
                        ...
                        total_cost += float(
                            usage.get("cost_usd", 0) or usage.get("costUsd", 0) or 0.0
                        )

                    if not model:
                        model = obj.get("model")

                    role = obj.get("role", ev_type)
                    content = obj.get("content", "")
                    if isinstance(content, list):
                        parts = []
                        for part in content:
                            if isinstance(part, dict):
                                ...
                                if ptype == "text":
                                    ...
                                    parts.append(
                                        ...
                                        + json.dumps(part.get("input", {}), separators=(",", ":"))[:200]
                                    )
                                elif ptype == "tool_result":
                                    parts.append(f"[result] {str(part.get('content', ''))[:200]}")
                            else:
                                parts.append(str(part))
                    ...
                    if role in ("user", "assistant", "system", "tool_use", "tool_result", "tool") \
                            or ev_type == "message":
                        messages.append({
                            "role": role,
        ...
        duration_s = None
        if first_ts_epoch and last_ts_epoch and last_ts_epoch > first_ts_epoch:
            duration_s = round(last_ts_epoch - first_ts_epoch, 1)

        total_tokens = sum(tokens.values())
        return {
            "session_id": sid,
            "model": model or "unknown",
            "start_time": datetime.utcfromtimestamp(first_ts_epoch).isoformat() + "Z"
            if first_ts_epoch else None,
            "end_time": datetime.utcfromtimestamp(last_ts_epoch).isoformat() + "Z"
            if last_ts_epoch else None,
    ...
    sessions_data = []
    norm_dir = os.path.normpath(sessions_dir)
    for fname in files:
        fpath = os.path.normpath(os.path.join(sessions_dir, fname))
        if not fpath.startswith(norm_dir):
            continue
        sessions_data.append(_parse_session(fpath))

    exported_at = datetime.utcnow().isoformat() + "Z"

    if fmt == "json":
        payload = json.dumps(
            {"exported_at": exported_at, "total_sessions": len(sessions_data), "sessions": sessions_data},
            indent=2,
            default=str,
        )
        return Response(
            ...
            headers={"Content-Disposition": "attachment; filename=clawmetry-sessions-export.json"},
        )

    buf = io.StringIO()
    writer = csv.writer(buf)
    writer.writerow([
        "session_id", "model", "start_time", "end_time", "duration_seconds",
        "total_tokens", "cost_usd", "msg_role", "msg_timestamp", "msg_content",
    ])
    for s in sessions_data:
        if not s["messages"]:
            writer.writerow([
                s["session_id"], s["model"], s["start_time"], s["end_time"],
                s["duration_seconds"], s["total_tokens"], s["cost_usd"],
                "", "", "",
            ])
            continue
        for msg in s["messages"]:
            writer.writerow([
                s["session_id"], s["model"], s["start_time"], s["end_time"],
                s["duration_seconds"], s["total_tokens"], s["cost_usd"],
                msg.get("role", ""), msg.get("timestamp", ""), msg.get("content", ""),
            ])

    return Response(
        buf.getvalue(),
        mimetype="text/csv",
        headers={"Content-Disposition": "attachment; filename=clawmetry-sessions-export.csv"},
    )
