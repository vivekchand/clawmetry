...
def api_entitlement_feature_spec():
    ...
    ``/api/features``: the catalogue row for one feature id in one round-trip,
    matching exactly one row from :func:`entitlements.feature_catalog`.

    - **400** when ``feature=`` is missing / blank
    - **404** when the id is not a known feature
    - **Never 5xxs**: resolver failure degrades to OSS-free shape.
    """
    ...
    try:
        ...
        if body is None:
            return jsonify({"error": "unknown feature", "feature": feature}), 404
        return jsonify(body)
    except Exception as exc:
...
def api_entitlement_runtime_spec():
    ...
    ``/api/runtimes``: the catalogue row for one runtime id (aliases like
    ``claude-code`` resolve to their canonical id), matching exactly one row
    from :func:`entitlements.runtime_catalog`.

    - **400** when ``runtime=`` is missing / blank
    - **404** when the id is not a known runtime (after alias expansion)
    - **Never 5xxs**: resolver failure degrades to OSS-free shape.
      The ``id`` field in the response echoes the canonical id.
    """
    raw = request.args.get("runtime")
    runtime = (raw or "").strip()
    if not runtime:
    ...
    try:
        ...
        if body is None:
            return jsonify({"error": "unknown runtime", "runtime": runtime}), 404
        return jsonify(body)
    except Exception as exc:
