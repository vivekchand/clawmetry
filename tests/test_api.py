...
class TestSecurity:
    ...
    def test_threats_count_consistency_forward_compat(self, api, base_url):
        """Threat counts add up correctly - forward compatible with new severity levels."""
        d = assert_ok(get(api, base_url, "/api/security/threats"))
        counts = d["counts"]
        severity_keys = [k for k in counts if k != "total"]
        expected_total = sum(counts[k] for k in severity_keys)
