...
class TestAuth:
    ...
    def test_auth_endpoint_xss_protection(self, base_url):
        """Token is escaped in /auth endpoint to prevent XSS."""
        xss_token = "</script><script>alert('XSS')</script>"
        r = requests.get(f"{base_url}/auth?token={xss_token}", timeout=5)
        assert r.status_code == 200
        content = r.text
        assert "<script>alert" not in content
        assert xss_token not in content
