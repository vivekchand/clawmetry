...
class TestProxyApp:
    ...
    def test_proxy_config_patch_rejects_unlimited_budget(self, client, tmp_path):
        """Setting an unrealistic unlimited budget should be rejected."""
        with patch("clawmetry.proxy.PROXY_CONFIG_FILE", tmp_path / "proxy.json"):
            resp = client.patch(
                "/proxy/config",
                data=json.dumps({"budget": {"daily_usd": 1_000_000.0}}),
                content_type="application/json",
            )
        assert resp.status_code == 400
        assert "exceeds maximum" in resp.get_json().get("error", "").lower()

    def test_proxy_config_patch_rejects_excessive_monthly_budget(
        self, client, tmp_path
    ):
        """Setting an unrealistic monthly budget should be rejected."""
        with patch("clawmetry.proxy.PROXY_CONFIG_FILE", tmp_path / "proxy.json"):
            resp = client.patch(
                "/proxy/config",
                data=json.dumps({"budget": {"monthly_usd": 1_000_000.0}}),
                content_type="application/json",
            )
        assert resp.status_code == 400
        assert "exceeds maximum" in resp.get_json().get("error", "").lower()
