"""Tests for ``tier_spec_path_batch(from_tier, to_tiers)`` plus its
HTTP endpoint ``/api/entitlement/tier-spec-path-batch``.

This is the batch sibling of ``tier_spec_path``: where the scalar path
helper walks one ``(from, to)`` pair, the batch helper walks N
candidate destinations from one source in ONE round-trip.

Each per-destination ``path`` must be byte-identical to the matching
scalar :func:`tier_spec_path` payload for the same ``(from, to)`` pair
-- pinned by the parity tests below so the scalar and batch path
accessors cannot drift.

Coverage:

* per-destination ``path`` byte-equal to the scalar ``tier_spec_path``
  payload
* per-destination ``direction`` derived from the same ranks the scalar
  endpoint uses
* batch envelope mirrors ``/tier-spec-path``'s envelope on the source
...
def test_helper_per_item_path_byte_equal_to_scalar(ent):
    ...
    :func:`tier_spec_path` payload for the same ``(from, to)`` pair."""
    candidates = [
        ent.TIER_CLOUD_FREE,
    ...
    out = ent.tier_spec_path_batch(ent.TIER_OSS, candidates)
    by_id = {item["to"]: item["path"] for item in out["tiers"]}
    for tid in candidates:
        scalar = ent.tier_spec_path(ent.TIER_OSS, tid)
...
def test_helper_per_item_direction_matches_rank_geometry(ent):
    ...
    out = ent.tier_spec_path_batch(ent.TIER_CLOUD_STARTER, candidates)
...
def test_helper_upgrade_walks_intermediate_rungs(ent):
    ...
    rungs = [row["id"] for row in item["path"]]
...
def test_helper_trial_destination_accepted(ent):
    """``trial`` is a valid endpoint (matching :func:`tier_spec_path`
    semantics) even though it is excluded from the walked intermediate
    rungs."""
    out = ent.tier_spec_path_batch(ent.TIER_OSS, [ent.TIER_TRIAL])
    assert out["unknown"] == []
...
def test_helper_unknown_from_tier_returns_none(ent):
    assert (
        ent.tier_spec_path_batch("not_a_tier", [ent.TIER_ENTERPRISE]) is None
    )
...
def test_helper_garbage_inputs_never_raise(ent):
    assert ent.tier_spec_path_batch("", []) is None
    assert ent.tier_spec_path_batch(None, None) is None  # type: ignore[arg-type]
    assert ent.tier_spec_path_batch("  ", "  ") is None
...
def test_helper_grace_and_enforce_yield_identical_output(ent, monkeypatch):
    ...
    grace = ent.tier_spec_path_batch(ent.TIER_OSS, candidates)
    monkeypatch.setenv("CLAWMETRY_ENFORCE", "1")
    ent.invalidate()
    enforced = ent.tier_spec_path_batch(ent.TIER_OSS, candidates)
...
def test_helper_row_failure_short_circuits_item(ent, monkeypatch):
    ...
    monkeypatch.setattr(ent, "tier_spec_path", fake)
    out = ent.tier_spec_path_batch(
...
# ── HTTP: /api/entitlement/tier-spec-path-batch ──────────────────────────────


def test_api_400_on_missing_from(client):
    r = client.get(
        "/api/entitlement/tier-spec-path-batch?to=cloud_pro,enterprise"
    )
    assert r.status_code == 400


def test_api_400_on_missing_to(client):
    r = client.get("/api/entitlement/tier-spec-path-batch?from=oss")
    assert r.status_code == 400
    body = r.get_json()
...
def test_api_400_on_empty_to(client):
    r = client.get("/api/entitlement/tier-spec-path-batch?from=oss&to=,,")
    assert r.status_code == 400


def test_api_404_on_unknown_from_tier(client):
    r = client.get(
        "/api/entitlement/tier-spec-path-batch?from=not_a_tier&to=enterprise"
    )
...
def test_api_downgrade_branch(client, ent):
    r = client.get(
        f"/api/entitlement/tier-spec-path-batch?from={ent.TIER_ENTERPRISE}"
        f"&to={ent.TIER_OSS}"
    )
    assert r.status_code == 200
    body = r.get_json()
...
def test_api_per_item_path_matches_scalar_route(client, ent):
    ...
    the scalar ``/tier-spec-path?from=&to=`` ``path`` payload for the
    same ``(from, to)`` pair."""
    candidates = [ent.TIER_CLOUD_STARTER, ent.TIER_CLOUD_PRO, ent.TIER_ENTERPRISE]
    batch = client.get(
        f"/api/entitlement/tier-spec-path-batch?from={ent.TIER_OSS}"
