...
# ---------------------------------------------------------------------------
# _reject_doctype -- entity expansion
# ---------------------------------------------------------------------------


def _entity_bomb(levels: int = 5, width: int = 10) -> bytes:
    """A JUnit document whose test name expands to width**levels characters."""
    names = "abcdefghij"[: levels + 1]
    decls = [f'<!ENTITY {names[0]} "{"a" * width}">']
    for prev, cur in zip(names, names[1:]):
        decls.append(f'<!ENTITY {cur} "{f"&{prev};" * width}">')
    return (
        '<?xml version="1.0"?>\n'
        "<!DOCTYPE testsuite [\n" + "\n".join(decls) + "\n]>\n"
        f'<testsuite><testcase classname="tests.test_foo" name="&{names[-1]};">'
        "<failure/></testcase></testsuite>"
    ).encode()


class TestRejectDoctype:
    def test_entity_expansion_is_refused(self):
        """A small document must not be able to expand into a large one."""
        bomb = _entity_bomb()
        with pytest.raises(ET.ParseError):
            aq._reject_doctype(bomb)

    def test_parse_of_entity_bomb_returns_empty(self):
        """_parse_junit_xml swallows it the same way it swallows malformed XML."""
        assert aq._parse_junit_xml(_entity_bomb()) == set()

    def test_doctype_without_entities_also_refused(self):
        xml = b'<?xml version="1.0"?><!DOCTYPE testsuite><testsuite/>'
        with pytest.raises(ET.ParseError):
            aq._reject_doctype(xml)
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            b"<!-- a comment mentioning <!DOCTYPE in passing -->",
        ],
    )
    def test_ordinary_prologs_are_accepted(self, prolog):
        """A legitimate JUnit document parses unchanged."""
        xml = prolog + (
            b'<testsuite><testcase classname="tests.test_foo" name="test_x">'
            b"<failure/></testcase></testsuite>"
        )
        aq._reject_doctype(xml)  # must not raise
        assert aq._parse_junit_xml(xml) == {"tests/test_foo.py::test_x"}

    def test_doctype_in_element_text_is_not_a_prolog_dtd(self):
        """The scan stops at the root element, so body text cannot false-positive."""
        xml = (
            b'<testsuite><testcase classname="tests.test_foo" name="test_x">'
            b"<failure>expected &lt;!DOCTYPE html&gt; here</failure>"
            b"</testcase></testsuite>"
        )
        aq._reject_doctype(xml)  # must not raise
        assert aq._parse_junit_xml(xml) == {"tests/test_foo.py::test_x"}

    def test_malformed_prolog_left_to_the_real_parser(self):
        """An unterminated PI is the parser's error to report, not ours."""
        aq._reject_doctype(b"<?xml version=")  # must not raise
        assert aq._parse_junit_xml(b"<?xml version=") == set()
