"""Tests for ``clawmetry.entitlements.runtime_catalog_at_path`` +
``runtime_catalog_at_path_batch`` and their two HTTP endpoints
``GET /api/entitlement/runtime-catalog-at-path`` +
``GET /api/entitlement/runtime-catalog-at-path-batch``.

Path-shaped what-if sibling of :func:`runtime_catalog_path` /
:func:`runtime_catalog_path_batch`: renders the FULL per-rung runtime-
catalog path between two tiers from a hypothetical ``perspective_tier``
in ONE round-trip. Fills the ``_at_path`` / ``_at_path_batch`` slots for
the runtime-catalog family so a pricing-comparison walkthrough surface
can call ``X_at_path(perspective, from, to)`` uniformly across every
``_at_path`` family member.

Pins:

* body byte-identical to :func:`runtime_catalog_path` /
  :func:`runtime_catalog_path_batch` for every perspective -- the
...
* per-rung row shape carries the same 4 keys as
  :func:`runtime_catalog_path` (``tier``, ``tier_label``, ``tier_rank``,
  ``runtimes``); each inner ``runtimes`` list byte-equals
  :func:`runtime_catalog_at(rung)` for the same rung, so the ``_at`` /
  ``_at_path`` / ``_at_batch`` / ``_at_path_batch`` runtime-catalog
...
* ``trial`` accepted as perspective and as destination (matching every
  ...
  :func:`runtime_catalog_path` which excludes trial from the walked
...
def test_scalar_returns_list(ent):
    path = ent.runtime_catalog_at_path(
...
def test_scalar_identity_yields_empty(ent):
    for tid in _all_tiers(ent):
        assert (
            ent.runtime_catalog_at_path(ent.TIER_CLOUD_PRO, tid, tid) == []
        )
...
# ─────────────────────────────────────────────────────────────────────────────
# scalar helper: byte-parity with runtime_catalog_path
# ─────────────────────────────────────────────────────────────────────────────


def test_scalar_body_parity_with_runtime_catalog_path(ent):
    """Body byte-identical to ``runtime_catalog_path(from, to)`` for every
    ...
    for p in tiers:
        for f in tiers:
            for t in tiers:
                got = ent.runtime_catalog_at_path(p, f, t)
                want = ent.runtime_catalog_path(f, t)
                assert got == want, (
...
def test_scalar_perspective_invariance(ent):
    ...
    baseline = ent.runtime_catalog_at_path(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, ent.TIER_ENTERPRISE
    )
    for p in _all_tiers(ent):
        assert ent.runtime_catalog_at_path(
            p, ent.TIER_OSS, ent.TIER_ENTERPRISE
        ) == baseline, f"perspective {p} drifted from cloud_pro baseline"


def test_scalar_per_rung_body_matches_runtime_catalog_at(ent):
    """Each per-rung ``runtimes`` list byte-equals
    :func:`runtime_catalog_at` for the same rung id -- pinned so the
    scalar and path what-if runtime-catalog surfaces cannot drift."""
    path = ent.runtime_catalog_at_path(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, ent.TIER_ENTERPRISE
    )
    for row in path:
...
def test_scalar_grace_vs_enforce_identical(ent, enforced):
    ...
    same rows."""
    grace_rows = ent.runtime_catalog_at_path(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, ent.TIER_ENTERPRISE
    )
    enforce_rows = enforced.runtime_catalog_at_path(
...
def test_scalar_delegate_crash_returns_none(ent, monkeypatch):
    """A ``runtime_catalog_path`` crash short-circuits to ``None``
    rather than surfacing an exception -- the delegate is wrapped in
    try/except."""

    def _boom(f, t):
        raise RuntimeError("boom")

    monkeypatch.setattr(ent, "runtime_catalog_path", _boom)
    assert ent.runtime_catalog_at_path(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, ent.TIER_ENTERPRISE
    ) is None
...
def test_batch_returns_dict_shape(ent):
    out = ent.runtime_catalog_at_path_batch(
        ent.TIER_CLOUD_PRO,
        ent.TIER_OSS,
...
def test_batch_body_parity_with_runtime_catalog_path_batch(ent):
    """For every perspective, batch body byte-equals
    :func:`runtime_catalog_path_batch(from, to_tiers)` for the same
    ``(from, to_tiers)``."""
    tiers = _all_tiers(ent)
    dests = [
    ...
    for p in tiers:
        for f in tiers:
            got = ent.runtime_catalog_at_path_batch(p, f, dests)
            want = ent.runtime_catalog_path_batch(f, dests)
            assert got == want, (
...
def test_batch_perspective_invariance(ent):
    ...
    baseline = ent.runtime_catalog_at_path_batch(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, dests
    )
    for p in _all_tiers(ent):
        assert ent.runtime_catalog_at_path_batch(
...
def test_batch_scalar_parity(ent):
    ...
    :func:`runtime_catalog_at_path(perspective, from, tid)` for the
    ...
    out = ent.runtime_catalog_at_path_batch(p, f, dests)
    by_id = {row["to"]: row for row in out["tiers"]}
    for tid in dests:
        assert (
            by_id[tid]["path"]
            == ent.runtime_catalog_at_path(p, f, tid)
        )
...
def test_batch_normalises_destinations(ent):
    got = ent.runtime_catalog_at_path_batch(
        "  cloud_pro  ",
        "  OSS  ",
        ["  Enterprise  ", "ENTERPRISE", "cloud_pro"],
    )
    want = ent.runtime_catalog_at_path_batch(
        ent.TIER_CLOUD_PRO,
        ent.TIER_OSS,
...
def test_batch_grace_vs_enforce_identical(ent, enforced):
    dests = [ent.TIER_CLOUD_PRO, ent.TIER_ENTERPRISE]
    grace_out = ent.runtime_catalog_at_path_batch(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, dests
    )
    enforce_out = enforced.runtime_catalog_at_path_batch(
...
def test_batch_never_raises_on_row_crash(ent, monkeypatch):
    """A per-destination ``runtime_catalog_path`` crash short-circuits
...
# ─────────────────────────────────────────────────────────────────────────────
# HTTP scalar: /runtime-catalog-at-path
# ─────────────────────────────────────────────────────────────────────────────


def test_http_scalar_happy_path(client, ent):
    r = client.get(
        "/api/entitlement/runtime-catalog-at-path"
    ...
    assert body["path"] == ent.runtime_catalog_at_path(
        ent.TIER_CLOUD_PRO, ent.TIER_OSS, ent.TIER_ENTERPRISE
    )
...
def test_http_scalar_body_parity_with_runtime_catalog_path(client, ent):
    """``path`` byte-parity with ``/runtime-catalog-path?from=&to=``
...
def test_http_scalar_perspective_invariance(client, ent):
    ...
    for p in ("oss", "cloud_free", "trial", "cloud_starter", "pro", "enterprise"):
        got = client.get(
            f"/api/entitlement/runtime-catalog-at-path"
...
# ─────────────────────────────────────────────────────────────────────────────
# HTTP batch: /runtime-catalog-at-path-batch
# ─────────────────────────────────────────────────────────────────────────────


def test_http_batch_happy_path(client, ent):
    r = client.get(
        "/api/entitlement/runtime-catalog-at-path-batch"
...
def test_http_batch_body_parity_with_runtime_catalog_path_batch(client):
    """``tiers[]`` byte-parity with ``/runtime-catalog-path-batch`` (the
...
def test_http_batch_perspective_invariance(client):
    ...
    for p in ("oss", "cloud_free", "trial", "cloud_starter", "pro", "enterprise"):
        got = client.get(
            f"/api/entitlement/runtime-catalog-at-path-batch"
