"""Tests for clawmetry/adapters/goose.py (FREE runtime, bundled in OSS).
...
def test_detect_when_present(db_path):
    ...
    # 3 local-Ollama sessions + 1 PAID (anthropic) row added so the non-zero
    # cost-parse path is covered (see _make_fixture.py).
...
def test_list_sessions_count_and_newest_first(db_path):
    ...
    # newest (highest updated_at) first; the paid row (20:10) is newest.
    assert [s.id for s in sessions] == [
        "20260525_4_paid", "20260525_3", "20260525_2", "20260525_1",
    ]


def test_paid_session_surfaces_exact_cost(db_path):
    """The PAID (anthropic) fixture row must surface accumulated_cost verbatim
    with cost_status='exact' — the only row that exercises non-zero cost (the
    Ollama rows are correctly $0). A broken paid-cost parse fails HERE."""
    by_id = {s.id: s for s in GooseAdapter(db_path=db_path).list_sessions()}
    paid = by_id["20260525_4_paid"]
    assert paid.source == "anthropic"
    assert paid.cost_usd is not None and paid.cost_usd > 0
    assert paid.cost_status == "exact"
...
def test_list_sessions_real_tokens(db_path):
    ...
    # local Ollama records no USD -> derived real $0.00 (not "unknown")
    assert s3.cost_usd == 0.0
    assert s3.cost_status == "derived"
...
def test_list_sessions_limit(db_path):
    ...
    # newest-first: the paid row (updated 20:10) sorts ahead of 20260525_3.
    assert sessions[0].id == "20260525_4_paid"
