...
| `clawmetry/detectors.py` | ~530 | The eight detectors' registry and orchestration. `run_all` normalizes once and shares the parse with every detector, then returns incidents ranked by what ignoring them costs. Holds the four **trajectory** detectors (is it stuck?): `stuck_loop`, `no_progress`, `repeated_tool_failure`, `action_discrepancy`. Re-exports the four modules below, so `clawmetry.detectors` stays the single import |
| `clawmetry/detector_core.py` | ~670 | Parse, normalize, redact. The shared floor: `normalize_events`, the action surface (paths / command / hosts, heredoc bodies stripped), and the redaction rules that decide what an incident may publish |
