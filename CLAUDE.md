...
| `clawmetry/detectors.py` | ~1,790 | Judge-free detectors over a session's tool stream, run by the daemon each tick. **Trajectory** (is it stuck?): `stuck_loop`, `no_progress`, `repeated_tool_failure`, `action_discrepancy`. **Behavioural** (is it doing something it does not normally do?): `file_blast_radius`, `credential_access`, `network_egress`, `privilege_change`. Also owns per-runtime threshold calibration (`resolve_thresholds`, `RUNTIME_PROFILES`) and the spend-at-risk model (`annotate_spend`) |
