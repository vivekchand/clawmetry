...
OURS = re.compile(r"^Clawmetry-Session:\s*(\S+)\s*$", re.MULTILINE)
COAUTH = re.compile(r"^Co-Authored-By:\s*Claude", re.MULTILINE | re.IGNORECASE)
PRNUM = re.compile(r"\(#(\d+)\)\s*$")


def sh(*a):
    return subprocess.run(a, cwd=REPO, capture_output=True, text=True, check=False).stdout
