...
CRON_RUN_TTL_SECS = 7 * 86400  # 7 days - dedup window for cron run timestamps
...
class HistoryCollector:
    ...
    def _cleanup_cron_runs(self):
        """Remove cron run timestamps older than CRON_RUN_TTL_SECS."""
        cutoff = time.time() - CRON_RUN_TTL_SECS
        for job_id in list(self._last_cron_runs.keys()):
            timestamps = self._last_cron_runs[job_id]
            to_remove = [ts for ts in timestamps if ts < cutoff]
            for ts in to_remove:
                timestamps.discard(ts)
            if not timestamps:
                del self._last_cron_runs[job_id]
