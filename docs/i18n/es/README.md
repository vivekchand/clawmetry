...
**Ve cómo piensa tu agente.** Observabilidad en tiempo real para **30 runtimes de agentes de IA**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex y 26 más. Un solo panel para toda tu flota de agentes.
...
- **Desbordamiento de contexto**: utilización de la ventana dimensionada por proveedor, compactación frente a desbordamiento forzado, además de un mapa por runtime de lo que *no podemos* ver ([cómo](docs/CONTEXT_BLOWOUT.md))
...
- **Alertas**: topes de presupuesto, picos de errores, agente desconectado, enrutadas a Slack, Discord, PagerDuty, Telegram, Email
...
proponer mediante PR](clawmetry/context_windows.py), que cubre Anthropic, OpenAI, Google, xAI,
DeepSeek, Kimi, Qwen, Mistral, Llama y GLM. No mide los 26 runtimes con la regla de un solo
proveedor. Eso importa: un turno de GPT-5 de 300K medido contra los 200K de Anthropic se lee
como ">100%, desbordado" cuando en realidad está al 75% de los 400K de GPT-5. Esa misma regla
oculta un turno de DeepSeek de 130K genuinamente desbordado como un cómodo 65%.

Cada ventana viene con su procedencia: `model_table`, `explicit_marker`,
`observed_floor`, o un `default` honesto cuando no conocemos el modelo. Un
...
"funcionó limpio" o "estamos ciegos"**. Un `0` que en realidad significa ceguera lo dice.
...
| Puerta de hook pre-herramienta (caché caliente) | **+44 ms** por llamada a herramienta con puerta, sobre un suelo de 36 ms del intérprete | desactivado |
...
último número está por encima de nuestro propio presupuesto declarado de 5-10%, así que se
...
Cada ruta está medida, incluyendo las puertas de hook y el proxy de aplicación de políticas,
...
el daemon actualmente sostiene alrededor del 12% de un núcleo, por encima de nuestro propio presupuesto de 5-10%.
El JSON en bruto, el método y lo que aún no está medido están en
...
a menos que ejecutes `clawmetry connect`** — ni prompts, respuestas, argumentos de herramientas,
contenido de archivos ni líneas de log. Cuando sí te conectas, la instantánea se cifra de extremo
a extremo con una clave que nunca sale de tu máquina, y se descifra en tu navegador. Si un
...
que se envía dentro del wheel y se sirve tal cual, fijado con un hash de Integridad
...
que cualquiera puede descargar. Autoalojar o quedarse solo en local elimina
...
| [Aprobaciones y políticas](docs/APPROVALS.md) | Filtrado antes de la ejecución, puntuación de riesgo, aprobaciones desde el teléfono |
...
Cada cifra a continuación proviene de una máquina real, leída en modo de solo lectura, sin nada preparado de antemano.

**Te dice cuándo algo está mal, no solo qué pasó.**
Dos banners de anomalías en la parte superior: el gasto corriendo 7 veces por encima del promedio diario, y un
...
factura. 14 agentes aquí, 3 sesiones trabajando, 13 en reposo.
...
a Bash y cada llamada al modelo obtiene su propia barra en la línea de tiempo, de modo que el comando que se
ejecutó durante 4.1 minutos y el que se ejecutó durante 226ms se distinguen de un vistazo.
...
que se activaron todas de forma proactiva en lugar de por un desbordamiento, además de la utilización de
cada turno detrás de ello.

![Context usage: window utilisation per turn, compaction events and tokens reclaimed](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/context.png)

**La detección funciona sin que configures nada.**
Los detectores integrados están activos desde la instalación: el agente se quedó en silencio, el
feed de telemetría se detuvo, pico de costo, ráfaga de tokens, errores en aumento, pico de errores, umbral
de presupuesto, firma de amenaza detectada, hallazgo de herramienta de seguridad, cambio en la postura
de seguridad. Tus propias reglas son opcionales, encima de esto.
...
no cambia nada. Una vez que una está activada, las llamadas que coincidan esperan aquí (o en tu teléfono)
para una aprobación o denegación.
