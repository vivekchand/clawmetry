...
Se abre en **http://localhost:8900** y ya está.
...
- **Transcripts** — Interfaz de burbujas de chat para leer el historial de sesiones
...
mensaje de permiso de Claude Code (ya respondiste). Las herramientas sin coincidencia cuestan ~40ms y
pasan al flujo de permisos normal de Claude Code. También recibes una notificación push en el teléfono cuando
...
La aplicación React v2 vive en `frontend/` y se sirve en `/v2` cuando el servidor Flask
se inicia con v2 habilitado.
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime distinto de OpenClaw incluye un adaptador de lectura dedicado que traduce el formato nativo de sus sesiones a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + snapshot en la nube, etiquetados con el runtime, y la pestaña Session replay muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para la matriz completa + una guía para añadir runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
| **Antigravity** | Adaptador beta | Brain JSONL bajo `~/.gemini/<flavor>/brain/`. Conversaciones, pasos de herramientas, razonamiento, desglose de tokens de Gemini por generación + costo, consumo por generación en segundo plano. |
...
## Rastrea cualquier agente basado en SDK — atribución de costos fuera del bucle

Los runtimes anteriores escriben sus sesiones en disco. Tu propio **agente de producción**, el que construiste con el OpenAI Agents SDK, LangChain, el Vercel AI SDK, LlamaIndex, E2B, o un simple bucle con `httpx`, no lo hace. El interceptor sin configuración de ClawMetry sigue capturando sus llamadas al LLM (costo, tokens, latencia, errores) parcheando dinámicamente `httpx`/`requests`:
...
**Exporta** cada sesión, llamadas al LLM, herramientas, subagentes, tokens, costo, como trazas OTLP/HTTP GenAI a cualquier colector (Datadog, Grafana, Honeycomb, o tu propio OTel Collector):
...
Obtienes el panel ClawMetry sin configuración y local por defecto **y** tus datos en el backend que tu equipo ya use, sin dependencia de proveedor, sin necesidad de instalar un segundo agente.
...
ClawMetry detecta automáticamente [NemoClaw](https://github.com/NVIDIA/NemoClaw), el contenedor de seguridad empresarial de NVIDIA para OpenClaw que ejecuta agentes dentro de contenedores OpenShell en sandbox.
...
Si necesitas ejecutar el daemon de sincronización **dentro** del sandbox OpenShell, añade esta regla de salida a tu política de red de NemoClaw para que pueda alcanzar la API de ingesta de ClawMetry:
...
| `localhost:8900` | 8900 | HTTP | Sí (panel local de la UI) |
...
|---|---|---|
| `install_id` | UUID aleatorio almacenado en `~/.clawmetry/install_id` | deduplicación; anónimo hasta que conectes explícitamente la sincronización con Cloud (el heartbeat autenticado del daemon entonces lo transporta, vinculando esta instalación a tu cuenta) |
...
a partir de la solicitud, y luego descarta la IP), nombre de host, nombre de usuario, ruta del espacio de trabajo,
contenido de archivos, tu api_key, tu correo electrónico, nada que sea PII o
específico del espacio de trabajo. La carga útil de la conexión se puede auditar en
[`clawmetry/telemetry.py`](clawmetry/telemetry.py).

**Optar por no participar** (cualquiera de estas la desactiva permanentemente):
