...
**Observa a tu agente pensar.** Observabilidad en tiempo real para **31 runtimes de agentes de IA**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex y 27 más. Un solo panel para toda tu flota de agentes.
...
Se abre en **http://localhost:8900**. Cero configuración: encuentra los runtimes de
agentes que ya tienes, los lee en modo de solo lectura y no cambia nada de cómo se ejecutan.
...
- **Sesiones y transcripciones**: qué hizo cada agente, turno a turno, con repetición
...
- **Sobrecarga de contexto**: utilización de la ventana dimensionada por proveedor, compactación frente a desbordamiento forzado, más un mapa por runtime de lo que *no podemos* ver ([cómo](docs/CONTEXT_BLOWOUT.md))
...
## Sobrecarga de contexto, y cuánto cuesta observar

Dos preguntas que vale la pena responder antes de confiar en cualquier herramienta de comparación de agentes.

**¿Cómo maneja la sobrecarga de la ventana de contexto entre runtimes?**

Un porcentaje de utilización solo es tan honesto como aquello por lo que se divide.
ClawMetry dimensiona la ventana por proveedor a partir de [una tabla que puedes leer y
...
comparado con los 200K de Anthropic se lee como ">100%, desbordado" cuando en realidad
...
| Puerta de enganche pre-herramienta (caché caliente) | **+44 ms** por llamada a herramienta con puerta, sobre un piso de intérprete de 36 ms | desactivado |
| Proxy de cumplimiento | **+9.7 ms** por llamada al LLM | desactivado |

Costo del host del daemon: **2,762 eventos/seg** de ingesta, **710 bytes/evento** en disco
(67.7 MB por 100k eventos), y **~12% de un núcleo** sostenido en una instalación ocupada. Ese
último número supera nuestro propio presupuesto declarado de 5-10%, así que se
publica como un bug a perseguir en lugar de omitirlo de la página.
...
Cada ruta está medida, incluidas las puertas de enganche y el proxy de cumplimiento,
...
el daemon actualmente sostiene cerca del 12% de un núcleo, por encima de nuestro propio presupuesto
de 5-10%. El JSON crudo, el método, y lo que aún no está medido están en
...
de archivos ni líneas de log. Cuando te conectas, el snapshot se cifra de extremo a extremo
...
Dos cosas sí se ejecutan por defecto antes de conectarte, ambas opcionales (opt-out) y ninguna
lleva datos de sesión: un ping anónimo de instalación y una comprobación de versión contra
PyPI. Una instalación por defecto también busca tu IP pública una vez para una línea de banner
de inicio. Cada destino, lo que transporta y cómo desactivarlo está listado en
[docs/EGRESS.md](docs/EGRESS.md); las instalaciones autoalojadas, redirigidas y con espacio de aire (air-gapped)
...
que se envía dentro del wheel y se sirve tal cual, fijado con un hash de Integridad de Subrecursos
...
| [Sobrecarga de contexto](docs/CONTEXT_BLOWOUT.md) | Ventanas por proveedor, compactación frente a desbordamiento, cobertura por runtime |
| [Sobrecarga (Overhead)](docs/OVERHEAD.md) | Cuánto cuesta la instrumentación, medido, con el arnés para reproducirlo |
...
| [OpenTelemetry](docs/OPENTELEMETRY.md) | Exporta trazas a cualquier parte, ingiere OTLP desde cualquier cosa |
...
| [Canales de chat](docs/CHANNELS.md) | Los adaptadores de chat que se muestran en Flujo |
...
Cada cifra a continuación proviene de una máquina real, en modo de solo lectura, sin nada preparado de antemano.

**Te dice cuándo algo está mal, no solo qué pasó.**
Dos banners de anomalías en la parte superior: el gasto corriendo 7 veces el promedio diario, y un
...
cerca de $1,128/mes desglosados como recuperables y $17,256/mes ya ahorrados
por la reutilización de caché.

![Cost: today, this week and this month, with an efficiency grade and itemised savings ideas](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/cost.png)

**Dibuja cómo un mensaje se convierte en una respuesta.**
El diagrama de flujo en vivo: tú, el canal por el que llegó, la puerta de enlace (gateway),
el modelo respondiendo en este momento, y cada herramienta que usó. Los nodos se iluminan
a medida que el trabajo se mueve a través de ellos.

![Flow: live diagram from you through the gateway to the model and its tools](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/flow.png)

**Cada agente de la máquina, en una sola tabla.**
Qué ejecuta, qué cuesta en las últimas 24 horas y en toda su vida útil, cuándo
...
Bash y cada llamada al modelo obtiene su propia barra en la línea de tiempo, de modo que el comando que se ejecutó
...
se detuvo, pico de costo, ráfaga de tokens, errores en aumento, pico de errores, umbral
de presupuesto, firma de amenaza detectada, hallazgo de herramienta de seguridad, cambio en la postura de
seguridad. Tus propias reglas son opcionales además de esto.
