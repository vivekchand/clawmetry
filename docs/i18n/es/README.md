...
**Observa a tu agente pensar.** Observabilidad en tiempo real para **14 runtimes de agentes de IA**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex y 10 más. Un solo panel para toda tu flota de agentes.
...
OpenClaw y NemoClaw son gratuitos en la app de código abierto; los demás runtimes se activan con ClawMetry Cloud o una licencia Pro autohospedada. Cambia de runtime desde el encabezado y cada pestaña (costo, tokens, herramientas, trazas) se reajusta a ese runtime. Consulta **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para conocer el reparto exacto entre gratuito y de pago, la matriz de niveles, la forma de `/api/entitlement` y la CLI de `clawmetry license`.

## Qué obtienes

- **Flow** — Diagrama animado en vivo que muestra los mensajes fluyendo por canales, brain, herramientas y de vuelta
- **Overview** — Comprobaciones de salud, mapa de calor de actividad, conteo de sesiones, información del modelo
- **Usage** — Seguimiento de tokens y costos con desgloses diarios/semanales/mensuales
- **Sessions** — Sesiones de agentes activas con modelo, tokens, última actividad
- **Crons** — Tareas programadas con estado, próxima ejecución, duración
- **Logs** — Transmisión de registros en tiempo real con colores
...
- **Alerts** — Límites de presupuesto, disparadores de tasa de error, detección de agente fuera de línea; enruta a Slack, Discord, PagerDuty, Telegram, Email
...
### 🧠 Brain — Transmisión en vivo de eventos del agente
...
**Bloqueo previo a la ejecución para Claude Code**: un solo comando instala
...
aviso de permisos de Claude Code (ya respondiste). Las herramientas sin coincidencia cuestan ~40ms y
caen al flujo de permisos normal de Claude Code. También recibes un aviso push en tu teléfono cuando el propio Claude Code
...
Abre `http://localhost:5173/v2/`. Vite reenvía las solicitudes `/api` a
...
Para compilar el paquete que se distribuye con el paquete Python:
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no sea OpenClaw incluye un adaptador lector dedicado que traduce su formato de sesión nativo a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + snapshot en la nube, etiquetados con el runtime, y la pestaña de repetición de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para ver la matriz completa y una guía para añadir runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
# ...tu agente se ejecuta normalmente; cada llamada al LLM ahora se rastrea y atribuye.
```

`set_source()` (o la variable de entorno `CLAWMETRY_SOURCE=support-agent`) etiqueta cada llamada con una **fuente nombrada**, de modo que cada producto que ejecutas aparece como su propia línea de primera clase y atribuible por costo en la tarjeta **🔌 Fuentes fuera del loop** del panel Overview: llamadas, proveedores, latencia, tasa de error por agente. ¿No se estableció ninguna fuente? Las llamadas se siguen rastreando; la tarjeta simplemente permanece oculta.
...
## OpenTelemetry: neutral respecto al proveedor, envía tus trazas a donde sea

ClawMetry habla **OpenTelemetry** en ambas direcciones, usando las **convenciones semánticas de GenAI**, de modo que las trazas de tu agente nunca quedan encerradas en una sola herramienta.

**Exporta** cada sesión (llamadas al LLM, herramientas, subagentes, tokens, costo) como spans GenAI de OTLP/HTTP hacia cualquier colector (Datadog, Grafana, Honeycomb, o tu propio OTel Collector):
...
CLAWMETRY_OTEL_EXPORT_INTERVAL=60                    # segundos (predeterminado 60)
```

**Ingesta**: el receptor OTLP integrado acepta trazas y métricas de cualquier otra fuente en `/v1/traces` y `/v1/metrics` (`pip install clawmetry[otel]` para ingesta de protobuf).

Obtienes el panel de ClawMetry sin configuración y local por defecto **y** tus datos en cualquier backend que tu equipo ya use, sin bloqueo de proveedor, sin necesidad de instalar un segundo agente.

## Configuración

La mayoría de las personas no necesita ninguna configuración. ClawMetry detecta automáticamente tu workspace, registros, sesiones y crons.

Si necesitas personalizar:

```bash
clawmetry --port 9000              # Puerto personalizado (predeterminado: 8900)
...
ClawMetry muestra actividad en vivo para cada canal de OpenClaw que tengas configurado. Solo los canales realmente configurados en tu `openclaw.json` aparecen en el diagrama de Flow; los no configurados se ocultan automáticamente.

Haz clic en cualquier nodo de canal en Flow para ver una vista en vivo de burbujas de chat con conteos de mensajes entrantes/salientes.
...
| 🔷 **Mattermost** | ✅ Completo | ✅ | Chat de equipo autohospedado |
...
> **Nota:** Al ejecutar en Docker, monta los directorios de datos + registros de tu agente (p. ej. `~/.openclaw`, `~/.claude`, `~/.codex`) para que ClawMetry pueda detectar automáticamente tu configuración.
...
2. **Detección de contenedor**: escanea los contenedores Docker en ejecución buscando imágenes `openshell`, `nemoclaw`, o `ghcr.io/nvidia/`, y luego lee las sesiones vía montajes de volumen o `docker cp`
...
Si debes ejecutar el daemon de sincronización **dentro** del sandbox de OpenShell, añade esta regla de salida a tu política de red de NemoClaw para que pueda llegar a la API de ingesta de ClawMetry:
...
para contar instalaciones reales (las cifras de descargas de PyPI en bruto son ~98% mirrors, CI,
...
|---|---|---|
| `install_id` | UUID aleatorio almacenado en `~/.clawmetry/install_id` | deduplicación; anónimo hasta que conectes explícitamente la sincronización con Cloud (el latido autenticado del daemon entonces lo transporta, vinculando esta instalación a tu cuenta) |
...
contenido de archivos, tu api_key, tu correo electrónico, nada que sea PII o
...
export DO_NOT_TRACK=1                          # estándar cruzado entre herramientas W3C
...
ping es fire-and-forget en un hilo daemon con un tiempo de espera de 3 s.
...
<p align="center">
  <strong>🦞 Observa a tu agente pensar</strong><br>
