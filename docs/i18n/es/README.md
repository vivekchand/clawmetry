...
**Observa cómo piensa tu agente.** Observabilidad en tiempo real para **21 runtimes de agentes de IA**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex y 17 más. Un solo panel para toda tu flota de agentes.
...
## Funciona con 21 runtimes de agentes
...
OpenClaw y NemoClaw son gratuitos en la aplicación de código abierto; los demás runtimes se activan con ClawMetry Cloud o una licencia Pro autoalojada. Cambia de runtime desde el encabezado y cada pestaña, costo, tokens, herramientas, trazas, se reajusta a ese runtime. Consulta **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para la división exacta entre gratuito y de pago, la matriz de niveles, la forma de `/api/entitlement` y la CLI `clawmetry license`.
...
- **Alerts**: Límites de presupuesto, disparadores por tasa de error, detección de agente desconectado; enruta a Slack, Discord, PagerDuty, Telegram, Email
- **Approvals**: Bloquea eliminaciones destructivas, force pushes, mutaciones de base de datos, sudo, instalaciones de paquetes, llamadas de red detrás de una autorización de un solo clic
...
### ✋ Approvals — Bloquea llamadas de herramientas riesgosas detrás de una autorización manual; reglas de protección respaldadas por políticas
![Approvals tab](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/approvals.png)

**Bloqueo previo a la ejecución para Claude Code**: un comando instala
un hook PreToolUse que pausa las llamadas a herramientas coincidentes *antes* de que se ejecuten y espera
...
permiso de Claude Code (ya respondiste). Las herramientas no coincidentes cuestan ~40ms y
pasan al flujo normal de permisos de Claude Code. También recibes una notificación push en tu teléfono cuando
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no es OpenClaw incluye un adaptador de lectura dedicado que traduce el formato nativo de sus sesiones a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + instantánea en la nube, etiquetados con el runtime, y la pestaña de repetición de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para la matriz completa y una guía para agregar runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
| **n8n** | Adaptador beta | SQLite `~/.n8n/database.sqlite`. Ejecuciones de flujo de trabajo, ejecuciones de nodos, prompts de AI Agent, modelo + tokens cuando n8n los registra. |
| **Antigravity** | Adaptador beta | Brain JSONL en `~/.gemini/<flavor>/brain/`. Conversaciones, pasos de herramientas, razonamiento, división de tokens de Gemini por generación + costo, consumo de generación en segundo plano. |
| **GitHub Copilot** | Adaptador beta | Copilot CLI `events.jsonl` en `~/.copilot/session-state/` + el libro de uso por llamada `session-store.db`. Conversaciones, llamadas a herramientas, enrutamiento de modelo, división de tokens con caché, costo facturado por el proveedor en créditos de IA. |
| **Grok** | Adaptador beta | xAI Grok Build CLI (binario en Rust en `~/.grok/bin/grok`): registro global de eventos `~/.grok/logs/unified.jsonl` + por sesión `~/.grok/sessions/<enc-cwd>/<uuid>/{events.jsonl,summary.json}`. Conversaciones, división de tokens por turno, enrutamiento de modelo, y el payload de repositorio saliente de la CLI almacenado en `~/.grok/upload_queue/` para que puedas ver qué salió de tu máquina. |

"Adaptador beta" significa que ClawMetry incluye un lector para el formato real en disco de ese runtime, cada uno construido y verificado contra una instalación real en una máquina real (consulta `tests/fixtures/runtimes/<rt>/`). Los adaptadores son de solo lectura; cada uno es honesto sobre lo que su runtime realmente almacena (por ejemplo, PicoClaw/NanoClaw/Cursor no escriben el costo de tokens en disco). Cuando varios runtimes se ejecutan en un mismo nodo, el selector de runtime limita la vista de sesiones a uno para un análisis detallado y limpio.

## Rastrea cualquier agente SDK — atribución de costos fuera del bucle

Los runtimes anteriores escriben todos sus sesiones en disco. Tu propio **agente de producción**, el que construiste sobre el OpenAI Agents SDK, LangChain, el Vercel AI SDK, LlamaIndex, E2B, o un bucle simple con `httpx`, no lo hace. El interceptor sin configuración de ClawMetry igualmente captura sus llamadas a LLM (costo, tokens, latencia, errores) mediante monkey-patching a `httpx`/`requests`:
...
`set_source()` (o la variable de entorno `CLAWMETRY_SOURCE=support-agent`) etiqueta cada llamada con una **fuente con nombre**, de modo que cada producto que ejecutas aparece como su propia línea independiente y atribuible por costo en la tarjeta **🔌 Fuentes fuera del bucle** del panel Overview: llamadas, proveedores, latencia, tasa de error por agente. ¿No configuraste una fuente? Las llamadas se siguen rastreando; la tarjeta simplemente permanece oculta.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Esta es la misma capa de datos que alimentan los adaptadores de runtime (DuckDB → instantánea en la nube), así que las fuentes fuera del bucle se sincronizan con el panel en la nube igual que todo lo demás, cifradas de extremo a extremo.
...
**Ingiere**: el receptor OTLP integrado acepta trazas, logs y métricas de cualquier otra fuente en `/v1/traces`, `/v1/logs` y `/v1/metrics`. Apunta cualquier aplicación instrumentada con OpenTelemetry hacia él:

```bash
OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:8900 OTEL_EXPORTER_OTLP_PROTOCOL=http/json your-app
```

Las trazas y logs de OTLP/JSON funcionan con un simple `pip install clawmetry`, sin extras. La ingesta de Protobuf (y las métricas de OTLP/JSON) necesita `pip install clawmetry[otel]`. Una aplicación que define su propio `service.name` aparece como su propio agente en el selector de runtime, con su costo y tokens.

Obtienes el panel de ClawMetry sin configuración y local primero, **y** tus datos en el backend que ya use tu equipo, sin bloqueos, sin necesidad de instalar un segundo agente.

## Configuración

La mayoría de las personas no necesita ninguna configuración. ClawMetry detecta automáticamente tu espacio de trabajo, logs, sesiones y crons.
...
ClawMetry muestra actividad en vivo para cada canal de OpenClaw que tengas configurado. Solo los canales que están realmente configurados en tu `openclaw.json` aparecen en el diagrama de Flow; los no configurados se ocultan automáticamente.
...
vez que ejecutas la CLI de `clawmetry` en una máquina nueva, un ping de `update`
en la primera ejecución tras actualizar a una nueva versión, y un ping de `onboarded`
cuando completas la elección de incorporación dentro del panel. Usamos esto
para contar instalaciones reales (las cifras de descargas en PyPI son ~98% mirrors, CI,
y redescargas de actualización automática) y para saber qué frameworks de agentes y
versiones están realmente en uso.

**Como máximo un POST por evento de ciclo de vida y por versión**, que contiene:

| Campo | Ejemplo | Por qué |
|---|---|---|
| `install_id` | UUID aleatorio almacenado en `~/.clawmetry/install_id` | deduplicación; anónimo hasta que conectes explícitamente la sincronización con Cloud (el heartbeat autenticado del daemon lleva entonces este dato, vinculando esta instalación con tu cuenta) |
...
espacio de trabajo, contenido de archivos, tu api_key, tu correo electrónico, nada que sea PII o
específico del espacio de trabajo. El payload en tránsito es auditable en
[`clawmetry/telemetry.py`](clawmetry/telemetry.py).

**Optar por no participar** (cualquiera de estas opciones lo desactiva permanentemente):
...
ping es de disparar y olvidar en un hilo del daemon con un tiempo de espera de 3 s.
