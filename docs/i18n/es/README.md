...
**Un agente puede hacer cien llamadas a herramientas sin lograr ningún progreso.** ClawMetry
...
Funciona con **30 runtimes de agentes de IA**: Claude Code, OpenAI Codex, Hermes, OpenClaw y 26 más. Un solo panel para toda tu flota de agentes. ([la lista completa](SUPPORTED_RUNTIMES.txt), generada a partir del catálogo.)
...
Un solo comando. Configuración cero. Detecta todo automáticamente.

```bash
pip install clawmetry && clawmetry
```

Se abre en **http://localhost:8900**. Configuración cero: encuentra los runtimes de agentes
que ya tienes, los lee en modo de solo lectura y no cambia nada sobre cómo se ejecutan.
...
| **Qué ves** | Línea de tiempo de la sesión, repetición herramienta por herramienta, desglose de tokens y costo, y señales de trayectoria (bucles, fallos repetidos), por runtime. |
| **Qué es gratis** | `pip install clawmetry` lee **OpenClaw, NVIDIA NemoClaw y Goose** sin necesidad de cuenta, clave ni llamadas de red. Los otros 27 (Claude Code, Codex, Cursor y el resto) son leídos por el complemento de código cerrado `clawmetry-pro`, que llega con la prueba de 7 días o con un plan; consulta [docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md) para conocer el reparto exacto. |
...
| **Qué sale de tu máquina** | Ningún dato de sesión, a menos que ejecutes `clawmetry connect`. Dos cosas se ejecutan por defecto, ambas con opción de exclusión y ninguna transporta contenido de sesión: un ping de instalación anónimo y una comprobación de versión en PyPI. Cada destino está inventariado en [docs/EGRESS.md](docs/EGRESS.md), reconstruido a partir de una captura de tráfico en lugar de a partir de la lectura de comentarios. |

Dos limitaciones que vale la pena conocer antes de juzgar el resultado: los runtimes exponen
datos muy diferentes (algunos no publican costo en absoluto; [la matriz](docs/compatibility.md)
...
encabezado reajusta el alcance de cada pestaña a uno de ellos.
...
- **Cerebro**: el flujo de eventos de razonamiento y llamadas a herramientas a medida que ocurre
...
- **Alertas**: límites de presupuesto, picos de error, agente sin conexión, enrutadas a Slack, Discord, PagerDuty, Telegram, Email
- **Aprobaciones**: pausa las llamadas a herramientas riesgosas *antes* de que se ejecuten y apruébalas desde tu teléfono ([cómo](docs/APPROVALS.md))

## Desbordamiento de contexto, y qué cuesta observarlo
...
Un porcentaje de utilización es tan honesto como aquello entre lo que se divide. ClawMetry
...
comparado contra los 200K de Anthropic se lee como ">100%, desbordado" cuando en realidad está al 75% de
...
ClawMetry solo puede ver eventos de compactación en algunos runtimes. Así que
`GET /api/context-coverage` informa, por runtime, si un **cero significa
"se ejecutó limpio" o "estamos ciegos"**. Un `0` que en realidad significa ciego lo indica.
[Detalle completo](docs/CONTEXT_BLOWOUT.md)

**¿Qué cuesta la instrumentación?**
...
| Puerta de enlace pre-herramienta (caché en caliente) | **+44 ms** por llamada a herramienta controlada, sobre un piso de intérprete de 36 ms | desactivado |
| Proxy de aplicación | **+9.7 ms** por llamada al LLM | desactivado |
...
activa. Ese último número supera nuestro propio presupuesto declarado de 5-10%, así que se
publica como un error a resolver en lugar de omitirse de la página.

Medido en un Apple M2 Pro con `benchmarks/overhead.py`. El arnés de pruebas ejecuta
cada condición en un proceso separado, alterna su orden, y **se niega a
imprimir un número cuando las rondas no coinciden en su signo**. Ejecútalo en tu propia
...
Cada ruta se mide, incluidas las puertas de enlace de hooks y el proxy de aplicación,
y el arnés de pruebas se ejecuta en Linux, macOS y Windows en CI. Dos resultados que vale la pena
conocer: el proxy cuesta alrededor de siete veces más en Windows que en Linux, y
el daemon actualmente sostiene alrededor del 12% de un núcleo, por encima de nuestro propio presupuesto
de 5-10%. El JSON en bruto, el método y lo que aún no está medido están en
...
funcionan sin la nube (`clawmetry license`). El reparto exacto entre gratis y de pago está
...
a menos que ejecutes `clawmetry connect`**: sin prompts, respuestas, argumentos de herramientas, contenido de
archivos ni líneas de registro. Cuando te conectas, la instantánea se cifra de extremo a extremo
...
Dos cosas se ejecutan por defecto antes de conectarte, ambas con opción de exclusión y ninguna
transportando datos de sesión: un ping de instalación anónimo y una comprobación de versión contra
PyPI. Una instalación predeterminada también consulta tu IP pública una vez para una línea de
banner de inicio. Cada destino, lo que transporta y cómo desactivarlo está listado en
[docs/EGRESS.md](docs/EGRESS.md); las instalaciones autoalojadas, redirigidas y aisladas
no realizan ninguna llamada saliente discrecional en absoluto.

El descifrado ocurre en tu navegador, en código que te servimos. Eso solía ser
...
que se incluye dentro del wheel y se sirve tal cual, fijado con un hash de integridad de subrecursos
(Subresource Integrity). Para confirmar que el navegador ejecuta lo que publicamos:
...
que cualquiera puede descargar. Autoalojarse o permanecer solo local elimina
...
O deja que el agente lo configure por ti. La habilidad [`agent-kill-switch`](skills/agent-kill-switch/SKILL.md)
...
detener una sesión a solicitud, y retener llamadas a herramientas riesgosas para su aprobación:
...
| [Sobrecarga](docs/OVERHEAD.md) | Qué cuesta la instrumentación, medido, con el arnés de pruebas para reproducirlo |
| [Derechos de uso (Entitlements)](docs/ENTITLEMENTS.md) | Gratis frente a de pago, matriz de niveles, CLI de licencia |
...
| [Trae tu propio agente](docs/BRING_YOUR_OWN_AGENT.md) | AWS AgentCore, Pydantic AI, LangChain de extremo a extremo, con ejemplos ejecutables |
| [Seguimiento de SDK](docs/SDK_TRACKING.md) | Atribución de costos para agentes que construiste tú mismo |
| [Canales de chat](docs/CHANNELS.md) | Los adaptadores de chat mostrados en Flujo |
...
Cada número a continuación proviene de una máquina real, en modo de solo lectura, sin nada sembrado artificialmente.
...
detrás de él y cuánto de eso ya cubre tu suscripción. Debajo de eso,
...
a Bash y cada llamada al modelo obtiene su propia barra en la línea de tiempo, para que el comando que se ejecutó
durante 4.1 minutos y el que se ejecutó en 226ms se distingan de un vistazo.

![Sessions: one agent turn on a timeline, every tool call with its own duration and the turn's cost](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/sessions.png)

**Califica el trabajo, no solo el gasto.**
Una A esta semana: 54 tareas volvieron limpias, 2 accidentadas costaron $48.57, y las
ejecuciones con muy poca actividad para juzgar se dejan fuera de la calificación en lugar de
contarse como victorias. Cada ejecución accidentada enlaza a su traza.
...
cada turno detrás de ella.

![Context usage: window utilisation per turn, compaction events and tokens reclaimed](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/context.png)

**La detección funciona sin que configures nada.**
Los detectores integrados están activos desde la instalación: el agente se quedó en silencio, la fuente de
telemetría se detuvo, pico de costo, ráfaga de tokens, errores en aumento, pico de errores, umbral
de presupuesto, firma de amenaza coincidente, hallazgo de herramienta de seguridad, cambio en la postura de
seguridad. Tus propias reglas son opcionales además de estas.

![Alerts: built-in detectors plus optional custom rules](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/alerts.png)

**Retener una llamada riesgosa es opcional, y se entrega desactivado.**
