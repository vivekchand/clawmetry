...
OpenClaw y NemoClaw son gratuitos en la aplicación de código abierto; los demás runtimes se activan con ClawMetry Cloud o una licencia Pro autoalojada. Cambia de runtime desde el encabezado y cada pestaña (costo, tokens, herramientas, trazas) se reajusta a ese runtime. Consulta **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para conocer la división exacta entre gratuito y de pago, la matriz de niveles, la forma de `/api/entitlement` y la CLI `clawmetry license`.
...
- **Crons**: tareas programadas con estado, próxima ejecución y duración
...
- **Alerts**: límites de presupuesto, disparadores de tasa de error, detección de agente fuera de línea; se enrutan a Slack, Discord, PagerDuty, Telegram, Email
- **Approvals**: bloquea eliminaciones destructivas, force pushes, mutaciones de base de datos, sudo, instalaciones de paquetes y llamadas de red detrás de una aprobación con un solo clic
...
permiso de Claude Code (ya respondiste). Las herramientas no coincidentes cuestan ~40 ms y
continúan hacia el flujo normal de permisos de Claude Code. También recibes una notificación push en tu teléfono cuando el propio Claude Code
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no sea OpenClaw incluye un adaptador de lectura dedicado que traduce el formato nativo de su sesión a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + instantánea en la nube, etiquetados con el runtime, y la pestaña de reproducción de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para ver la matriz completa y una guía para agregar runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.

¿Estás usando la herramienta de seguridad de agentes [numbat de Perplexity](https://github.com/perplexityai/numbat)? ClawMetry ingiere sus hallazgos y decisiones de aplicación de políticas de manera integrada; consulta [`docs/NUMBAT.md`](docs/NUMBAT.md).
...
| **GitHub Copilot** | Adaptador beta | `events.jsonl` de Copilot CLI bajo `~/.copilot/session-state/` + el libro de uso por llamada `session-store.db`. Conversaciones, llamadas a herramientas, enrutamiento de modelo, desglose de tokens con caché, costo en créditos de IA facturados por el proveedor. |
| **Grok** | Adaptador beta | xAI Grok Build CLI (binario en Rust bajo `~/.grok/bin/grok`): registro de eventos global `~/.grok/logs/unified.jsonl` + por sesión `~/.grok/sessions/<enc-cwd>/<uuid>/{events.jsonl,summary.json}`. Conversaciones, desglose de tokens por turno, enrutamiento de modelo, y el payload de repositorio saliente de la CLI almacenado en `~/.grok/upload_queue/` para que puedas ver qué salió de tu máquina. |

"Adaptador beta" significa que ClawMetry incluye un lector para el formato real en disco de ese runtime, cada uno construido y verificado contra una instalación real en una máquina real (ver `tests/fixtures/runtimes/<rt>/`). Los adaptadores son de solo lectura; cada uno es transparente sobre lo que su runtime realmente almacena (por ejemplo, PicoClaw/NanoClaw/Cursor no escriben el costo de tokens en disco). Cuando varios runtimes se ejecutan en un mismo nodo, el selector de runtime limita la vista de sesiones a uno solo para un análisis detallado y ordenado.

## Rastrea cualquier agente basado en SDK: atribución de costos fuera del loop

Los runtimes anteriores escriben sesiones en disco. Tu propio **agente de producción**, ya sea construido sobre el OpenAI Agents SDK, LangChain, el Vercel AI SDK, LlamaIndex, E2B, o un simple loop con `httpx`, no lo hace. El interceptor sin configuración de ClawMetry sigue capturando sus llamadas al LLM (costo, tokens, latencia, errores) mediante monkey-patching de `httpx`/`requests`:
...
Esta es la misma capa de datos que alimentan los adaptadores de runtime (DuckDB → instantánea en la nube), así que las fuentes fuera del loop se sincronizan con el panel en la nube igual que todo lo demás, cifradas de extremo a extremo.
...
Obtienes el panel de ClawMetry sin configuración y local por defecto, **y** tus datos en el backend que ya usa tu equipo, sin bloqueo de proveedor ni necesidad de instalar un segundo agente.
...
ClawMetry detecta automáticamente [NemoClaw](https://github.com/NVIDIA/NemoClaw), el envoltorio de seguridad empresarial de NVIDIA para OpenClaw que ejecuta agentes dentro de contenedores OpenShell en espacio aislado (sandbox).
...
Si debes ejecutar el daemon de sincronización **dentro** del sandbox de OpenShell, agrega esta regla de salida (egress) a tu política de red de NemoClaw para que pueda acceder a la API de ingesta de ClawMetry:
...
en la primera ejecución después de actualizar a una nueva versión, y un ping de `onboarded`
cuando completas la elección de incorporación dentro del panel. Usamos esto
para contar instalaciones reales (los números brutos de descargas de PyPI son ~98% espejos, CI
...
|---|---|---|
| `install_id` | UUID aleatorio almacenado en `~/.clawmetry/install_id` | deduplicación; anónimo hasta que conectes explícitamente la sincronización en la nube (el latido de daemon autenticado luego lo transporta, vinculando esta instalación a tu cuenta) |
| `event` | `install` / `update` / `onboarded` | instalación nueva frente a actualización de una existente |
...
contenido de archivos, tu api_key, tu correo electrónico, ni nada de PII o
específico del workspace. La carga útil en tránsito es auditable en
...
ping es de disparar y olvidar (fire-and-forget) en un hilo del daemon con un tiempo de espera de 3 s.
