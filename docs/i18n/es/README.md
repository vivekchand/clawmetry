...
lee los archivos de sesión que tus agentes de codificación ya generan, y reúne la línea de tiempo,
...
Funciona con **31 runtimes de agentes de IA**: Claude Code, OpenAI Codex, Hermes, OpenClaw y 27 más. Un solo panel para toda tu flota de agentes. ([la lista completa](SUPPORTED_RUNTIMES.txt), generada a partir del catálogo.)
...
| | |
|---|---|
| **Qué hace** | Lee los archivos de sesión y los logs que tus agentes ya generan. Sin SDK, sin cambios de código, sin instrumentación en tu aplicación. |
...
| **Qué sale de tu máquina** | Ningún dato de sesión, a menos que ejecutes `clawmetry connect`. Por defecto sí se ejecutan dos cosas, ambas opcionales y ninguna transporta contenido de sesión: un ping anónimo de instalación y una verificación de versión en PyPI. Cada destino está inventariado en [docs/EGRESS.md](docs/EGRESS.md), reconstruido a partir de una captura de tráfico y no de la lectura de comentarios. |

Vale la pena conocer dos límites antes de juzgar el resultado: los runtimes exponen datos muy
distintos (algunos no publican costo alguno; [la matriz](docs/compatibility.md) indica cuáles,
por runtime), y observar una acción no es lo mismo que poder bloquearla ([qué controles son reales, por runtime](docs/APPROVALS.md)).
...
Todos los runtimes obtienen el mismo panel. Ejecuta varios a la vez y el selector del encabezado
reencuadra cada pestaña a uno de ellos.

¿Construiste tu propio agente sobre un SDK en lugar de usar uno de estos? El interceptor también
...
- **Flujo**: diagrama en vivo de los mensajes que se mueven entre canales, modelos y herramientas
- **Brain**: el flujo de eventos de razonamiento y llamadas a herramientas mientras ocurre
...
- **Aprobaciones**: pausa las llamadas a herramientas de riesgo *antes* de que se ejecuten y apruébalas desde tu teléfono ([cómo](docs/APPROVALS.md))
...
Un porcentaje de utilización es tan honesto como el divisor que usa. ClawMetry
...
DeepSeek, Kimi, Qwen, Mistral, Llama y GLM. No mide los 31
runtimes con la regla de un solo proveedor. Eso importa: un turno de GPT-5 de 300K
comparado contra los 200K de Anthropic se lee como ">100%, desbordado" cuando en realidad
está al 75% de los 400K de GPT-5. La misma regla oculta un turno de DeepSeek de 130K
genuinamente desbordado, mostrándolo como un cómodo 65%.
...
"corrió limpio" o "estamos ciegos"**. Un `0` que en realidad significa ceguera lo indica.
...
|---|---|---|
| Seguimiento de archivos de sesión (los 31 runtimes) | **0**. Proceso separado, sin código de ClawMetry en tu agente | activado |
| Interceptor HTTP (`CLAWMETRY_INTERCEPT=1`) | **+0.44 ms** por llamada al LLM, o 0.009% de una llamada de 5s | desactivado |
| Compuerta del hook previo a la herramienta (caché caliente) | **+44 ms** por llamada a herramienta con gate, sobre un piso de intérprete de 36 ms | desactivado |
...
con mucha actividad. Ese último número supera nuestro propio presupuesto declarado del 5-10%, así que
se publica como un bug a perseguir en lugar de omitirse de la página.
...
Cada ruta está medida, incluyendo las compuertas de hooks y el proxy de aplicación de
políticas, y el harness corre en Linux, macOS y Windows en CI. Dos resultados que vale la pena
...
del 5-10%. El JSON crudo, el método, y lo que todavía no está medido están en
...
| **Pro** | Starter + control y evaluación: aprobaciones, políticas de riesgo de herramientas, evaluaciones, detección de anomalías, optimizador de costos, exportación a OTel, registro de auditoría a prueba de manipulaciones | $19 por nodo / mes |
...
funcionan sin la nube (`clawmetry license`). El reparto exacto entre gratis y de pago
está en [docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md).
...
de archivos ni líneas de log. Cuando sí te conectas, la instantánea se cifra de extremo a extremo
...
Por defecto sí se ejecutan dos cosas antes de que te conectes, ambas opcionales y ninguna
transporta datos de sesión: un ping anónimo de instalación y una verificación de versión contra
PyPI. Una instalación por defecto también consulta tu IP pública una vez para una línea del
mensaje de inicio. Cada destino, qué transporta y cómo desactivarlo está listado en
...
que se distribuye dentro del wheel y se sirve tal cual, fijado con un hash de Integridad de
Subrecursos. Para confirmar que el navegador ejecuta lo que publicamos:
...
Lo que esto no demuestra: nosotros servimos la página que carga el archivo, así que podríamos
...
detener una sesión a solicitud, y retener llamadas a herramientas de riesgo para su aprobación:
...
| [Overhead](docs/OVERHEAD.md) | Qué cuesta la instrumentación, medido, con el harness para reproducirlo |
| [Entitlements](docs/ENTITLEMENTS.md) | Gratis frente a de pago, matriz de niveles, CLI de licencias |
...
| [OpenTelemetry](docs/OPENTELEMETRY.md) | Exporta trazas a cualquier destino, ingiere OTLP desde cualquier fuente |
...
**Te dice cuándo algo está mal, no solo lo que pasó.**
Dos avisos de anomalías en la parte superior: gasto corriendo 7 veces por encima del promedio
diario, y un pico de costo de 4.2x. Debajo, 324 de 667 sesiones recientes con una
...
detrás y cuánto de ello ya cubre tu suscripción. Debajo, alrededor de $1,128/mes
desglosados como recuperables y $17,256/mes ya ahorrados por
...
que responde en este momento, y cada herramienta a la que recurrió. Los nodos se iluminan mientras el trabajo
...
se le vio por última vez, quién es su dueño, y si una suscripción está cubriendo la
...
a Bash y cada llamada al modelo obtiene su propia barra en la línea de tiempo, de modo que el comando
que corrió durante 4.1 minutos y el que corrió durante 226ms se distinguen de un vistazo.
...
que se activaron todas de forma proactiva en lugar de por desbordamiento, y la utilización de
cada turno detrás de eso.

![Context usage: window utilisation per turn, compaction events and tokens reclaimed](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/context.png)

**La detección funciona sin que configures nada.**
Los detectores integrados están activos desde la instalación: el agente quedó en silencio, la
alimentación de telemetría se detuvo, pico de costo, ráfaga de tokens, errores en aumento, pico de errores, umbral
de presupuesto, firma de amenaza detectada, hallazgo de herramienta de seguridad, cambio de postura de
seguridad. Tus propias reglas son opcionales, encima de esto.

![Alerts: built-in detectors plus optional custom rules](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/alerts.png)

**Retener una llamada de riesgo es opcional, y se envía desactivado.**
...
no cambia nada. Una vez que activas una, las llamadas que coinciden esperan aquí (o en tu teléfono)
una aprobación o un rechazo.
