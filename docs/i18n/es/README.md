...
> 🌐 **Leer esto en:** [English](README.md) · [简体中文](docs/i18n/zh-CN/README.md) · [日本語](docs/i18n/ja/README.md) · [한국어](docs/i18n/ko/README.md) · [Español](docs/i18n/es/README.md) · [Português (BR)](docs/i18n/pt-BR/README.md) · [Français](docs/i18n/fr/README.md) · [Deutsch](docs/i18n/de/README.md) · [हिन्दी](docs/i18n/hi/README.md) · [العربية](docs/i18n/ar/README.md) · [Русский](docs/i18n/ru/README.md) · [más →](docs/i18n/)
...
pasan al flujo de permisos normal de Claude Code. También recibes una notificación push en el teléfono cuando el propio Claude Code está esperando tu respuesta (notificaciones
`permission_prompt` / `idle_prompt`).

## Instalación

**Línea única (recomendado):**
```bash
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime distinto de OpenClaw incluye un adaptador lector dedicado que traduce su formato de sesión nativo a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + instantánea en la nube, etiquetados con el runtime, y la pestaña de repetición de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para ver la matriz completa y una guía para agregar runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
`set_source()` (o la variable de entorno `CLAWMETRY_SOURCE=support-agent`) etiqueta cada llamada con una **fuente con nombre**, de modo que cada producto que ejecutas aparece como su propia línea de primera clase, atribuible por costo, en la tarjeta **🔌 Fuentes fuera del loop** del panel en Overview: llamadas, proveedores, latencia, tasa de error por agente. ¿No configuraste una fuente? Las llamadas se siguen rastreando; la tarjeta simplemente permanece oculta.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Esta es la misma capa de datos que alimentan los adaptadores de runtime (DuckDB → instantánea en la nube), así que las fuentes fuera del loop se sincronizan con el panel en la nube igual que todo lo demás, con cifrado E2E.
...
**Exporta** cada sesión (llamadas a LLM, herramientas, sub-agentes, tokens, costo) como spans GenAI de OTLP/HTTP a cualquier colector (Datadog, Grafana, Honeycomb, o tu propio OTel Collector):
...
**Ingesta**: el receptor OTLP integrado acepta trazas y métricas de cualquier otra fuente en `/v1/traces` y `/v1/metrics` (`pip install clawmetry[otel]` para ingesta con protobuf).

Obtienes el panel de ClawMetry sin configuración, local por defecto, **y** tus datos en cualquier backend que tu equipo ya utilice: sin dependencia de un proveedor, sin necesidad de instalar un segundo agente.
...
ClawMetry muestra actividad en vivo para cada canal de OpenClaw que tengas configurado. Solo los canales que estén realmente configurados en tu `openclaw.json` aparecen en el diagrama de Flow; los que no estén configurados se ocultan automáticamente.
...
2. **Detección de contenedor** — escanea los contenedores de Docker en ejecución en busca de imágenes `openshell`, `nemoclaw`, o `ghcr.io/nvidia/`, y luego lee las sesiones vía volúmenes montados o `docker cp`
...
espacio de trabajo, contenido de archivos, tu api_key, tu correo electrónico, ni nada de información personal identificable o específica del espacio de trabajo. La carga útil transmitida es auditable en
