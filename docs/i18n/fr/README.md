...
ClawMetry a démarré comme outil d'observabilité pour OpenClaw, et mesure désormais **toute votre flotte d'agents** dans un seul tableau de bord, en détectant automatiquement chaque runtime présent sur votre machine :
...
- **Approvals** — Bloque les suppressions destructrices, les force push, les mutations de base de données, sudo, les installations de paquets, les appels réseau derrière une validation en un clic
...
### ✋ Approvals — Bloque les appels d'outils risqués derrière une validation manuelle ; règles de protection basées sur des politiques
...
votre décision (un seul geste depuis votre téléphone avec les
...
Un refus bloque uniquement cet appel d'outil, l'agent conserve sa session et peut
...
retombent dans le flux de permission normal de Claude Code. Vous recevez également une notification push sur votre téléphone lorsque Claude Code lui-même attend votre réponse (notifications `permission_prompt` /
`idle_prompt`).
...
| **n8n** | Adaptateur bêta | SQLite `~/.n8n/database.sqlite`. Exécutions de workflows, exécutions de nœuds, invites d'AI Agent, modèle + tokens lorsque n8n les enregistre. |

« Adaptateur bêta » signifie que ClawMetry fournit un lecteur pour le format sur disque réel de ce runtime, chacun construit et vérifié sur une installation réelle sur une machine réelle (voir `tests/fixtures/runtimes/<rt>/`). Les adaptateurs sont en lecture seule ; chacun reflète honnêtement ce que son runtime stocke réellement (par exemple, PicoClaw/NanoClaw/Cursor n'écrivent pas le coût en tokens sur disque). Lorsque plusieurs runtimes s'exécutent sur un même nœud, le sélecteur de runtime recentre la vue des sessions sur un seul pour une analyse approfondie et claire.

## Suivre n'importe quel agent SDK — attribution des coûts hors boucle

Les runtimes ci-dessus écrivent tous des sessions sur disque. Votre propre **agent de production**, celui que vous avez construit sur le SDK OpenAI Agents, LangChain, le SDK Vercel AI, LlamaIndex, E2B, ou une simple boucle `httpx`, ne le fait pas. L'intercepteur sans configuration de ClawMetry capture tout de même ses appels LLM (coût, tokens, latence, erreurs) en patchant dynamiquement `httpx`/`requests` :
...
`set_source()` (ou la variable d'environnement `CLAWMETRY_SOURCE=support-agent`) étiquette chaque appel avec une **source nommée**, de sorte que chaque produit que vous exécutez apparaît comme sa propre ligne de premier ordre, attribuable en termes de coûts, dans la carte **🔌 Sources hors boucle** du tableau de bord sur Overview, appels, fournisseurs, latence, taux d'erreur par agent. Aucune source définie ? Les appels sont tout de même suivis ; la carte reste simplement masquée.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Il s'agit de la même couche de données que celle alimentée par les adaptateurs de runtime (DuckDB → instantané cloud), donc les sources hors boucle se synchronisent avec le tableau de bord cloud comme tout le reste, chiffrées de bout en bout.
...
**Exportez** chaque session, appels LLM, outils, sous-agents, tokens, coûts, sous forme de traces OTLP/HTTP GenAI vers n'importe quel collecteur (Datadog, Grafana, Honeycomb, ou votre propre OTel Collector) :
...
Les en-têtes d'authentification et l'intervalle d'interrogation sont des variables d'environnement facultatives :
...
Vous obtenez le tableau de bord ClawMetry sans configuration, local d'abord, **et** vos données dans le backend que votre équipe utilise déjà, sans verrouillage propriétaire, sans second agent à installer.
...
> **Détection automatique :** ClawMetry lit votre `~/.openclaw/openclaw.json` et n'affiche que les canaux que vous avez réellement configurés. Aucune configuration manuelle n'est requise.
...
# Ou monter le répertoire de données de votre agent (exemple : ~/.openclaw d'OpenClaw)
...
ClawMetry détecte automatiquement [NemoClaw](https://github.com/NVIDIA/NemoClaw), l'enveloppe de sécurité d'entreprise de NVIDIA pour OpenClaw qui exécute les agents à l'intérieur de conteneurs OpenShell isolés.
...
1. **Détection binaire** — vérifie la présence de la CLI `nemoclaw` et exécute `nemoclaw status` pour obtenir les informations du bac à sable
...
### Facultatif : nom de bac à sable explicite
...
| Socket Docker (`/var/run/docker.sock`) | — | Socket Unix | Pour la découverte de session par conteneur |
...
ClawMetry envoie un unique ping anonyme de « première exécution » vers
...
à partir de la requête, puis rejette l'IP), nom d'hôte, nom d'utilisateur, chemin de l'espace de
travail, contenu des fichiers, votre api_key, votre email, ou toute donnée personnelle ou
...
export DO_NOT_TRACK=1                          # norme W3C multi-outils
