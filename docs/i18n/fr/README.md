...
## Fonctionne avec 14 runtimes d'agents
...
OpenClaw et NemoClaw sont gratuits dans l'application open source ; les autres runtimes s'activent avec ClawMetry Cloud ou une licence Pro autohébergée. Changez de runtime depuis l'en-tête et chaque onglet, coût, tokens, outils, traces, se recentre sur ce runtime. Voir **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** pour la répartition exacte gratuit/payant, la matrice des niveaux, la forme de `/api/entitlement`, et la CLI `clawmetry license`.

## Ce que vous obtenez

- **Flow** — Diagramme animé en direct montrant les messages circulant entre canaux, cerveau, outils, et retour
- **Overview** — Contrôles de santé, carte thermique d'activité, nombre de sessions, informations sur le modèle
...
- **Transcripts** — Interface de type bulles de discussion pour lire l'historique des sessions
...
retombent dans le flux de permission normal de Claude Code. Vous recevez aussi une notification push sur votre téléphone quand Claude Code lui-même attend votre réponse (notifications `permission_prompt` /
`idle_prompt`).
...
Ouvrez `http://localhost:5173/v2/`. Vite fait office de proxy pour les requêtes `/api` vers
...
**Ingestion** — le récepteur OTLP intégré accepte les traces et métriques provenant de n'importe quelle autre source sur `/v1/traces` et `/v1/metrics` (`pip install clawmetry[otel]` pour l'ingestion protobuf).

Vous obtenez le tableau de bord ClawMetry zéro configuration, local d'abord, **et** vos données dans le backend que votre équipe utilise déjà, sans verrouillage, sans second agent à installer.
...
Cliquez sur n'importe quel nœud de canal dans Flow pour voir une vue en bulles de discussion en direct avec les décomptes de messages entrants/sortants.
...
| 📡 **IRC** | ✅ Complet | ✅ | Interface bulles de type terminal |
...
| 🔷 **Mattermost** | ✅ Complet | ✅ | Chat d'équipe autohébergé |
...
# Ou monter le répertoire de données de votre agent (affiché : le ~/.openclaw d'OpenClaw)
...
> **Remarque :** Lors de l'exécution dans Docker, montez les répertoires de données + logs de votre agent (par ex. `~/.openclaw`, `~/.claude`, `~/.codex`) afin que ClawMetry puisse détecter automatiquement votre configuration.
...
ClawMetry détecte automatiquement [NemoClaw](https://github.com/NVIDIA/NemoClaw), l'enveloppe de sécurité d'entreprise de NVIDIA pour OpenClaw qui exécute les agents dans des conteneurs OpenShell en bac à sable.

Aucune configuration supplémentaire n'est nécessaire dans la plupart des cas. Le daemon de synchronisation détecte automatiquement les fichiers de session, qu'ils se trouvent dans `~/.openclaw/` sur l'hôte ou à l'intérieur d'un conteneur OpenShell.
...
`https://app.clawmetry.com/api/install` la première fois que vous exécutez la
CLI `clawmetry` sur une nouvelle machine. Nous utilisons cela pour compter les installations (la
seule métrique marketing dont nous disposons pour un projet open source) et pour savoir quels
...
|---|---|---|
| `install_id` | UUID aléatoire stocké dans `~/.clawmetry/install_id` | dédoublonnage ; non lié à votre email ou api_key |
...
| `is_ci` / `ci_provider` | `true` / `github_actions` | séparer les installations humaines du bruit de CI |

**Ce que nous n'envoyons PAS** : IP (le cloud dérive le code pays côté serveur
à partir de la requête, puis rejette l'IP), nom d'hôte, nom d'utilisateur, chemin de l'espace de travail,
contenu de fichiers, votre api_key, votre email, ni rien de PII ou de
spécifique à l'espace de travail. La charge utile réseau est auditable dans
...
ping est envoyé sans attente de réponse sur un thread daemon avec un délai d'expiration de 3 s.
