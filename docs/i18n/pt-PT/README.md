...
O OpenClaw e o NemoClaw são gratuitos na aplicação open-source; os outros runtimes ativam-se com o ClawMetry Cloud ou uma licença Pro self-hosted. Mude de runtime a partir do cabeçalho e todos os separadores, custo, tokens, ferramentas, traces, reajustam-se a esse runtime. Consulte **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para a divisão exata entre gratuito/pago, a matriz de níveis, a estrutura de `/api/entitlement` e a CLI `clawmetry license`.

## O que obtém

- **Flow** — Diagrama animado ao vivo que mostra as mensagens a fluir por canais, cérebro, ferramentas e de volta
- **Overview** — Verificações de saúde, mapa de calor de atividade, contagem de sessões, informação do modelo
- **Usage** — Acompanhamento de tokens e custos com decomposições diárias/semanais/mensais
...
### 💰 Tokens — Decomposição de custos por modelo e sessão
...
### ✋ Approvals — Bloqueia chamadas de ferramentas de risco atrás de aprovação manual; regras de proteção com base em políticas
![Approvals tab](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/approvals.png)

**Bloqueio antes da execução para o Claude Code** — um comando instala um
hook PreToolUse que pausa as chamadas de ferramentas correspondentes *antes* de serem executadas e aguarda
pela sua decisão (um toque a partir do seu telemóvel com
...
Uma recusa bloqueia apenas essa chamada de ferramenta, o agente mantém a sua sessão e pode
...
está à espera de si (notificações `permission_prompt` / `idle_prompt`).
...
A aplicação React v2 está em `frontend/` e é servida em `/v2` quando o
servidor Flask é iniciado com o v2 ativado.

Utilize dois terminais durante o desenvolvimento:

```bash
# Terminal 1: Flask API/servidor em :8900
...
"Adaptador beta" significa que o ClawMetry disponibiliza um leitor para o formato real em disco desse runtime, cada um construído + verificado com uma instalação real numa máquina real (ver `tests/fixtures/runtimes/<rt>/`). Os adaptadores são apenas de leitura; cada um é transparente sobre o que o respetivo runtime efetivamente armazena (por exemplo, o PicoClaw/NanoClaw/Cursor não escrevem o custo em tokens em disco). Quando vários runtimes correm num único nó, o seletor de runtime restringe a vista de sessões a um só, para uma análise aprofundada limpa.

## Acompanhe qualquer agente SDK — atribuição de custo out-loop

Os runtimes acima escrevem todos as sessões em disco. O seu próprio **agente de produção**, o que construiu com o OpenAI Agents SDK, LangChain, o Vercel AI SDK, LlamaIndex, E2B, ou um simples loop `httpx`, não o faz. O interceptor de configuração zero do ClawMetry continua a capturar as suas chamadas LLM (custo, tokens, latência, erros) através de monkey-patching a `httpx`/`requests`:

```python
import clawmetry.track            # ativa o interceptor
clawmetry.track.set_source("support-agent")   # nomeie este produto

# ...o seu agente corre normalmente; cada chamada LLM é agora monitorizada + atribuída.
```

`set_source()` (ou a variável de ambiente `CLAWMETRY_SOURCE=support-agent`) marca cada chamada com uma **fonte nomeada**, para que cada produto que executar apareça como uma linha própria, de primeira classe e com custo atribuível, no cartão **🔌 Out-loop sources** do painel, na secção Overview, chamadas, fornecedores, latência, taxa de erro por agente. Nenhuma fonte definida? As chamadas continuam a ser monitorizadas; o cartão apenas permanece oculto.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Esta é a mesma camada de dados que os adaptadores de runtime alimentam (DuckDB → snapshot na cloud), pelo que as fontes out-loop sincronizam com o painel na cloud tal como tudo o resto, com encriptação E2E.

## OpenTelemetry — neutro em relação ao fornecedor, envie os seus traces para qualquer lugar

O ClawMetry fala **OpenTelemetry** em ambas as direções, utilizando as **convenções semânticas GenAI**, para que os traces do seu agente nunca fiquem presos a uma única ferramenta.

**Exporte** cada sessão, chamadas LLM, ferramentas, sub-agentes, tokens, custo, como spans GenAI OTLP/HTTP para qualquer collector (Datadog, Grafana, Honeycomb, ou o seu próprio OTel Collector):
...
**Ingira** — o receptor OTLP incorporado aceita traces e métricas de qualquer outra fonte em `/v1/traces` e `/v1/metrics` (`pip install clawmetry[otel]` para ingestão protobuf).

Obtém o painel ClawMetry de configuração zero e local-first **e** os seus dados no backend que a sua equipa já utiliza, sem lock-in, sem um segundo agente para instalar.
...
O ClawMetry mostra atividade ao vivo para cada canal do OpenClaw que tenha configurado. Apenas os canais que estão realmente configurados no seu `openclaw.json` aparecem no diagrama Flow; os não configurados ficam automaticamente ocultos.

Clique em qualquer nó de canal no Flow para ver uma vista de balões de chat ao vivo, com contagens de mensagens de entrada/saída.
...
> **Deteção automática:** O ClawMetry lê o seu `~/.openclaw/openclaw.json` e apenas renderiza os canais que efetivamente configurou. Não é necessária configuração manual.
...
# Ou montar o diretório de dados do seu agente (mostrado: o ~/.openclaw do OpenClaw)
...
> **Nota:** Ao executar no Docker, monte os diretórios de dados + logs do seu agente (por exemplo, `~/.openclaw`, `~/.claude`, `~/.codex`) para que o ClawMetry consiga detetar automaticamente a sua configuração.
...
2. **Deteção de contentor** — analisa contentores Docker em execução à procura de imagens `openshell`, `nemoclaw`, ou `ghcr.io/nvidia/`, e depois lê as sessões via montagens de volume ou `docker cp`

Os ficheiros de sessão sincronizados a partir de contentores NemoClaw são marcados com `runtime=nemoclaw` e metadados `container_id` no painel da cloud, para que os consiga distinguir das sessões padrão do OpenClaw à primeira vista.
...
`https://app.clawmetry.com/api/install` na primeira vez que executa a CLI
`clawmetry` numa nova máquina. Utilizamos isto para contar instalações (a
única métrica de marketing que temos para um projeto OSS) e para saber quais
as frameworks de agentes que os nossos utilizadores têm instaladas.
...
**O que NÃO enviamos**: IP (a cloud deriva o código do país no servidor
a partir do pedido e depois descarta o IP), hostname, nome de utilizador, caminho do
...
**Opt-out** (qualquer uma destas opções desativa-o permanentemente):
...
fire-and-forget numa thread daemon, com um timeout de 3 s.
