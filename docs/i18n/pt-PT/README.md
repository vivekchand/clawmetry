...
**Veja o seu agente a pensar.** Observabilidade em tempo real para **25 runtimes de agentes de IA**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex e mais 19. Um único painel para toda a sua frota de agentes.
...
regista as suas chamadas ao LLM. Consulte [docs/SDK_TRACKING.md](docs/SDK_TRACKING.md).
...
- **Brain**: o fluxo de eventos de raciocínio e chamadas a ferramentas, em tempo real
...
- **Alertas**: limites de orçamento, picos de erros, agente offline, encaminhados para Slack, Discord, PagerDuty, Telegram, Email
- **Aprovações**: pause chamadas a ferramentas de risco *antes* de serem executadas e aprove a partir do telemóvel ([como](docs/APPROVALS.md))
...
self-hosted funcionam sem a cloud (`clawmetry license`). A divisão exata entre
...
menos que execute `clawmetry connect`. Mesmo nesse caso, o snapshot é
encriptado de ponta a ponta com uma chave que nunca sai da sua máquina, e é
desencriptado no seu navegador.
...
| [SDK tracking](docs/SDK_TRACKING.md) | Atribuição de custos para agentes construídos por si |
...
| **Custo**: por modelo e sessão | **Aprovações**: controla chamadas a ferramentas de risco |
