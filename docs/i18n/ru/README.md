...
**Наблюдайте за мышлением вашего агента.** Наблюдаемость в реальном времени для **14 сред выполнения AI-агентов**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex и ещё 10 других. Единая панель для всего вашего флота агентов.
...
Откроется по адресу **http://localhost:8900** — и всё готово.

![Flow Visualization](https://clawmetry.com/screenshots/flow.png)

## Работает с 14 средами выполнения агентов

ClawMetry начинался как инструмент наблюдаемости для OpenClaw, а теперь измеряет **весь ваш флот агентов** на одной панели, автоматически определяя каждую среду выполнения на вашей машине:
...
OpenClaw и NemoClaw бесплатны в open-source приложении; остальные среды выполнения включаются с ClawMetry Cloud или лицензией Pro для самостоятельного хостинга. Переключайте среды выполнения из шапки, и каждая вкладка — стоимость, токены, инструменты, трассировки — пересчитывается для выбранной среды. Смотрите **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** для точного разделения на бесплатное/платное, матрицы уровней, формата `/api/entitlement` и CLI `clawmetry license`.
...
- **Alerts** — лимиты бюджета, триггеры по частоте ошибок, обнаружение отключения агента; отправка в Slack, Discord, PagerDuty, Telegram, Email
- **Approvals** — блокировка деструктивных удалений, force push, изменений в БД, sudo, установок пакетов, сетевых вызовов до получения одобрения в один клик
...
### ✋ Approvals — блокировка рискованных вызовов инструментов до ручного одобрения; правила защиты на основе политик
...
Claude Code (вы уже ответили). Несовпавшие инструменты стоят ~40 мс и
попадают в обычный процесс разрешений Claude Code. Вы также получите push-уведомление на телефон, когда
...
Приложение React v2 находится в `frontend/` и раздаётся по пути `/v2`, когда сервер
Flask запущен с включённым v2.
...
Чтобы собрать сборку, которая поставляется вместе с Python-пакетом:
...
ClawMetry наблюдает за многими средами выполнения AI-агентов, а не только за OpenClaw. Каждая среда выполнения, отличная от OpenClaw, поставляется с выделенным адаптером-ридером, который переводит её нативный формат сессий в унифицированные форматы ClawMetry; демон загружает их в то же хранилище DuckDB + облачный снимок, помечая среду выполнения, а вкладка воспроизведения сессий показывает **переключатель сред выполнения**, если присутствует больше одной. Смотрите [`docs/compatibility.md`](docs/compatibility.md) для полной матрицы и руководства по добавлению сред выполнения, а также [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) для введения в семейство OpenClaw.
...
| **Claude Code** | Бета-адаптер | JSONL `~/.claude/projects/.../<id>.jsonl`. Транскрипты, модель, вызовы инструментов + мышление, использование токенов. |
...
"Бета-адаптер" означает, что ClawMetry поставляет ридер для реального формата данных этой среды выполнения на диске, каждый из которых собран и проверен на реальной установке на реальной машине (см. `tests/fixtures/runtimes/<rt>/`). Адаптеры доступны только для чтения; каждый честно отражает то, что среда выполнения на самом деле хранит (например, PicoClaw/NanoClaw/Cursor не записывают стоимость токенов на диск). Когда на одном узле работает несколько сред выполнения, переключатель сред выполнения ограничивает представление сессий одной средой для чистого детального анализа.

## Отслеживание любого SDK-агента — атрибуция затрат вне цикла

Все перечисленные выше среды выполнения записывают сессии на диск. Ваш собственный **продакшн-агент** — тот, который вы построили на OpenAI Agents SDK, LangChain, Vercel AI SDK, LlamaIndex, E2B или обычном цикле на `httpx` — этого не делает. Перехватчик ClawMetry без настройки всё равно фиксирует его вызовы LLM (стоимость, токены, задержку, ошибки), встраиваясь в `httpx`/`requests`:
...
**Экспорт** каждой сессии — вызовов LLM, инструментов, суб-агентов, токенов, стоимости — в виде спанов OTLP/HTTP GenAI в любой коллектор (Datadog, Grafana, Honeycomb или ваш собственный OTel Collector):
...
Заголовки авторизации и интервал опроса — опциональные переменные окружения:
...
Вы получаете панель ClawMetry без настройки, локальную по умолчанию, **и** ваши данные в любом бэкенде, который уже использует ваша команда — без привязки к поставщику, без второго агента для установки.
...
ClawMetry показывает живую активность для каждого настроенного вами канала OpenClaw. В диаграмме Flow отображаются только те каналы, которые действительно настроены в вашем `openclaw.json` — ненастроенные автоматически скрываются.

Нажмите на любой узел канала во Flow, чтобы увидеть живой вид чат-пузырей со счётчиками входящих/исходящих сообщений.
...
| 📡 **IRC** | ✅ Полностью | ✅ | Интерфейс пузырей в стиле терминала |
...
| 🔷 **Mattermost** | ✅ Полностью | ✅ | Самостоятельно размещённый командный чат |
...
- Среда выполнения AI-агента на той же машине: OpenClaw, NVIDIA NemoClaw, Claude Code, Codex, Cursor, Goose, Hermes, opencode, Qwen Code, Aider, NanoClaw, PicoClaw, Pi, Deep Agents или n8n (либо смонтированные тома для Docker)
- Linux или macOS
...
1. **Обнаружение по бинарнику** — проверяет наличие CLI `nemoclaw` и запускает `nemoclaw status`, чтобы получить информацию о песочнице
2. **Обнаружение по контейнеру** — сканирует запущенные Docker-контейнеры на наличие образов `openshell`, `nemoclaw` или `ghcr.io/nvidia/`, затем читает сессии через смонтированные тома или `docker cp`

Файлы сессий, синхронизированные из контейнеров NemoClaw, помечаются метаданными `runtime=nemoclaw` и `container_id` в облачной панели, чтобы вы могли отличить их от обычных сессий OpenClaw с первого взгляда.
...
Если автоопределение не срабатывает, укажите ClawMetry нужную песочницу:
...
чтобы подсчитывать реальные установки (сырые цифры загрузок PyPI примерно на 98% состоят из зеркал, CI
и повторных загрузок при автообновлении) и узнавать, какие фреймворки агентов и
...
| Поле | Пример | Зачем |
|---|---|---|
| `install_id` | случайный UUID, хранящийся в `~/.clawmetry/install_id` | дедупликация; анонимно, пока вы явно не подключите облачную синхронизацию (аутентифицированный heartbeat демона затем несёт его, связывая эту установку с вашим аккаунтом) |
...
| `is_ci` / `ci_provider` | `true` / `github_actions` | отделение установок реальных людей от шума CI |

**Что мы НЕ отправляем**: IP (облако вычисляет код страны на стороне сервера
из запроса, а затем отбрасывает IP), имя хоста, имя пользователя, путь к рабочему
пространству, содержимое файлов, ваш api_key, вашу электронную почту, ничего из персональных данных или данных,
специфичных для рабочего пространства. Передаваемые данные можно проверить в
...
отправляется в режиме "выстрелил и забыл" в отдельном потоке демона с тайм-аутом 3 с.
