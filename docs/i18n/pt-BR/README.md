...
**Um agente pode fazer cem chamadas de ferramentas sem fazer progresso algum.** O ClawMetry
lê os arquivos de sessão que seus agentes de codificação já escrevem, e reúne a linha do tempo,
as chamadas de ferramentas e quaisquer dados de tokens e custo que o runtime exponha em uma única
visualização — para que você consiga distinguir uma execução longa que está funcionando de uma que está travada.
...
| | |
|---|---|
| **O que ele faz** | Lê os arquivos de sessão e logs que seus agentes já escrevem. Sem SDK, sem mudança de código, sem instrumentação na sua aplicação. |
| **O que você vê** | Linha do tempo da sessão, replay ferramenta por ferramenta, detalhamento de tokens e custo, e sinais de trajetória (loop, falhas repetidas) — por runtime. |
| **O que é gratuito** | `pip install clawmetry` lê **OpenClaw, NVIDIA NemoClaw e Goose** sem necessidade de conta, chave ou chamada de rede. Os outros 27 — Claude Code, Codex, Cursor e o restante — são lidos pelo complemento de código fechado `clawmetry-pro`, que chega com o teste de 7 dias ou um plano — veja [docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md) para a divisão exata. |
...
| **O que sai da sua máquina** | Nenhum dado de sessão, a menos que você execute `clawmetry connect`. Duas coisas rodam por padrão, ambas com opção de desativar e nenhuma delas carregando conteúdo de sessão: um ping anônimo de instalação e uma verificação de versão no PyPI. Todo destino está inventariado em [docs/EGRESS.md](docs/EGRESS.md), reconstruído a partir de uma captura de tráfego em vez de a partir da leitura de comentários. |

Dois limites que vale a pena conhecer antes de julgar o resultado: os runtimes expõem
dados bem diferentes (alguns não publicam custo algum — [a matriz](docs/compatibility.md)
mostra quais, por runtime), e observar uma ação não é o mesmo que ser capaz de
bloqueá-la ([quais controles são reais, por runtime](docs/APPROVALS.md)).


## Funciona com 31 runtimes de agentes

**Gratuito no aplicativo open source:** 🦞 **[OpenClaw](https://clawmetry.com/runtimes/openclaw)** · 🟩 **[NVIDIA NemoClaw](https://clawmetry.com/nemoclaw)** · 🪿 **[Goose](https://clawmetry.com/runtimes/goose)**
...
Construiu seu próprio agente usando um SDK em vez disso? O interceptor rastreia as
chamadas de LLM dele também. Veja [docs/SDK_TRACKING.md](docs/SDK_TRACKING.md).
...
- **Brain**: o fluxo de eventos de raciocínio e chamadas de ferramenta em tempo real
...
Uma porcentagem de utilização é tão honesta quanto o valor pelo qual ela é dividida. O ClawMetry
...
DeepSeek, Kimi, Qwen, Mistral, Llama e GLM. Ele não mede os 31
runtimes com a régua de um único fornecedor. Isso importa: um turno de 300K do GPT-5
pontuado contra os 200K da Anthropic aparece como ">100%, estourado" quando na verdade está em 75% dos
...
`observed_floor`, ou um honesto `default` quando não sabemos o modelo. Um
medidor construído sobre um palpite nunca é renderizado com a mesma autoridade de um construído
...
**Qual é o custo da instrumentação?**

| Caminho | Adicionado ao seu agente | Padrão? |
|---|---|---|
| Leitura contínua do arquivo de sessão (todos os 31 runtimes) | **0**. Processo separado, nenhum código do ClawMetry no seu agente | ativado |
| Interceptor HTTP (`CLAWMETRY_INTERCEPT=1`) | **+0,44 ms** por chamada de LLM, ou 0,009% de uma chamada de 5s | desativado |
| Gate de hook pré-ferramenta (cache aquecido) | **+44 ms** por chamada de ferramenta controlada, sobre um piso de 36 ms do interpretador | desativado |
| Proxy de aplicação de políticas | **+9,7 ms** por chamada de LLM | desativado |

Custo do daemon no host: **2.762 eventos/s** de ingestão, **710 bytes/evento** em disco
...
publicado como um bug a ser perseguido em vez de deixado de fora da página.
...
**[clawmetry.com/pricing](https://clawmetry.com/pricing)**. Chaves de licença auto-hospedadas
...
nó não tiver uma chave, o envio é ignorado em vez de enviado sem criptografia, e nenhuma
resposta do servidor pode desativar isso.

Duas coisas rodam por padrão antes de você conectar, ambas com opção de desativar e nenhuma
carregando dados de sessão: um ping anônimo de instalação e uma verificação de versão contra o
PyPI. Uma instalação padrão também consulta seu IP público uma vez para uma linha de
banner na inicialização. Todo destino, o que ele carrega e como desativá-lo está listado em
...
A descriptografia acontece no seu navegador, em código que fornecemos a você. Isso costumava ser
...
que é distribuído dentro do wheel e servido literalmente, fixado com um hash de Integridade
...
servir uma página diferente. Hashes de integridade protegem você de uma CDN comprometida,
...
que qualquer um pode obter. Auto-hospedar ou permanecer apenas local remove a
...
Ou o comando de uma linha: `curl -sSL https://raw.githubusercontent.com/vivekchand/clawmetry/main/install.sh | bash`
...
Todo número abaixo é de uma máquina real, somente leitura, sem nada semeado artificialmente.
...
chamada de Bash e chamada de modelo recebe sua própria barra na linha do tempo, para que o comando que rodou
por 4,1 minutos e o que rodou por 226ms sejam diferenciados rapidamente.
...
serem contadas como acertos. Cada execução problemática tem link para seu trace.
...
que dispararam de forma proativa em vez de por estouro, e a utilização de
...
parou, pico de custo, explosão de tokens, erros aumentando, pico de erros, limite
de orçamento, assinatura de ameaça encontrada, achado de ferramenta de segurança, postura de segurança
...
recebem, cada uma, uma regra que você pode ativar. Até você fazer isso, o ClawMetry observa e
não muda nada. Uma vez ativada, as chamadas correspondentes esperam aqui (ou no seu celular)
por uma aprovação ou uma negação.
...
MIT · Desenvolvido por [@vivekchand](https://github.com/vivekchand) · [clawmetry.com](https://clawmetry.com)
