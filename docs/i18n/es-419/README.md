...
- **Alerts**: límites de presupuesto, disparadores por tasa de error, detección de agente fuera de línea; se enruta a Slack, Discord, PagerDuty, Telegram, Email
...
hook de PreToolUse que pausa las llamadas a herramientas coincidentes *antes* de que se ejecuten y espera
...
permiso de Claude Code (ya lo respondiste). Las herramientas sin coincidencia cuestan ~40ms y
caen al flujo de permisos normal de Claude Code. También recibes un push al teléfono cuando Claude Code
...
La app v2 en React vive en `frontend/` y se sirve en `/v2` cuando el
...
Abre `http://localhost:5173/v2/`. Vite redirige las solicitudes de `/api` a
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no sea OpenClaw incluye un adaptador de lectura dedicado que traduce el formato nativo de sus sesiones a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + snapshot en la nube, etiquetados con el runtime, y la pestaña de reproducción de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para ver la matriz completa y una guía para añadir runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
"Adaptador beta" significa que ClawMetry incluye un lector para el formato real en disco de ese runtime, cada uno construido y verificado contra una instalación real en una máquina real (ver `tests/fixtures/runtimes/<rt>/`). Los adaptadores son de solo lectura; cada uno es honesto sobre lo que su runtime realmente almacena (por ejemplo, PicoClaw/NanoClaw/Cursor no escriben el costo de tokens en disco). Cuando corren varios runtimes en un mismo nodo, el selector de runtime acota la vista de sesiones a uno solo para un análisis limpio y detallado.
...
`set_source()` (o la variable de entorno `CLAWMETRY_SOURCE=support-agent`) etiqueta cada llamada con una **fuente nombrada**, así que cada producto que ejecutes aparece como su propia línea de primera clase, atribuible por costo, en la tarjeta **🔌 Fuentes fuera del loop** del panel de Overview: llamadas, proveedores, latencia, tasa de error por agente. ¿No configuraste una fuente? Las llamadas se siguen rastreando; la tarjeta simplemente permanece oculta.
...
ClawMetry habla **OpenTelemetry** en ambas direcciones, usando las **convenciones semánticas de GenAI**, así que las trazas de tu agente nunca quedan atadas a una sola herramienta.
...
Obtienes el panel de ClawMetry sin configuración y local por defecto, **y** tus datos en el backend que ya use tu equipo: sin ataduras, sin necesidad de instalar un segundo agente.
...
> **Detección automática:** ClawMetry lee tu `~/.openclaw/openclaw.json` y solo renderiza los canales que realmente configuraste. No requiere configuración manual.
...
1. **Detección por binario**: verifica la existencia de la CLI `nemoclaw` y ejecuta `nemoclaw status` para obtener información del sandbox
2. **Detección por contenedor**: escanea los contenedores de Docker en ejecución en busca de imágenes `openshell`, `nemoclaw`, o `ghcr.io/nvidia/`, y luego lee las sesiones vía volúmenes montados o `docker cp`

Los archivos de sesión sincronizados desde contenedores de NemoClaw se etiquetan con `runtime=nemoclaw` y metadatos de `container_id` en el panel de la nube, para que puedas distinguirlos de las sesiones estándar de OpenClaw de un vistazo.
...
Si debes ejecutar el daemon de sincronización **dentro** del sandbox de OpenShell, añade esta regla de egreso a tu política de red de NemoClaw para que pueda alcanzar la API de ingesta de ClawMetry:
...
El daemon de sincronización solo hace llamadas HTTPS salientes a `ingest.clawmetry.com`. No se requieren puertos entrantes.
...
workspace, contenido de archivos, tu api_key, tu correo, nada de PII o
específico del workspace. La carga útil de la conexión es auditable en
[`clawmetry/telemetry.py`](clawmetry/telemetry.py).

**Optar por no participar** (cualquiera de estas la deshabilita permanentemente):
...
ping es de tipo "fire-and-forget" en un hilo daemon con un tiempo de espera de 3 s.
