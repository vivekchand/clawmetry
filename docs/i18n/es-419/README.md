...
**Mira pensar a tu agente.** Observabilidad en tiempo real para **31 runtimes de agentes de IA**: [OpenClaw](https://github.com/openclaw/openclaw), [NVIDIA NemoClaw](https://github.com/NVIDIA/NemoClaw), Claude Code, OpenAI Codex y 27 más. Un solo dashboard para toda tu flota de agentes.
...
Se abre en **http://localhost:8900**. Cero configuración: encuentra los runtimes de agentes que ya tienes, los lee en modo solo lectura y no cambia nada de cómo funcionan.
...
Cada runtime obtiene el mismo dashboard. Corre varios a la vez y el selector del encabezado reenfoca cada pestaña a uno de ellos.

¿Construiste tu propio agente sobre un SDK en vez de esto? El interceptor también rastrea sus llamadas a LLM. Consulta [docs/SDK_TRACKING.md](docs/SDK_TRACKING.md).
...
- **Flujo**: diagrama en vivo de los mensajes moviéndose por canales, modelos y herramientas
- **Brain**: el flujo de eventos de razonamiento y llamadas a herramientas a medida que ocurre
- **Explosión de contexto**: uso de la ventana dimensionado por proveedor, compactación vs. desbordamiento forzado, más un mapa por runtime de lo que *no* podemos ver ([cómo](docs/CONTEXT_BLOWOUT.md))
...
ClawMetry solo puede ver eventos de compactación en algunos runtimes. Por eso `GET /api/context-coverage` reporta, por runtime, si un **cero significa "corrió limpio" o "estamos ciegos"**. Un `0` que en realidad significa ciego lo dice así. [Detalle completo](docs/CONTEXT_BLOWOUT.md)
...
Costo en el host del daemon: **2,762 eventos/seg** de ingesta, **710 bytes/evento** en disco
(67.7 MB por 100k eventos), y **~12% de un núcleo** sostenido en una instalación con carga.
Ese último número supera nuestro propio presupuesto declarado de 5-10%, así que se publica
como un bug a perseguir en vez de omitirse de la página.

Medido en un Apple M2 Pro con `benchmarks/overhead.py`. El arnés corre cada condición en un
proceso separado, alterna su orden, y **se niega a imprimir un número cuando las rondas
no coinciden en su signo**. Corre esto en tu propia máquina en un minuto:

```bash
pip install clawmetry && python -m benchmarks.overhead
```

Cada ruta está medida, incluyendo las compuertas de hook y el proxy de aplicación de políticas,
y el arnés corre en Linux, macOS y Windows en CI. Dos resultados que vale la pena conocer: el
proxy cuesta cerca de siete veces más en Windows que en Linux, y el daemon actualmente sostiene
cerca del 12% de un núcleo, por encima de nuestro propio presupuesto de 5-10%. El JSON crudo,
el método, y lo que aún no está medido están en [docs/OVERHEAD.md](docs/OVERHEAD.md).

## Precios

| Plan | Qué cubre | Precio |
|---|---|---|
| **Free** | OpenClaw + NVIDIA NemoClaw + Goose, dashboard completo, solo local | $0 |
...
**[clawmetry.com/pricing](https://clawmetry.com/pricing)**. Las claves de licencia autohospedadas
funcionan sin la nube (`clawmetry license`). La división exacta entre gratis y pago está en
...
a menos que ejecutes `clawmetry connect`**; nada de prompts, respuestas, argumentos de
herramientas, contenido de archivos ni líneas de log. Cuando te conectas, la instantánea
va cifrada de extremo a extremo con una clave que nunca sale de tu máquina, y se descifra
en tu navegador. Si un nodo no tiene clave, la subida se omite en vez de enviarse sin cifrar,
y ninguna respuesta del servidor puede desactivar eso.

Dos cosas sí corren por defecto antes de que te conectes, ambas opcionales y ninguna
lleva datos de sesión: un ping anónimo de instalación y una verificación de versión contra
PyPI. Una instalación por defecto también consulta tu IP pública una vez para la línea de
un banner de inicio. Cada destino, lo que transporta y cómo desactivarlo está listado en
[docs/EGRESS.md](docs/EGRESS.md); las instalaciones autohospedadas, redirigidas y
aisladas de la red no hacen ninguna llamada saliente discrecional.

El descifrado ocurre en tu navegador, con código que te servimos nosotros. Eso solía ser
una promesa; ahora es algo que puedes verificar. Cada línea que toca tu clave vive en un
único archivo legible, [`clawmetry/static/js/cm-e2e.js`](clawmetry/static/js/cm-e2e.js),
que se distribuye dentro del wheel y se sirve tal cual, fijado con un hash de Integridad
...
comprometido, no del proveedor. Lo que ganas es que cualquier sustitución tendría que
ser deliberada, visible en el código fuente de la página, y distinta de un artefacto en
PyPI que cualquiera puede descargar. Autohospedar o quedarte solo local elimina la
...
enseña a Claude Code, Codex, Cursor, Gemini CLI, Copilot u OpenCode a instalar ClawMetry,
reportar qué están haciendo y gastando los agentes en la máquina, detener una sesión a
pedido, y retener llamadas a herramientas riesgosas para aprobación:
...
| [Arquitectura](ARCHITECTURE.md) · [Desarrollo](docs/DEVELOPMENT.md) | Cómo funciona por dentro; cómo correrlo desde el código fuente |
| [Telemetría](docs/TELEMETRY.md) | Los pings anónimos de instalación y apertura del escritorio, y cómo desactivarlos |

## Capturas de pantalla

Cada número de abajo proviene de una máquina real, solo lectura, sin nada preparado.
...
pico de costo de 4.2x. Debajo, 324 de 667 sesiones recientes con una señal de
desperdicio, desglosadas por causa.
...
desglosados como recuperables y $17,256/mes ya ahorrados por reutilización de caché.
...
respondiendo en este momento, y cada herramienta que usó. Los nodos se iluminan
...
vio por última vez, quién es su dueño, y si una suscripción está cubriendo la cuenta.
14 agentes aquí, 3 sesiones trabajando, 13 en reposo.
...
a Bash y llamada al modelo tiene su propia barra en la línea de tiempo, así que la
que corrió durante 4.1 minutos y la que corrió durante 226ms se distinguen de un vistazo.

![Sessions: one agent turn on a timeline, every tool call with its own duration and the turn's cost](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/sessions.png)

**Califica el trabajo, no solo el gasto.**
Una A esta semana: 54 tareas salieron limpias, 2 problemáticas costaron $48.57, y las
corridas con muy poca actividad para juzgar quedan fuera de la calificación en vez de
contarse como victorias. Cada corrida problemática enlaza a su traza.

![Quality: this week's report card with the rough runs and what they cost](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/quality.png)

**Muestra por qué la ventana de contexto se sigue llenando.**
715K de una ventana de 1M de tokens en el último turno, un pico de 83.3%, 4 compactaciones
que se activaron todas de forma proactiva en vez de por desbordamiento, y el uso de
cada turno detrás de eso.

![Context usage: window utilisation per turn, compaction events and tokens reclaimed](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/context.png)

**La detección funciona sin que configures nada.**
Los detectores integrados están activos desde la instalación: el agente se quedó en
silencio, el feed de telemetría se detuvo, pico de costo, ráfaga de tokens, errores en
aumento, pico de errores, umbral de presupuesto, firma de amenaza detectada, hallazgo de
...
**Retener una llamada riesgosa es opcional, y se distribuye desactivado.**
Borrados recursivos, force pushes, sudo, secretos, instalaciones de paquetes y llamadas
salientes tienen cada una una regla que puedes activar. Hasta que lo hagas, ClawMetry
observa y no cambia nada. Una vez que activas una, las llamadas coincidentes esperan
aquí (o en tu teléfono) para ser aprobadas o rechazadas.
...
MIT · Construido por [@vivekchand](https://github.com/vivekchand) · [clawmetry.com](https://clawmetry.com)
