...
lee los archivos de sesión que tus agentes de código ya generan, y reúne la línea de tiempo,
las llamadas a herramientas y los datos de tokens y costo que el runtime exponga en una sola
vista, para que puedas distinguir una ejecución larga que está funcionando de una que está estancada.
...
| **Qué es gratis** | `pip install clawmetry` lee **OpenClaw, NVIDIA NemoClaw y Goose** sin cuenta, sin clave y sin ninguna llamada de red. Los otros 27 (Claude Code, Codex, Cursor y el resto) son leídos por el complemento de código cerrado `clawmetry-pro`, que llega con la prueba de 7 días o con un plan; consulta [docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md) para conocer el reparto exacto. |
| **Cómo empezar** | `pip install clawmetry && clawmetry`, y luego abre localhost:8900. ¿Todavía no tienes agentes en esta máquina? `clawmetry --sample` se abre con tres sesiones sintéticas etiquetadas. |
| **Qué sale de tu máquina** | Ningún dato de sesión, a menos que ejecutes `clawmetry connect`. Dos cosas sí se ejecutan por defecto, ambas desactivables y ninguna transporta contenido de sesión: un ping anónimo de instalación y una verificación de versión en PyPI. Cada destino está inventariado en [docs/EGRESS.md](docs/EGRESS.md), reconstruido a partir de una captura de tráfico y no de la lectura de comentarios. |

Vale la pena conocer dos límites antes de juzgar los resultados: los runtimes exponen datos
muy distintos (algunos no publican ningún costo; [la matriz](docs/compatibility.md)
...
Todos los runtimes obtienen el mismo panel. Ejecuta varios a la vez y el selector
del encabezado reencauza cada pestaña hacia uno de ellos.

¿Construiste tu propio agente sobre un SDK en lugar de usar un runtime? El interceptor
también rastrea sus llamadas al LLM. Consulta [docs/SDK_TRACKING.md](docs/SDK_TRACKING.md).
...
- **Desbordamiento de contexto**: uso de la ventana dimensionado por proveedor, compactación frente a desbordamiento forzado, además de un mapa por runtime de lo que *no* podemos ver ([cómo](docs/CONTEXT_BLOWOUT.md))
- **Memoria y skills**: los archivos y skills que cada runtime realmente cargó
- **Salud y logs**: disco, memoria, tasas de error, límites de frecuencia, stream de logs en vivo
...
## Desbordamiento de contexto, y cuánto cuesta observar
...
Un porcentaje de uso solo es tan honesto como el valor por el que divide. ClawMetry
...
medido contra los 200K de Anthropic se lee como ">100%, desbordado" cuando en realidad
está al 75% de los 400K de GPT-5. La misma regla oculta un turno de DeepSeek
genuinamente desbordado de 130K como un cómodo 65%.
...
| Gate de hook pre-herramienta (caché caliente) | **+44 ms** por llamada a herramienta con gate, sobre un piso de intérprete de 36 ms | desactivado |
...
(67.7 MB por cada 100k eventos), y **~12% de un núcleo** en régimen sostenido en una
instalación con mucha actividad. Ese último número supera nuestro propio presupuesto declarado
de 5-10%, así que se publica como un bug a resolver en lugar de omitirse de la página.
...
Cada ruta se mide, incluyendo los gates de hook y el proxy de aplicación de políticas,
y el arnés de pruebas corre en Linux, macOS y Windows en CI. Dos resultados que vale la pena
...
presupuesto de 5-10%. El JSON crudo, el método, y lo que todavía no está medido están en
...
funcionan sin la nube (`clawmetry license`). El reparto exacto entre gratis y pago está
...
a menos que ejecutes `clawmetry connect`**: nada de prompts, respuestas, argumentos de herramientas, contenido
...
Dos cosas sí se ejecutan por defecto antes de conectarte, ambas desactivables y ninguna
...
no hacen ninguna llamada saliente discrecional.
...
Para confirmar que el navegador ejecuta lo que publicamos:
...
instalar ClawMetry, reportar qué están haciendo y gastando los agentes de la máquina,
...
| [OpenTelemetry](docs/OPENTELEMETRY.md) | Exporta trazas a donde sea, ingiere OTLP desde cualquier cosa |
...
Dos banners de anomalía en la parte superior: gasto corriendo 7 veces por encima del promedio diario, y un
...
que está respondiendo ahora mismo, y cada herramienta a la que recurrió. Los nodos se iluminan
...
se lo vio por última vez, quién es su dueño, y si una suscripción está cubriendo la
...
a Bash y cada llamada al modelo obtiene su propia barra en la línea de tiempo, así que el comando que corrió
...
Una A esta semana: 54 tareas salieron limpias, 2 imperfectas costaron $48.57, y las
ejecuciones con muy poca actividad para evaluar quedan fuera de la calificación en lugar de
contarse como aciertos. Cada ejecución imperfecta enlaza a su traza.
...
que se dispararon todas de forma proactiva en lugar de por un desbordamiento, y el uso de
cada turno detrás de eso.

![Context usage: window utilisation per turn, compaction events and tokens reclaimed](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/context.png)

**La detección funciona sin que configures nada.**
Los detectores integrados están activos desde la instalación: el agente quedó en silencio, se detuvo
el feed de telemetría, pico de costo, ráfaga de tokens, errores en aumento, pico de errores, umbral
de presupuesto alcanzado, firma de amenaza detectada, hallazgo de herramienta de seguridad, cambio en la postura
de seguridad. Tus propias reglas son opcionales, además de estas.

![Alerts: built-in detectors plus optional custom rules](https://raw.githubusercontent.com/vivekchand/clawmetry/main/screenshots/alerts.png)

**Retener una llamada riesgosa es opcional, y viene desactivado.**
Los borrados recursivos, force pushes, sudo, secretos, instalaciones de paquetes y llamadas salientes
tienen cada uno una regla que puedes activar. Hasta que lo hagas, ClawMetry observa y
no cambia nada. Una vez activada una regla, las llamadas coincidentes esperan aquí (o en tu teléfono)
a que las apruebes o las rechaces.
