...
OpenClaw y NemoClaw son gratuitos en la app de código abierto; los demás runtimes se activan con ClawMetry Cloud o una licencia Pro autoalojada. Cambia de runtime desde el encabezado y cada pestaña (costo, tokens, herramientas, trazas) se reajusta a ese runtime. Consulta **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para ver la división exacta de gratis/pago, la matriz de niveles, la forma de `/api/entitlement` y el CLI `clawmetry license`.
...
### 🧠 Brain — Stream en vivo de eventos del agente
...
```bash
# Terminal 1: Flask API/servidor en :8900
...
Abre `http://localhost:5173/v2/`. Vite redirige las peticiones `/api` a
`http://localhost:8900`, para que la app React pueda hablar con el servidor
Flask local sin configuración adicional de CORS.
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no es OpenClaw incluye un adaptador de lectura dedicado que traduce su formato nativo de sesión a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + snapshot en la nube, etiquetados con el runtime, y la pestaña de repetición de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para la matriz completa + una guía para agregar runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
**Exporta** cada sesión (llamadas al LLM, herramientas, subagentes, tokens, costo) como spans OTLP/HTTP de GenAI a cualquier colector (Datadog, Grafana, Honeycomb, o tu propio OTel Collector):
...
Obtienes el dashboard de ClawMetry de cero configuración y local por defecto **y** tus datos en cualquier backend que tu equipo ya use, sin dependencia exclusiva ni un segundo agente que instalar.
...
nombre de usuario, ruta del espacio de trabajo, contenido de archivos, tu
api_key, tu correo, nada de PII o específico del espacio de trabajo. El
payload de la conexión es auditable en
[`clawmetry/telemetry.py`](clawmetry/telemetry.py).

**Desactivarla** (cualquiera de estas la desactiva permanentemente):
...
ping es de disparar y olvidar en un hilo daemon con un tiempo de espera de 3 s.
