...
OpenClaw y NemoClaw son gratuitos en la aplicación de código abierto; los demás runtimes se habilitan con ClawMetry Cloud o una licencia Pro autoalojada. Cambia de runtime desde el encabezado y cada pestaña, costo, tokens, herramientas, trazas, se reajusta a ese runtime. Consulta **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para ver la división exacta entre gratis y de pago, la matriz de niveles, la forma de `/api/entitlement` y la CLI `clawmetry license`.
...
- **Logs** — Transmisión de logs en tiempo real con colores
...
- **Approvals** — Bloquea eliminaciones destructivas, force pushes, mutaciones de base de datos, sudo, instalaciones de paquetes, llamadas de red detrás de una aprobación de un solo clic
...
Un "deny" bloquea solo esa llamada a herramienta; el agente conserva su sesión y puede
intentar otro enfoque. Aprobar desde tu teléfono omite el propio
aviso de permiso de Claude Code (ya respondiste). Las herramientas sin coincidencia cuestan ~40ms y
caen al flujo de permisos normal de Claude Code. También recibes una notificación push en tu teléfono cuando
el propio Claude Code está esperando tu respuesta (notificaciones `permission_prompt` /
`idle_prompt`).
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no es OpenClaw incluye un adaptador lector dedicado que traduce el formato nativo de sus sesiones a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB + snapshot en la nube, etiquetados con el runtime, y la pestaña de repetición de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para ver la matriz completa + una guía para agregar runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.

¿Ejecutas la herramienta de seguridad de agentes [numbat de Perplexity](https://github.com/perplexityai/numbat)? ClawMetry ingiere sus hallazgos y decisiones de aplicación de forma nativa; consulta [`docs/NUMBAT.md`](docs/NUMBAT.md).
...
| **n8n** | Adaptador beta | SQLite `~/.n8n/database.sqlite`. Ejecuciones de workflows, ejecuciones de nodos, prompts de AI Agent, modelo + tokens donde n8n los registra. |
| **Antigravity** | Adaptador beta | Brain JSONL en `~/.gemini/<flavor>/brain/`. Conversaciones, pasos de herramientas, razonamiento, desglose de tokens Gemini por generación + costo, consumo de generación en segundo plano. |
| **GitHub Copilot** | Adaptador beta | `events.jsonl` de Copilot CLI en `~/.copilot/session-state/` + el libro mayor de uso por llamada `session-store.db`. Conversaciones, llamadas a herramientas, enrutamiento de modelos, desglose de tokens con caché, costo en créditos de IA facturados por el proveedor. |
| **Grok** | Adaptador beta | xAI Grok Build CLI (binario en Rust bajo `~/.grok/bin/grok`): registro global de eventos `~/.grok/logs/unified.jsonl` + por sesión `~/.grok/sessions/<enc-cwd>/<uuid>/{events.jsonl,summary.json}`. Conversaciones, desglose de tokens por turno, enrutamiento de modelos, y el payload de repositorio saliente de la CLI almacenado en `~/.grok/upload_queue/` para que veas qué salió de tu máquina. |
...
`set_source()` (o la variable de entorno `CLAWMETRY_SOURCE=support-agent`) etiqueta cada llamada con una **fuente con nombre**, así cada producto que ejecutas aparece como su propia línea de primera clase y atribuible en costos en la tarjeta **🔌 Fuentes fuera del loop** de Overview en el dashboard: llamadas, proveedores, latencia, tasa de error por agente. ¿No se configuró ninguna fuente? Las llamadas se siguen rastreando; la tarjeta simplemente permanece oculta.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Esta es la misma capa de datos que alimentan los adaptadores de runtime (DuckDB → snapshot en la nube), así que las fuentes fuera del loop se sincronizan con el dashboard en la nube igual que todo lo demás, con cifrado extremo a extremo.

## OpenTelemetry — neutral al proveedor, envía tus trazas a donde quieras
...
**Exporta** cada sesión (llamadas a LLM, herramientas, subagentes, tokens, costo) como spans GenAI OTLP/HTTP a cualquier colector (Datadog, Grafana, Honeycomb, o tu propio OTel Collector):
...
Los headers de autenticación y el intervalo de sondeo son variables de entorno opcionales:
...
**Ingesta** — el receptor OTLP incorporado acepta trazas y métricas desde cualquier otra fuente en `/v1/traces` y `/v1/metrics` (`pip install clawmetry[otel]` para ingesta protobuf).

Obtienes el dashboard de ClawMetry sin configuración y local primero, **y** tus datos en el backend que ya usa tu equipo, sin ataduras, sin necesidad de instalar un segundo agente.
...
ClawMetry muestra actividad en vivo para cada canal de OpenClaw que tengas configurado. Solo los canales que realmente están configurados en tu `openclaw.json` aparecen en el diagrama de Flow; los no configurados se ocultan automáticamente.
...
2. **Detección de contenedor** — escanea los contenedores Docker en ejecución en busca de imágenes `openshell`, `nemoclaw`, o `ghcr.io/nvidia/`, luego lee las sesiones vía volúmenes montados o `docker cp`

Los archivos de sesión sincronizados desde contenedores NemoClaw se etiquetan con `runtime=nemoclaw` y metadatos `container_id` en el dashboard en la nube, así puedes distinguirlos de las sesiones estándar de OpenClaw de un vistazo.
...
`https://app.clawmetry.com/api/install`: un ping `install` la primera vez
que ejecutas la CLI de `clawmetry` en una máquina nueva, un ping `update`
en la primera ejecución tras actualizar a una nueva versión, y un ping
`onboarded` cuando completas la opción de incorporación dentro del dashboard.
Usamos esto para contar instalaciones reales (las cifras brutas de descargas
de PyPI son ~98% mirrors, CI y redescargas de actualización automática) y
para saber qué frameworks de agentes y versiones están realmente en uso.

**Como máximo un POST por evento de ciclo de vida por versión**, que contiene:

| Campo | Ejemplo | Por qué |
|---|---|---|
| `install_id` | UUID aleatorio almacenado en `~/.clawmetry/install_id` | deduplicación; anónimo hasta que conectas explícitamente la sincronización en la nube (el heartbeat autenticado del daemon lleva entonces este dato, vinculando esta instalación a tu cuenta) |
...
**Qué NO enviamos**: IP (la nube deriva el código de país del lado del
servidor a partir de la solicitud, y luego descarta la IP), nombre de host,
nombre de usuario, ruta del workspace, contenido de archivos, tu api_key, tu
email, nada de PII ni específico del workspace. El payload es auditable en
[`clawmetry/telemetry.py`](clawmetry/telemetry.py).

**Optar por no participar** (cualquiera de estos lo desactiva permanentemente):
...
Un fallo de red aquí nunca bloquea la ejecución de `clawmetry`; el ping
se envía sin esperar respuesta (fire-and-forget) en un hilo daemon con un
tiempo de espera de 3 s.
