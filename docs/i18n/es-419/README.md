...
OpenClaw y NemoClaw son gratuitos en la app de código abierto; los demás runtimes se activan con ClawMetry Cloud o una licencia Pro autoalojada. Cambia de runtime desde el encabezado y cada pestaña, costo, tokens, herramientas, trazas, se reajusta a ese runtime. Consulta **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** para conocer el reparto exacto entre gratuito y de pago, la matriz de niveles, la forma de `/api/entitlement` y la CLI `clawmetry license`.
...
- **Alerts**: límites de presupuesto, disparadores por tasa de error, detección de agente fuera de línea; se enrutan a Slack, Discord, PagerDuty, Telegram, Email
- **Approvals**: bloquea eliminaciones destructivas, force pushes, mutaciones de base de datos, sudo, instalaciones de paquetes y llamadas de red detrás de una aprobación de un clic
...
tu decisión (un toque desde tu teléfono con las
...
caen al flujo de permisos normal de Claude Code. También recibes un push en el teléfono cuando Claude Code
...
`http://localhost:8900`, para que la app React pueda comunicarse con el servidor Flask local
sin configuración extra de CORS.
...
ClawMetry observa muchos runtimes de agentes de IA, no solo OpenClaw. Cada runtime que no sea OpenClaw incluye un adaptador de lectura dedicado que traduce el formato nativo de sus sesiones a las formas unificadas de ClawMetry; el daemon los ingiere en el mismo almacén DuckDB y snapshot en la nube, etiquetados con el runtime, y la pestaña de repetición de sesión muestra un **selector de runtime** cuando hay más de uno presente. Consulta [`docs/compatibility.md`](docs/compatibility.md) para la matriz completa y una guía para agregar runtimes, y [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) para la introducción a la familia OpenClaw.
...
"Adaptador beta" significa que ClawMetry incluye un lector para el formato real en disco de ese runtime, cada uno construido y verificado contra una instalación real en una máquina real (ver `tests/fixtures/runtimes/<rt>/`). Los adaptadores son de solo lectura; cada uno es honesto sobre lo que su runtime realmente almacena (por ejemplo, PicoClaw/NanoClaw/Cursor no escriben el costo de tokens en disco). Cuando corren varios runtimes en un solo nodo, el selector de runtime limita la vista de sesiones a uno para un análisis limpio y enfocado.

## Rastrea cualquier agente de SDK, atribución de costos fuera del loop

Los runtimes anteriores escriben las sesiones en disco. Tu propio **agente de producción**, el que construiste con el OpenAI Agents SDK, LangChain, el Vercel AI SDK, LlamaIndex, E2B o un simple loop con `httpx`, no lo hace. El interceptor de configuración cero de ClawMetry igual captura sus llamadas a LLM (costo, tokens, latencia, errores) mediante monkey-patching de `httpx`/`requests`:
...
`set_source()` (o la variable de entorno `CLAWMETRY_SOURCE=support-agent`) etiqueta cada llamada con una **fuente nombrada**, para que cada producto que ejecutes aparezca como su propia línea de primera clase, atribuible en costo, en la tarjeta **🔌 Fuentes fuera del loop** de Overview en el panel: llamadas, proveedores, latencia, tasa de error por agente. ¿No configuraste una fuente? Las llamadas igual se rastrean, solo que la tarjeta permanece oculta.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Esta es la misma capa de datos que alimentan los adaptadores de runtime (DuckDB → snapshot en la nube), así que las fuentes fuera del loop se sincronizan con el panel en la nube igual que todo lo demás, cifrado de extremo a extremo.

## OpenTelemetry, neutral respecto al proveedor, envía tus trazas a donde quieras

ClawMetry habla **OpenTelemetry** en ambas direcciones, usando las **convenciones semánticas de GenAI**, para que las trazas de tu agente nunca queden encerradas en una sola herramienta.
...
Obtienes el panel de ClawMetry con configuración cero y local por defecto, **y** tus datos en el backend que ya use tu equipo, sin ataduras, sin necesidad de instalar un segundo agente.
...
ClawMetry muestra actividad en vivo para cada canal de OpenClaw que tengas configurado. Solo aparecen en el diagrama Flow los canales que están realmente configurados en tu `openclaw.json`; los que no están configurados se ocultan automáticamente.
...
1. **Detección por binario**: verifica la CLI de `nemoclaw` y ejecuta `nemoclaw status` para obtener información del sandbox
2. **Detección por contenedor**: escanea los contenedores de Docker en ejecución en busca de imágenes `openshell`, `nemoclaw` o `ghcr.io/nvidia/`, y luego lee las sesiones mediante montajes de volumen o `docker cp`

Los archivos de sesión sincronizados desde contenedores de NemoClaw se etiquetan con `runtime=nemoclaw` y metadatos de `container_id` en el panel en la nube, para que puedas distinguirlos de las sesiones estándar de OpenClaw de un vistazo.
...
|---|---|---|
| `install_id` | UUID aleatorio guardado en `~/.clawmetry/install_id` | deduplicación; no vinculado a tu correo o api_key |
...
contenido de archivos, tu api_key, tu correo, nada que sea PII o
específico del workspace. El payload de la red es auditable en
...
Una falla de red aquí nunca bloquea la ejecución de `clawmetry`; el
ping es de disparar y olvidar en un hilo daemon con un timeout de 3 s.
