...
OpenClaw en NemoClaw zijn gratis in de opensource-app; de andere runtimes worden geactiveerd met ClawMetry Cloud of een self-hosted Pro-licentie. Wissel van runtime via de header en elk tabblad, kosten, tokens, tools, traces, past zich automatisch aan die runtime aan. Zie **[docs/ENTITLEMENTS.md](docs/ENTITLEMENTS.md)** voor de exacte gratis/betaalde verdeling, de tiermatrix, de vorm van `/api/entitlement` en de `clawmetry license`-CLI.

## Wat je krijgt

- **Flow** — Live geanimeerd diagram dat berichten laat zien die door kanalen, brain, tools en terug stromen
- **Overview** — Health checks, activiteitsheatmap, sessieaantallen, modelinformatie
...
- **Approvals** — Blokkeer destructieve verwijderingen, force pushes, DB-mutaties, sudo, pakketinstallaties, netwerkoproepen achter goedkeuring met één klik
...
### 💰 Tokens — Kostenoverzicht per model & sessie
...
### ✋ Approvals — Blokkeer risicovolle tool-calls achter handmatige goedkeuring; beleidsgebonden beschermingsregels
...
[cloud push-notificaties](https://app.clawmetry.com/push) ingeschakeld):
...
vallen terug op de normale toestemmingsflow van Claude Code. Je krijgt ook een push naar je telefoon wanneer Claude Code zelf
op je wacht (`permission_prompt`- / `idle_prompt`-notificaties).
...
Om de bundel te bouwen die met het Python-package wordt meegeleverd:
...
ClawMetry observeert veel AI-agentruntimes, niet alleen OpenClaw. Elke niet-OpenClaw-runtime levert een eigen reader-adapter die het native sessieformaat vertaalt naar ClawMetry's uniforme vormen; de daemon neemt ze op in dezelfde DuckDB-store + cloud-snapshot, getagd met de runtime, en het tabblad Session replay toont een **runtime-switcher** wanneer er meer dan één aanwezig is. Zie [`docs/compatibility.md`](docs/compatibility.md) voor de volledige matrix + een handleiding voor het toevoegen van runtimes, en [`docs/RUNTIME_FAMILY.md`](docs/RUNTIME_FAMILY.md) voor de OpenClaw-familie-primer.

Draai je [Perplexity's numbat](https://github.com/perplexityai/numbat) agent-security-tool? ClawMetry neemt de bevindingen en handhavingsbeslissingen daarvan direct op, zie [`docs/NUMBAT.md`](docs/NUMBAT.md).

| Runtime / Agent | Status | Notities |
|---|---|---|
| **OpenClaw** | Native | Referentierun­time, automatisch gedetecteerd |
...
| **n8n** | Bèta-adapter | SQLite `~/.n8n/database.sqlite`. Workflow-executies, node-runs, AI Agent-prompts, model + tokens waar n8n die registreert. |
| **Antigravity** | Bèta-adapter | Brain-JSONL onder `~/.gemini/<flavor>/brain/`. Conversaties, tool-stappen, thinking, per-generatie Gemini-tokensplitsing + kosten, achtergrondgeneratie-verbruik. |
...
De bovenstaande runtimes schrijven allemaal sessies naar schijf. Jouw eigen **productieagent**, degene die je bouwde op de OpenAI Agents SDK, LangChain, de Vercel AI SDK, LlamaIndex, E2B, of een gewone `httpx`-lus, doet dat niet. ClawMetry's zero-config interceptor legt zijn LLM-calls (kosten, tokens, latentie, fouten) toch vast door `httpx`/`requests` te monkey-patchen:
...
`set_source()` (of de omgevingsvariabele `CLAWMETRY_SOURCE=support-agent`) tagt elke call met een **benoemde bron**, zodat elk product dat je draait als eigen, kostentoewijsbare regel verschijnt in de kaart **🔌 Out-loop sources** van het dashboard op Overview, calls, providers, latentie, foutpercentage per agent. Geen bron ingesteld? De calls worden nog steeds getrackt; de kaart blijft dan gewoon verborgen.

```bash
CLAWMETRY_SOURCE=billing-agent python my_agent.py
```

Dit is dezelfde datalaag die de runtime-adapters voeden (DuckDB → cloud-snapshot), dus out-loop-bronnen synchroniseren net als al het andere, end-to-end versleuteld, naar het clouddashboard.

## OpenTelemetry — leveranciersneutraal, stuur je traces overal naartoe

ClawMetry spreekt in beide richtingen **OpenTelemetry**, met de **GenAI semantic conventions**, zodat je agenttraces nooit vastzitten aan één tool.
...
De meeste mensen hebben geen configuratie nodig. ClawMetry detecteert je werkruimte, logs, sessies en crons automatisch.
...
1. **Binary-detectie** — controleert op de `nemoclaw`-CLI en voert `nemoclaw status` uit om sandbox-informatie op te halen
...
Sessiebestanden die vanuit NemoClaw-containers gesynchroniseerd worden, worden in het clouddashboard getagd met `runtime=nemoclaw` en `container_id`-metadata, zodat je ze op het eerste gezicht kunt onderscheiden van standaard OpenClaw-sessies.
...
Als je de sync-daemon **binnen** de OpenShell-sandbox moet draaien, voeg dan deze egress-regel toe aan je NemoClaw-netwerkbeleid, zodat hij de ClawMetry ingest-API kan bereiken:
...
en auto-update-herdownloads) en om te leren welke agentframeworks en
...
|---|---|---|
| `install_id` | willekeurige UUID opgeslagen op `~/.clawmetry/install_id` | dedup; anoniem totdat je expliciet Cloud sync verbindt (de geauthenticeerde daemon-heartbeat draagt dit dan mee en koppelt deze installatie aan je account) |
...
**Wat we NIET versturen**: IP (cloud leidt de landcode server-side af
uit het verzoek en verwerpt daarna het IP), hostnaam, gebruikersnaam, werkruimtepad,
bestandsinhoud, je api_key, je e-mailadres, iets persoonlijks of
werkruimtespecifieks. De wire-payload is controleerbaar in
...
Een netwerkfout hier blokkeert `clawmetry` nooit in het draaien, de
