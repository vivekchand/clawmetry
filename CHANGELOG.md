...
- **Evals is now Quality: a report card that answers "is my agent doing good work?" in three seconds. Hand-stamped letter grade on a paper card, ranked failure patterns with the $ cost, plain-English rough runs, and an inline "Prevent this →" builder that turns a real trace into a persistent check.**
  - **Why:** founder ask 2026-08-14 on `/goal` after the score-first + drill-down slice (0.12.695) shipped: "what is this really? I'm unable to even understand what is to be done here. Can you please get steve jobs & elon musk involved first to design how evals should be for clawmetry, use claude design skill to brainstorm & create mockups & then implement." The previous release was tactical: rearranged the same page (Recently Scored table, tile grid, Judge card). The founder was right that this stayed inside the old page's shape rather than rethinking what Evals should BE for the newcomer audience CLAUDE.md calls out ("people who have never used one, ordinary people running and managing hundreds of AI agents"). This release starts from a blank canvas.
