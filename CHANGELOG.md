...
### Feat: answer from anywhere — questions ride the cloud relay by default when connected (2026-07-24)
- **Why:** answer links in Slack/Telegram/WhatsApp messages pointed at `localhost:8900`, so a tap only worked when the phone could reach the machine. Pushary's core promise is "approve from your lock screen, wherever you are" — ClawMetry Cloud already has the exact plumbing (E2E-encrypted cache push up, heartbeat `pending_queries` drain down) used by tool-call approvals.
- **What:** cloud-connected nodes (`clawmetry connect`) get the full relay automatically. **Up:** the daemon pushes the pending-questions queue (+ kill-switch state) E2E-encrypted under `questions:{owner_hash}:queue` each heartbeat, plus a plaintext `pending_questions_count` push cue — and the heartbeat drops to FAST (3 s) while any question is pending, so a phone tap unblocks the agent in seconds. **Down:** answers come back as `question_answer` / `question_cancel` / `killswitch_set` heartbeat actions; every question mints a per-question `action_token` (new column) delivered only inside the encrypted blob and the node's own outbound links — the NODE validates it, so the cloud can never approve anything on its own (worst forgery = a denied/stopped agent, fail-safe). Answer links in all channel messages become `app.clawmetry.com/q/<id>?n=<node>&t=<token>&v=yes|no` tap links when connected (select/input links render a tiny answer form); local dashboard links remain the fallback. Cloud side (clawmetry-cloud, same branch): `GET /api/cloud/questions` (blob forward, decrypted in-browser), `POST /api/cloud/questions/<id>/answer|/cancel` and `POST /api/cloud/killswitch` via the DURABLE `node_pending_actions` Postgres queue (a Cloud-Run replica handoff can't drop a blocking ask_user answer), the public `/q/<id>` tap endpoint with a strict `[A-Za-z0-9_-]` charset gate, route-policy classifications for the new OSS routes, and the pending-count heartbeat fields allow-listed. The Approvals-tab inbox works in cloud mode too (decrypts the queue blob client-side, answers via the relay).
- **Verified:** OSS 43/43 (`tests/test_agent_questions.py` — action_token stamping, cloud-vs-local links, relay appliers incl. token mismatch rejection + select validation, kill-switch relay, FAST-while-pending cadence, encrypted cache-push build); cloud 10/10 new (`tests/test_questions_relay.py` — cache hit/miss, ciphertext forwarded verbatim, durable enqueue shapes, tap-link confirm/form/charset-reject/unknown-node) plus route-policy suite 39/39 and the approvals baseline 8/8 with the OSS tree on PYTHONPATH.

### Feat: approve your agents from your phone — human-in-the-loop questions, permission hooks, kill switch (2026-07-24)
- **Why:** when an autonomous agent hits a decision it can't make, it freezes at a permission prompt until someone walks back to the terminal — hours of idle compute for a 2-second "yes". Competing standalone tools (Pushary et al.) ship this as a paid SaaS; ClawMetry already owns the approvals data model, policy engine, and channel fan-out, so the ask/notify/gate loop belongs here.
- **What:** a full ask/notify/gate engine, local-first and E2E-consistent with the existing approvals architecture. **Ask:** new `agent_questions` DuckDB table + `clawmetry/questions.py` engine — confirm (yes/no), select (2-6 options), and input (free text) questions with 10-minute default expiry, first-answer-wins idempotency, and per-decision latency stamped into the audit trail; surfaced as `bp_questions` (`/api/questions*`, long-poll `?wait_ms=`, CSV audit export) and a new Agent Questions inbox + phone/push channel config on the Approvals tab. **Notify:** channel fan-out to ntfy (tap-to-approve action buttons on the lock screen), Pushover, Slack incoming webhooks (Approve/Deny buttons), Telegram (bot sendMessage with approve/deny links), Discord, a generic n8n-compatible webhook, and — opt-in — the operator's connected OpenClaw chat channel (WhatsApp / Signal / Telegram / iMessage / …) via the gateway `message.send` RPC, the same path budget alerts already use; secrets redacted before anything leaves the machine; only question metadata travels, never transcripts or code. Configure-once: when the questions config leaves a channel blank, credentials are borrowed live from the existing alerts config (Slack/Discord webhooks) and budget config (Telegram bot), with the borrowed source surfaced in the UI and `clawmetry hooks doctor`. **MCP:** the stdio server grows `send_notification`, `ask_user` (blocking, ≤55 s per poll), `wait_for_answer`, and `cancel_question`, so any MCP-compatible agent can reach the operator. **Gate:** `clawmetry hooks setup` wires Claude Code PreToolUse/Notification/Stop hooks; risky commands (rm -rf, force pushes, history rewrites, DB drops, deploys, sysadmin, firewall, spend) escalate to the phone per delivery mode (`push_only` / `push_first` / `terminal_only` / `notify_only`, with `clawmetry hooks mode … --for 30m` temporary overrides and a `wait` ladder), honoring existing `policies.yml`/cloud policies (auto-approve / deny / monitor / require_approval) with most-specific-wins matching; unanswered escalations fail closed (deny) or fall back to the agent's own terminal prompt — never a silent allow. **Kill switch:** global or session-scoped, one tap on the Approvals tab or `clawmetry hooks killswitch on`, denies every gated call until released. `clawmetry hooks doctor/clean/stats` verify, remove, and report.
- **Verified:** 33 new tests in `tests/test_agent_questions.py` (lifecycle, select validation, expiry, kill switch, mode override, redaction, risky-command classification, PreToolUse stdin/stdout protocol incl. garbage-input fail-safe, settings.json install/remove idempotency + foreign-hook preservation, full HTTP round-trips incl. CSV export and credential masking) — all green; daemon-allowlist lint green on the four new proxy methods; py3.9 AST clean; dashboard boots with `bp_questions` registered and all new endpoints answering 200.
