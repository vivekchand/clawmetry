...
- **Release: the desktop app now reports that it was opened, so a download can be told apart from a launch, and a launch that works from one that never gets off the ground. Carries #5020 to PyPI and the fleet.**
  - **Who this reaches:** anyone who installs the ClawMetry desktop app on macOS, Windows or Linux. Nothing changes for a `pip install clawmetry` user: the open report fires only when the desktop shell is the thing that started the daemon.
  - **What changes at runtime:** each launch sends up to two short reports to `app.clawmetry.com/api/desktop/open`. The first goes out the moment the window appears, before the app has provisioned its Python runtime, and carries the launch counter, the bundle version and the platform. The second goes out once the dashboard is actually serving, and carries what only a running daemon knows: which agent runtimes have data on this machine (IDs only), whether the install syncs to cloud or stays local, and its node. Both halves of one launch share a session ID, so an install that opened and never reached a working dashboard is a subtraction rather than a guess. Paired installs attach their cloud key as a header, and only so the machine can be listed under "Desktop apps" on its own owner's account page.
  - **Opting out is unchanged and absolute:** `CLAWMETRY_NO_TELEMETRY=1`, `DO_NOT_TRACK=1`, or `~/.clawmetry/notelemetry` silences both reports, and an enterprise endpoint disables them outright, because a self-hosted deployment's data stays inside the deployment. The launch counter lives on the machine, so opting out leaves it correct and offline launches still count when reporting resumes.
  - **Verified before release:** both stages were driven end to end against a real sink from a seeded home directory. Three consecutive launches reported 1, 2, 3 with the first-launch flag flipping exactly once; the daemon stage arrived under the same install and session ID carrying live runtime detection; and each of the three silences (local-only mode, `DO_NOT_TRACK`, an enterprise endpoint) was confirmed by a sink that received nothing. The receiving side was smoked on the deployed cloud revision before traffic moved to it.

### Feature: the desktop app reports its opens, including the ones that never reach a working dashboard (#5020) (2026-08-20)
- **Why:** the desktop channel produced a download notification and then silence. Three questions about it were all unanswerable: does a download become an open, does an open become a working dashboard, and does anyone come back a second time. The middle one matters most and was the one we were structurally blind to. The bundle is a thin shell that provisions a Python runtime and installs clawmetry from PyPI on first launch, so every bootstrap failure we know about (no system interpreter, the Microsoft Store alias stub, an unreachable PyPI, a half written venv) happens before any daemon exists to report anything. An install wedged there shows its owner a splash screen forever and tells us nothing at all, and it is the case least likely to arrive as a support report, because the app never worked well enough to look like a product with a support channel.
- **What:** the launch is reported twice. The shell reports when its window appears, before bootstrapping, carrying the on-disk launch counter (so launches made offline are not lost), the bundle version, the platform, and whether this window attached to a daemon another instance already owns, which never spawns a daemon and must not be scored as a failed startup. The daemon reports once the dashboard is serving, carrying the runtimes present on the machine, cloud versus local, and the node. A per-launch session ID ties them together. The install identity is the one the existing install ping already owns, so a machine is one install rather than two. The bundle now knows its own version: the release workflow stamps `desktop/assets/version.txt` before PyInstaller runs and each spec picks it up when present, so a developer build honestly reports `dev` instead of guessing.
  On the receiving side the cloud keeps one durable record per install and one append-only record per launch stage, unique on install plus session plus stage. That uniqueness is the idempotency gate: the counters are advanced only when a stage record is new, so a client retry cannot inflate anything, and the client's counter wins when it is ahead of ours. A shell report never nulls the runtime list, mode or node that only a daemon report can establish. The sink is public and always answers `200 {"ok": true}`, for a duplicate and a malformed body alike, because an install that never finished bootstrapping has no key to present and telemetry must never make a client retry. A cloud key is optional and attribution only: it is hashed on arrival, never stored, an unknown key downgrades the report to anonymous instead of rejecting it, and a later anonymous report never clears an install's existing attribution. Request addresses are not stored, only the country derived from them, and the reported runtime list is allowlisted to ID slugs on the way in.
- **Verified:** 21 tests on the client side over both stages (counter persistence across processes, first versus nth, opt-out silencing the network while the local counter keeps working, enterprise endpoint silence, attached windows, key in the header and never the body, network failure invisibility, runtime detection failure tolerance) and 22 on the sink (retry idempotency, junk and oversized install identities, unknown stages, allowlisting, the offline counter merge, shell versus daemon column ownership, attribution downgrade paths, the first-open email firing exactly once, both gated endpoints failing closed). Beyond the tests, both stages were run for real against a live sink and the deployed cloud revision was smoked before traffic moved to it. Blueprint *Desktop Application Distribution* reconciled to v18 across four drift-bot rounds; the round that mattered was placement, not wording, because an appended section past roughly 50k characters is not read.
- **Reach:** the daemon half ships in the wheel and lands on existing desktop installs at their next auto-update. The shell half ships in the bundle, so it reaches users when they download a new installer, which is the standing asymmetry of the thin-shell design.

- **Release: the first-install sync banner names the agents it is actually syncing, instead of telling everyone it is syncing OpenClaw. Carries #5010 to PyPI and the fleet.**
  - **Who this reaches:** every new install, and most sharply the ones that have never had OpenClaw. The banner is one of the first things a first-time user sees, and until this release it announced a product they may not run.
  - **What changes at runtime:** the banner title is built from the runtimes detected on the machine ("Syncing your Claude Code data", "Syncing Claude Code and Cursor", "Syncing Claude Code, Cursor and 2 more"), and falls back to "Syncing your AI agents" when detection has not landed or finds nothing. A runtime is named only when it has sessions on disk, so an installed-but-unused Cursor is not claimed as something being synced. Detection runs on a background refresh behind a 60s cache, never inline in the polled handler.
  - **Verified before release:** 8 guard tests, each confirmed red against `origin/main` and green here; the endpoint returning the real runtime set on a live machine; and a browser screenshot of the rendered banner.

### Fix: the sync banner claimed to be syncing an OpenClaw workspace on machines that have never had one (#5010) (2026-08-19)
- **Why:** founder report, looking at a fresh dashboard: "why does it say syncing your OpenClaw workspace, even if the user does not have OpenClaw installed? is it hardcoded?" It was. `banners.html` shipped the literal string "Syncing your OpenClaw workspace" and nothing ever rewrote it, so the first-install banner asserted OpenClaw on every machine, including the majority that only run Claude Code, Codex or Cursor. During onboarding an unfamiliar product name scrolling past reads as the tool being confused about your setup, which is the opposite of what a first-install banner exists to do.
- **What:** `/api/sync-progress` grows a `runtimes: [{id, label}]` list and `_cmSyncScopeTitle` builds the title from it, with the template's static default now runtime-neutral because it renders before any detection has landed. The list is composed by the dashboard rather than the daemon, so it takes effect without a daemon release. Two constraints shape the implementation. Honesty: a runtime is named only when it has sessions ON DISK, the same rule `_detect_runtimes_for_heartbeat` already applies, because the Cursor IDE creates its state dir whether or not the agent was ever run and naming that would simply replace one lie with another. Cost: detection globs session dirs and measured ~3.3s on a busy machine while the banner polls every 15s, so it never runs inline; the handler serves a 60s cache and a daemon thread refreshes it, which means the first request returns an empty list (neutral title) and the next poll carries the names. Detection merges three sources because none is complete alone: the OSS adapter registry for OpenClaw and NemoClaw (with a direct-instantiation fallback, since registration happens at app creation and can be empty), `_detect_runtimes_lite`, and `_detect_family_runtimes`, which lives in clawmetry-pro and so returns nothing in OSS.
- **Verified:** 8 new tests, each confirmed red against `origin/main` and green here, pinning the template copy, the JS fallback staying runtime-neutral, the render wiring (a title builder nobody calls is dead code), both endpoint branches including the cold-install 404, the non-blocking contract, the zero-session phantom rule, and the thread-failure path that would otherwise wedge the cache. The endpoint returns the real set on this machine, and the rendered banner was read in a browser. `test_sync_progress_sticky.py`, `test_adapters.py` and `test_adapter_registry_override.py` stay green.
- Note for whoever gets there next: this surfaced a latent bug it does not fix. `detect_config()`'s `args=None` path builds a bare `argparse.Namespace` and then raises `AttributeError` on `args.log_dir`. The CLI always passes a full Namespace, so it never fires in production; the test fixture works around it rather than widening that PR's scope.

- **Release: a Pro account is no longer shown an upgrade modal during the first seconds after launch, and the daemon proxy stops dropping positional arguments. Carries #5003 to PyPI and the fleet.**
  - **Who this reaches:** every paying install, and most visibly the desktop app, where the dashboard starts in the same second as the daemon and so loads its runtime catalogue before the account's plan has been fetched. Until this release, picking any paid runtime in that window opened the plan modal, and the stale answer survived until the page was reloaded.
  - **What changes at runtime:** entitlement now distinguishes "the plan has not resolved yet" from "this account is on the free plan", and publishes that distinction on `/api/runtimes`, `/api/entitlement` and `/api/entitlement/runtime-detection`. Nothing locks while the plan is resolving, the dashboard re-checks until it lands, and the upgrade modal re-verifies before it renders, so no snapshot taken at page load can paywall somebody who is entitled.
  - **Also:** cross-process store calls made with positional arguments now reach the daemon with their arguments intact, instead of arriving empty and being rejected. This silently returned empty results for authority-violation counts and reported successful approval writes as failed. Approval-policy reloads log only when the policy set changed, which takes a busy `sync.log` from over a thousand identical lines to one.
  - **Verified before release:** the failure was reproduced against `origin/main` in an isolated instance staged in the boot window (an account linked, no plan cached yet) — all 20 paid runtimes locked and the modal rendered — then shown gone on the fixed build under identical conditions, with the modal still rendering correctly for a genuinely locked runtime.

### Fix: a Pro account was shown the upgrade modal, and the daemon proxy dropped every positional argument (2026-08-19)
- **Why:** founder report from the desktop app, with two unrelated causes behind one "why is it behaving weirdly". First, picking Claude Code in the runtime switcher opened the plan modal on a `cloud_pro` account. `/api/entitlement` was correct at the time of the report (`tier: cloud_pro`, `is_paid: true`, `enforced: false`, `locked_runtimes: []`), so nothing was wrong with the server *then*. The lock map is built once, at page load, and the page had loaded 38 seconds before the answer existed: the shell started at 18:08:59, the dashboard at 18:09:01, and the daemon wrote `cloud_plan.json` at 18:09:39. Entitlement resolves from that file, so every fetch in between fell through to `_oss_free()`, whose runtime set is OpenClaw and NemoClaw only. `_cmLoadRuntimeCatalog` read `entitled: false` on all 20 paid runtimes and locked them, and the re-check that would have repaired the map was gated on `window.CLOUD_MODE`, which the desktop app never sets. The stale map then survived for the life of the page. The deeper bug is that an OSS-free verdict was being read as "this user is on the free plan" when it actually meant "we do not know yet".
  Second, `sync.log` carried a steady trickle of 400s from the daemon proxy (`query_session_authority_counts`, `ingest_approval`, `_fetch`). `_ProxyStore.__getattr__` forwarded calls as `local_store_via_daemon(name, **kwargs)`, and the transport has no slot for positional arguments, so `*args` was silently discarded. The daemon then answered `400 missing required argument` and every caller swallowed that as an empty result. The signature is visible in the log: the same method returns 200 for the kwargs callsite and 400 for the positional fallback two lines later.
- **What:** `entitlements.plan_pending()` names the third state. It is true when an account is linked (an `api_key` in `config.json`) but no licence and no readable cloud plan exist yet, and false both once the plan lands and on a machine that was never linked, where OSS-free is the honest answer. `/api/runtimes`, `/api/entitlement` and `/api/entitlement/runtime-detection` publish it, including on their degraded fallback paths, because a resolver crash is also "unknown" and must not re-open the same hole. The client then gets three independent guards, because one of them being wrong is how this shipped in the first place: the catalog never locks while `pending` and re-checks until the plan resolves (bounded at 20 tries, and no longer cloud-only); the switcher re-verifies against a fresh catalog before refusing a switch, and honours the switch when the runtime turns out to be entitled; and `_cmShowRuntimePaywall` itself re-verifies before rendering, which covers all three entry points into that modal at once, including the inventory banner that reads a different endpoint.
  `_ProxyStore` now binds positional arguments against the real `LocalStore` signature, so both call styles work over the proxy. Calls it cannot express (a keyword-only signature, an unknown method) return `None` with a warning rather than a silent 400, private helpers like `_fetch` are refused locally instead of being sent to an allowlist that will never accept them, and dunder probes raise `AttributeError` instead of resolving to a proxy call. Two callers that read a successful-but-empty answer as failure are fixed with them: a new `PROXY_UNAVAILABLE` sentinel separates "the daemon could not answer" from "the answer is `{}`/`None`", which stops `_stamp_authority_counts` re-running its whole lookup against a direct store on every request and stops `_ls_write` reporting a landed `ingest_approval` as failed (every writer it calls returns `None` on success).
  Also: `load_policies` announces itself at INFO only when the policy set actually changed. It runs on every evaluation pass, and the reported log held 1,382 identical "loaded 17 approval policies" lines, which is enough to bury the errors an operator is grepping for.
- **Verified:** 17 new tests, each confirmed red against `origin/main` and green here (7 failed, 10 errored on the base). They pin the positional binding for the two real callsites, the keyword-only signature that must not be faked, `_fetch` never reaching the transport, all four `plan_pending()` states, the premise that an unresolved plan really does resolve to a tier without `claude_code`, `pending` reaching the wire on `/api/runtimes` and `/api/entitlement`, and the policy log staying quiet on an unchanged reload. The existing `test_entitlement_api_fallback_shape.py` parity guard caught the first attempt at this, which added `pending` to the healthy response and not to the frozen fallback snapshot; both carry it now. No new failures anywhere: the approvals and local-store suites are compared failure-by-failure against the base, where the branch introduces none and repairs 9 that main is already failing, several of them (`test_authority_counts_empty_input`, the approval round-trips) direct evidence of the argument-dropping bug. `make lint` holds at its pre-existing 215 findings.

### Fix: the expired-trial screen sells the plan, and you can switch accounts (#4986) (2026-08-18)
- **Why:** two dead ends on the same screen, both founder-reported. A self-hosted install whose 7-day trial ended showed one button, "See pricing", which opened the public pricing page in a new tab. The machine already knows which account it belongs to, so that discarded the identity it holds and asked somebody who had just signed in to go start a checkout from scratch on a marketing site. The second dead end was worse because it had no exit at all: if the Pro licence belongs to a different email, the onboarding gate blocks the dashboard and every button on it reuses the identity already on disk, so the only way off the wrong account was `clawmetry disconnect` in a terminal, which is not a thing the person this product is built for will find.
- **What:** the expired step now sells in place. A monthly/annual segmented control, Starter and Pro cards priced per node with the previous ladder struck through, the free $149 desk device shown on annual and hidden on monthly (the cloud already collects a shipping address on annual checkouts and ships on the first paid invoice, so it is a real promise and not a made-up one), and one button that POSTs `/api/trial/checkout` with the chosen `{tier, plan}` so the cloud mints a Stripe Checkout Session scoped to this account. Prices and blurbs come from `window.CM_PLANS`, published by the hard-block overlay in `app.js`: two hardcoded price ladders is how a repricing ships half-done. The step also polls for an entitlement now, because signing in as an account that ALREADY pays lands on this exact screen (the self-host rail reports "trial expired" because the trial is spent, while the real licence arrives on the daemon's next heartbeat), and selling somebody a plan they own is worse than any dead end. `POST /api/account/signout` forgets the account: the licence, the cm_ key and its sync state, this gate's recorded choice, and the desktop shell's own onboarding stamp. All four go together or the operator is still stuck, and because `run_daemon()` reads `config.json` once at startup and holds the key in memory for the life of the process, it also restarts the daemon, or the next heartbeat would write the old account's licence straight back. Ingested DuckDB data is untouched: this is an identity change, not a reset. It is offered where each person actually is, as "Use a different account" on the modal and "Switch account" in the profile menu, both behind a second explicit click. `/api/onboarding/*`, `/api/cloud-cta/*` and `/api/account/signout` join the hard-block allowlist, because both ways OUT of the block (pay, or sign in as the account that already paid) have to answer while it is up, or the block is a trap rather than a paywall; sign-out strictly narrows entitlement, so it can never become a bypass.
- **Verified:** ran a dashboard from the branch against an isolated HOME and drove it in a real browser. The picker renders and switches live ($90/$190 per node/year to $9/$19 per node/month, device line hiding on monthly); the checkout button opens its tab synchronously, as popup blockers kill a `window.open` from inside a fetch callback, and reports that it is waiting for payment; the sign-out link arms on the first click and the live endpoint returned `ok`, reopened the gate, wrote the nocloud marker and brought the daemon back local-only; the profile menu renders "Switch account" for a signed-in account. 15 new tests plus the existing onboarding and hard-block suites; ruff clean on every file touched, `dashboard.py` holds at its pre-existing 216 findings. Blueprints *Local Agent Observability* (v16) and *Team Access and Subscription Management* (v7) reconciled across four drift-bot rounds; two findings in the final round were merge-base misreads and are disproved on the PR rather than written into the spec.
- Also drops an em-dash from the sign-out confirmation label, per the house rule that user-facing copy does not use them.

### Fix: an always-visible refresh button, so a wedged dashboard is one click from working (#4988) (2026-08-18)
- **Why:** follow-up to #4976. That change made a dead backend visible; it did not make it easy to leave. Founder question, directly: can there not be a refresh button so the user gets back to a stable running state without closing and reopening. There could not, previously: a pywebview window has no address bar and no reload button, and the Cocoa backend swallows the native Cmd-R, so the dashboard shipped with no way to retry a page at all.
- **What:** one recovery implementation, `cmReconnect()`, behind three surfaces: an always-visible refresh control in the topbar (spins while working, turns amber once the backend is known unreachable, so the way out is discoverable before the operator goes hunting), Cmd/Ctrl+R re-implemented in JS for the shell, and the existing outage overlay's button rewired to the same function instead of carrying a second copy of the logic. The important part is what it does NOT do: a bare `location.reload()` against a closed port replaces a frozen-but-readable dashboard with a blank WebKit error page carrying no controls, which is strictly worse than the freeze it was meant to clear. So `cmReconnect()` probes `/api/health` first (with the unwrapped `fetch`, so a failing probe cannot inflate the outage counter that raised the overlay), reloads when the backend answers, restarts the backend over the JS bridge and waits for it to bind when a bridge exists, and otherwise refuses to reload and says what does help. In a real browser the Cmd-R handler deliberately stands aside, because native reload is already correct there.
- **Verified:** booted a live instance and drove it in a real browser: the control renders in the topbar, an outage turns it amber and raises the overlay, and the overlay's action works. 17 behavioural JS tests (up from 9) cover every branch, including the one that matters most, that a dead backend with no bridge must not reload; 7 Python guards pin placement (dashboard.py defines DASHBOARD_HTML twice and the second wins, so markup in the first renders for nobody), the absence of a bare `location.reload`, the exported helpers, probe isolation, the CSS states and the locale key. Reverting auth-bootstrap.js fails the JS suite; `dashboard.py` holds at its pre-existing 113 ruff findings both before and after. Blueprint *Desktop Application Distribution* reconciled to v13.
- **Reach:** the topbar control and the overlay ship in the wheel, so they land on existing desktop installs at their next auto-update (offering "Reload", since the old shell's bridge has no `restart_backend`). The Cmd-R and bridge-restart paths need the new shell and arrive with this release's bundles.

- **Release: Kimi CLI support goes live. `pip install clawmetry` now observes Kimi CLI and Kimi Code CLI out of the box (with a paid plan). Carries #4978 to PyPI and the fleet.**
  - Pairs with clawmetry-pro 0.7.9 (the `KimiAdapter` wheel, clawmetry-pro#145/#146). Upgrading daemons that are entitled provision a pro build that knows the runtime immediately; the runtime is detected zero-config from `~/.kimi` and `~/.kimi-code` with no configuration.
  - **What lands for a Kimi user:** sessions and transcripts (turns, thinking, tool calls with their approvals category, tool results), per-step token usage with the cache read/write split, subagents as child sessions with real lineage, compactions, approvals, step retries, and the runtime's own log file under Logs.
  - **Verified before release:** the feasibility question was answered by running the real CLI, not by reading its README. Real captures from kimi-cli 1.49.0 (wire protocol 1.10) drive 29 adapter tests, including a subagent instance the CLI actually spawned and a proof that the share dir is byte-identical after every read. `kimi` also joins the pro live-conformance matrix as its first keyless leg: CI installs the real binary and runs a real turn with no API key and $0 spend, then asserts through the adapter that the turn landed, was attributed to `kimi`, and surfaced tokens.
  - **One honest limit:** kimi-cli does not write the model id to disk, so cost is derived from the configured model and labelled `estimated`. When the installed pricing table does not know the provider the adapter reports `tokens_only` rather than a fallback-rate figure that would look real and be wrong.


- **Add Kimi CLI / Kimi Code CLI (`kimi`) as the 22nd runtime — a paid runtime, adapter in `clawmetry-pro`.**
  - **Why:** founder ask 2026-08-18 ("we support 20+ harnesses, can we add MoonshotAI/kimi-cli?"). Feasibility was proven before any code was written, by cloning the repo and running real headless turns: kimi-cli persists a complete per-session event log on disk, so Sessions, Transcripts, Brain, Usage, Subagents and Logs all light up from the filesystem.
  - **What (OSS wiring; the `KimiAdapter` itself ships in clawmetry-pro 0.7.9):** family-adapter spec + lite detection paths in `sync.py` (both share dirs, `$KIMI_SHARE_DIR` and the `CLAWMETRY_KIMI_ROOTS` override), entitlements (paid runtime, label, landing path, `kimi-cli`/`kimi-code` aliases), runtime prefix sets (`local_store`, `routes/usage`, `routes/harness`, `routes/attention`), runtime probe, Memory/Skills catalog entry (`AGENTS.md` at `<dir>/.kimi/` and root, `~/.kimi/skills`, plugins, `config.toml` hooks), Kimi's PascalCase tool names in `tool_risk.py` so `Shell`/`WriteFile`/`FetchURL` classify like every other runtime's, Moonshot pricing in `providers_pricing.py` (K3 / K2.7 Code / K2.6 / K2.5 / Moonshot V1, from platform.kimi.ai 2026-08-18), all six `app.js` runtime maps (labels, prefixes, caps `SESSIONS/EVENTS/COST/SUBAGENTS`, flow tile, icon 🌙), README/i18n/docs counts via `scripts/sync_runtime_count.py` (21 → 22).
  - **One runtime, two homes.** kimi-cli is being folded into the standalone Kimi Code CLI, which uses `~/.kimi-code` instead of `~/.kimi`. Both write the same store shape, so one adapter reads both and reports one runtime. This also closes a real gap: numbat already installs its hooks into `~/.kimi-code/config.toml` and stamps `source_agent="kimi-code"`, which without an alias fell through to an uncatalogued `kimi_code` runtime id nothing else in the app knew about.
  - **Cost honesty:** kimi-cli records the full token split with cache reads and writes, but never the model id. The adapter resolves the model from `config.toml` and labels the session `cost_status="estimated"` (a subagent that recorded its own effective model gets `derived`), and refuses to price at all when the installed pricing table does not know the provider, rather than letting the generic fallback rate produce a plausible invented figure.
  - **Verified:** real captures from kimi-cli 1.49.0 (wire protocol 1.10) including a real subagent instance; 29 pro adapter tests; `scripts/sync_runtime_count.py --check` clean at 22; entitlement/catalogue/memory-catalog/tool-risk guards updated in the same change. `kimi` also joins the pro live-conformance matrix as its first keyless leg (kimi-cli's built-in echo provider runs a real turn with no API key and $0 spend).

### Fix: the desktop app's supervisor could die silently, freezing every tab (#4976) (2026-08-17)
- **Why:** a desktop window sat frozen across every tab for 6 hours 39 minutes. Activity showed `Failed to load: TypeError: Load failed` under a `Connection lost. Reconnecting...` banner that never recovered; Cost sat on `Loading...` with `--` tiles; Agents reported "No agents yet, and that is fine." The app process was alive and responsive the whole time. It had no backend. The daemon's own in-process updater pip-installed a new release into the runtime venv it runs from, which unlinked `venv/bin/clawmetry` with the old distribution. Four seconds later, with the install still in flight, the watcher read the new version off the freshly-created `dist-info` directory name, saw drift against the running daemon, stopped a healthy serving daemon, waited 500ms, and called `Popen` on a console script pip would not re-create for another 548ms. `FileNotFoundError` propagated into a loop with no exception handler, on a bare daemon thread, inside a windowed PyInstaller bundle with no stderr sink. The exception vanished, the watcher thread died, and nothing remained to respawn the daemon or reload the window; `bootstrap.log` ends mid-story at the drift line. Two things turned a narrow race into a six-hour outage: nothing was watching the watcher, and the window had no recovery affordance at all (pywebview supplies no browser chrome, its Cocoa backend swallows Cmd-R and strips the context menu, and the app menu offered only Quit and Uninstall).
- **What:** supervision is now designed for silent death. The watcher body moved into `_tick()` behind `try/except BaseException`, records `_last_tick`, and is re-armed by a watchdog when that goes stale. `start_daemon()` probes the launcher, catches `OSError`, and returns a bool instead of raising. `restart_daemon()` probes *before* `stop()`, so a half-written venv costs a deferred restart rather than a working backend, and retries with backoff. Drift must hold across two consecutive ticks and `_get_installed_version()` requires `dist-info/RECORD`, so the shell only restarts onto an install pip has finished. The child is spawned with `CLAWMETRY_AUTO_UPDATE=0` so one venv has one updater. Every off-main-thread pywebview call is bounded by `_gui_call()`, since `cocoa.py`'s `evaluate_js`/`load_url` park the caller on a semaphore with no timeout. The operator now gets a way out at two layers: native "Reload Dashboard" and "Restart Backend" menu items plus `restart_backend`/`reload_dashboard`/`backend_state` on the JS bridge (the only channel reachable once the HTTP origin is dead), and a global frontend outage overlay that fires after 3 consecutive network-level fetch failures, names the problem on every tab, and always carries an action.
- **Verified:** root cause established from the live machine, not inferred: `dist-info` and console-script `st_birthtime` pin the 548ms window; the absent `app-instance.json` pins execution to the `Popen` call (`_write_instance_file()` runs on the next statement); and `sample(1)` of the wedged process showed no Python threads besides the Cocoa main loop, refuting an initial deadlock hypothesis. The race was then reproduced directly: during one `pip install --upgrade`, a tight poller caught the console script absent on 29/265 samples with 28 `FileNotFoundError`s. 20 supervisor tests plus 9 behavioural frontend tests; all 29 fail against pre-fix main and pass with the fix. Blueprint *Desktop Application Distribution* patched to v12 (ADR-010); drift-bot green.
- **Reach:** the frontend half ships in the wheel and lands on existing desktop installs at their next auto-update. The shell half ships in the bundle, so it reaches users when they redownload the `.dmg`/`.exe`/`.AppImage` this release builds.

### Fix: the Security tab tells the truth (#4973) (2026-08-17)
- **Why:** the tab printed "No threats detected" on a machine holding 1,060 recorded agent-EDR findings, 938 of them high and 2 critical, and it announced "Tampered, the activity log may have been altered" about a log where nothing had been altered. Four independent defects: (1) the built-in signature scanner gated on the legacy `EXEC/READ/WRITE/BROWSER/SEARCH` labels that only the JSONL parser produced, while DuckDB-first rows arrive as `TOOL_CALL/TOOL_RESULT/MESSAGE/THINKING/ERROR`, so the two vocabularies never intersected and the scanner was structurally pinned at zero no matter what an agent did; it also read `ev["source"]`, a key the DuckDB path does not emit, so a node with five live sessions reported one. (2) The hash chain is built per flush batch sorted by id but was verified in `created_at` order, which assumes one batch per millisecond; two batches inside the same millisecond interleave two independently chained runs and the walk reports a break. (3) Two Claude Code posture checks returned `pass` on every code path, donating 10 weight of unearned credit, which is how an install with no deny rules scored A/94.4%, and Codex scored an A on an empty `config.toml` because both of its risk checks passed on absent keys. (4) The severity tiles and the all-clear line were painted from the live scan alone, so they contradicted the findings panel directly below them.
- **What:** the scanner bridges both event vocabularies and reads the session id both paths emit; agent speech (`MESSAGE`/`THINKING`) and tool RESULTS are excluded from the action path, since a result is data the agent received and scanning those free-text blobs produced only false positives. Integrity verification now follows the `chain_prev_hash` links instead of assuming a row order, and separates content (does each event still match its own hash, the real tamper signal) from linkage (can the events be placed in one ordered chain), adding a third `degraded` outcome so a writer artefact is never reported as tampering and an insertion is never reported as clean. The always-pass posture checks now either warn or carry zero weight, and an unset Codex sandbox reads as unmeasured rather than safe. Findings are scoped server side by runtime and counted in SQL, so the tiles show the real total instead of the page size, and `delete_security_events_by_id_prefix` gives one engine's output an undo.
- **Verified:** measured against a live 42,110 event chain on a real node: 0 content mismatches and 4,510 fork points, so the "Tampered" banner had been firing on a log that was fully intact. The scanner was confirmed live before and after: 4 hits (all false positives, all from tool results) then 0 hits across 6 sessions, where it had previously been incapable of reporting anything. Both integrity regression tests were confirmed red against the previous verifier before being kept. CI's unit list was run on the branch and on clean main with identical results (32 failed / 82 passed / 10 errors both sides), so the change adds no regressions.

- **Exo gets its public page link, and "add a runtime" becomes a checked, cross-repo flywheel step instead of a memory.**
  - **Why:** founder report 2026-08-17 with two screenshots: the homepage "Supported agent runtimes" grid stopped at DeepSeek Harness (20 tiles) and the README's "Works with 21 agent runtimes" line listed Exo as bare bold text. Exo had shipped in the catalogue, the wheel (0.12.726), the cloud pin and `/api/runtimes` a day earlier; nothing on the storefront followed, and no guard noticed.
  - **What:** README grid now links Exo to `https://clawmetry.com/runtimes/exo` (page ships in clawmetry-landing#646). New `RUNTIME_LANDING_PATHS` in `clawmetry/entitlements.py` declares the public page for every catalogue runtime, next to `RUNTIME_LABELS`. New guard `tests/test_runtime_public_surfaces.py`: offline it asserts the map covers exactly `ALL_RUNTIMES` and the README grid carries a link to each; with `CLAWMETRY_LIVE_CHECKS=1` it also asserts every page is 200 on clawmetry.com (revert-proven: unlinking Exo goes red). FLYWHEEL.md section 2a is now the canonical four-repo "adding a runtime" checklist (pro adapter, OSS wiring + count + `[RELEASE]`, cloud roll + pin, landing page + tile + counts + README) with the order of operations; the pro, cloud and landing FLYWHEELs point at it.
  - **Verified:** `scripts/sync_runtime_count.py --check` clean at 21; new guard green offline; the live variant currently fails on exactly one path (`/runtimes/exo`, 404 until the landing PR deploys), which is the intended signal.
...
- **Release: the Sessions list stops showing sessions that open to nothing, and a healthy local-only node stops reporting its own heartbeat as dead. Carries #4943 to PyPI and the fleet.**
  - **Why:** founder report 2026-08-16, from a screenshot of a session detail card reading "Messages 0" with no model, no duration and no turns, plus a red "Agent heartbeat SILENT for 8h 3m" banner that came straight back after every Dismiss. Four defects behind the two symptoms.
  - **Ghost sessions.** Sessions are derived by grouping the whole event history by session id, so an id that appears only on a gateway log line (`{event_type: "log", data: {kind: "gateway_log"}}`) mints a session with no conversation in it. Those sort to the TOP, because a log line is newer than the last real turn of any long-running session: on the reporter's install 22 of the 50 listed rows were ghosts, and the first row they clicked was one. #1718 had already made the reported count correct (0); the list simply never acted on it. The list now withholds sessions that would render no turn, and scans up to 400 candidates to fill its 50 rows so a ghost-heavy head cannot shorten it (the LIMIT applies after the GROUP BY, so the wider scan costs DuckDB nothing).
  - **The 200 that should have been a 404.** In the detail path the existing "no rows" guard never fired, because rows DID exist, they were just all non-renderable. It served a zero-filled 200 that rendered as a real session containing nothing, and it pre-empted the JSONL fallback that could still have read the transcript off disk (#1772). It now declines, so the fallback runs and the request 404s when that also finds nothing. The frontend needed the matching guard: `showTranscript` read `data.name`/`data.messageCount` straight off the payload, so a 404 body would have painted "Session undefined".
  - **The heartbeat banner was lying.** `send_heartbeat()` returns early when there is no cloud egress (local-only, or self-hosted with no linked account), and the sync loop's no-egress branch never calls it. But the DuckDB write-through lived INSIDE `send_heartbeat`, so on those nodes the `heartbeats` table froze at whatever row predated the switch and the reported gap grew without bound. Observed live: newest row v0.12.712 at 22:16 UTC while the daemon ran fine on 0.12.715, banner climbing past 8h. The write-through's own comment says it exists "so the dashboard has a per-node liveness history even when offline"; it was dead for exactly those users. Recording is now independent of delivery. The rest of `send_heartbeat` was audited: `ingest_heartbeat` was its only local side effect, so the class is closed. This makes the table honest; it is still never a liveness gate.
  - **Dismiss never stuck.** The button set `display:none` inline and the 30s poller set it back to `flex`, so Dismiss bought the user 30 seconds. It is now keyed on the last-heartbeat timestamp, which is constant through one outage and changes when the agent checks in, so dismissing silences that outage while a genuinely new one still alerts.
  - **Verified:** against live DuckDB on a real install, not fixtures. `/api/transcripts` went from 50 rows with 22 ghosts to 50 rows with 0, still a full 50; the ghost id went from 200-with-`messageCount: 0` to 404; a real 230-message session was unchanged. Dismiss driven through the real functions: fresh silence shows, Dismiss hides, two poller ticks keep it hidden, a new outage re-alerts, recovery hides it. Each of the three guards revert-proven red then green. All 30 CI checks green including drift-bot (2 blueprints + 2 requirements patched, with ADR-012/013 and ADR-021/022).
  - **Also found:** store-backed tests pass VACUOUSLY on any dev box running a daemon. `get_store()` hands back a proxy to the real `~/.clawmetry` DB whenever one is registered, so one existing test was reading 500 production rows and asserting 6. Fixtures need `mark_writer_owner()` or a throwaway `HOME`.
...
- **Add Exo harness (`exo`) as the 21st runtime — a paid runtime, adapter in `clawmetry-pro`.**
  ...
  - **Notable adapter semantics (verified on a real store written by the exoharness crate's own storage code):** an Exo fork COPIES the parent conversation's full event history before the `thread_forked` marker — the adapter resets inherited usage so forks never double-count spend; usage records whose `cost_usd` Exo left unset (price-table fetch failure) report `cost_status` honestly (`exact`/`partial`/`tokens_only`).
  - **Verified:** 16 pro adapter tests (fork-reset guard revert-proven red→green), full pro suite green, OSS count-sync `--check` clean at 21, entitlement/catalogue/memory-catalog guards updated in the same PR.

- **Release: the Alerts tab no longer wedges on "Loading alerts…" once a rule is enabled, and self-hosted edits actually save. Carries #4936 to PyPI and the fleet.**
  - **Why:** founder report 2026-08-16, from a screenshot of the Alerts tab stuck on "Loading alerts…" — "once enabled — I'm unable to modify or edit alerts". Three stacked defects, all in the self-hosted (local-entitlement) path.
  - **The wedge.** The local store returns a rule's `channels` as a JSON *string* (`'["banner"]'`). The local-mode normaliser in `alerts.js` copied it straight into `channel_ids`, and `renderRules` threw `TypeError: .map is not a function` — an uncaught exception that aborted the page load before any row was written. With zero saved rules the `.map` path is never reached, so the tab worked right up until the moment you enabled your first rule, then never rendered a row (and therefore never an Edit button or toggle) again. Channel lists are now coerced to real arrays in the normaliser and again at the render site, so one malformed rule can never blank the whole tab.
  - **The paywall that gated paid users.** `alertsSaveRule` fired the cloud-signup paywall on ANY save of a canned example row — including for a locally-entitled Pro/trial user, who therefore could never save an edited example. The paywall now fires only for free/no-cloud tiers; for entitled users, saving an example creates the real rule (the old path would have PUT to `example_*`, a 404).
  - **The edits that silently did nothing.** The local `PUT /api/alerts/rules/<id>` only understood the local schema (`threshold`/`channels`) while the editor sends the cloud vocabulary (`threshold_value`/`channel_ids`) — so changing a threshold in the editor returned ok (because `enabled` was present) and changed nothing. The PUT now applies the same field bridge the POST path has had since 0.12.721. Rule of the class: every cloud-vocab bridge added to POST must be mirrored in PUT.
  - **Verified:** reproduced live against the reporter's own store (exact TypeError captured in the browser console), then the fixed branch run on :8901 over the same data — 10 rows render including the enabled rule and two orphans, Edit opens the populated editor, and an editor save of the Daily-spend rule (50 → 75) persisted server-side and re-rendered; state then restored. All 24 CI checks green including drift-bot.

- **Approval delivery moved behind the license. Sending an approval to Slack/Telegram/WhatsApp/email/phone, and answering a runtime's own permission prompts from your phone, now ship in `clawmetry-pro`. Everything that PAUSES an agent stays open source.**
  - **Why:** the feature shipped straight into the public repo in 0.12.713, against the open-core rule that paid implementations live in the private package. Four PRs moved it back out: #4904 (seam), clawmetry-pro#139/#140 (impl + 0.7.7), clawmetry-cloud#2029 (wheel roll), #4905 (delete + 402 stub + boundary guard).
  - **Where the line is drawn — and why there.** The approval stack for paid runtimes was ALREADY public and predates this feature (`approvals.py`, `claude_code_gate.py`, `routes/hooks.py`, `routes/policy.py`). So this was not a clean lift of a self-contained feature; it was a decision about where to cut machinery that was already half-public. The cut is between **pausing** and **paging**: the policy engine, both pre-execution gates, the permission-prompt mirror installer/receiver and the approval queue stay open source, so an unlicensed install keeps a complete approval workflow — approvals park, render, and are decidable. Only telling a human ELSEWHERE is licensed.
  - **New OSS mechanism: `extensions.call()`.** `emit()` is fire-and-forget, which covered the three sites that only announce ("an approval parked", "one resolved"). Three sites ASK a question — is mirroring on for this runtime, and how long does the human get — and there was no mechanism that returns a value. `call()` returns the first non-None handler result; `None` means "no opinion, try the next"; a raising handler is skipped; an empty registry returns the caller's default. `clawmetry/approval_events.py` is the published boundary, so no OSS module ever names a module that may not be installed.
  - **The safety property.** With no paid package, `mirror_wanted()` is False and the `PermissionRequest` hook is never installed — a node that loses its license returns to the runtime's own terminal prompt rather than keeping a hook aimed at a component that stopped answering. A stale hook surviving a downgrade is already covered by the receiver's contract: no answer means `ask`, which IS that prompt. Paid handlers must return real `False`, never `None`, or an explicit "off" is read as "nobody answered" and silently replaced by the OSS default.
  - **Entitlements.** Delivery rides on the existing `approval_queue` (Starter) — an approval queue that cannot tell you is not the feature the Starter card promises. New `approval_mirror` (Pro) gates answering hands-free from a phone. CI rejected the first attempt at a separate Starter key: `STARTER_FEATURES` is locked to /pricing's exact 7 keys, so adding one is a public pricing change, not a code change.
  - **BLOCKER found and fixed in the same PR — self-hosted nodes never upgraded `clawmetry-pro`.** `_download_and_install_pro` is reached ONLY from `activate_license()`, so a signed-license node installed the wheel current on its activation day and froze there; cloud-key nodes track releases through the ~30-min entitlement watcher. Harmless for as long as nothing had ever MOVED out of OSS — and this is the first thing that has. Without the fix, OSS drops the code, the frozen wheel lacks it, and approval notifications disappear on every self-hosted licensee with no recovery path and no prompt to re-activate. New `license.refresh_pro_from_license()` runs on the entitlement watcher's no-`api_key` branch (which previously did nothing at all); safe on a schedule because `_provision_pro_wheel` already version-compares and no-ops unless the served wheel is newer, honours `CLAWMETRY_OFFLINE`, skips when pro is absent (first install belongs to activation), and never raises into the watcher thread. Reloads plugins after a real upgrade so the capability attaches without a daemon restart.
  - **Verified:** 29/29 CI green, drift-bot green (4 blueprints patched across both PRs, incl. ADR-005 "split at delivery, not at approvals" and ADR-006 "licensed components must reach every licensed node"). Pro 481 passed/25 skipped. New OSS tests: 18 seam, 15 mirror-gate (rewritten to drive the seam with a fake handler rather than a routing config — the honest contract shape), 10 boundary guard (both guards verified falsifiable by reintroducing the violation), 6 self-hosted refresh. Pro wheel cracked before the cloud roll; Cloud Run deploy succeeded, so 0.7.7 is downloadable ahead of this release.
  - **Also fixed:** a test-isolation bug this work created — extension registrations are module-global and `monkeypatch` cannot undo them, so a fixture leaking `MIRROR_WANTED→True` made every later `gate_handler` in the session install a mirror hook.
- **Release: every alert now has somewhere to go, and the Alerts tab stops offering rules that nothing evaluates. Carries #4903 and #4913 to PyPI, the fleet, and the cloud pin.**
  - **Why:** founder ask 2026-08-15, from a screenshot of two red banners sitting above an Alerts tab with every rule switched off. Three questions, each a separate defect: "unable to understand what action I need to take -- for the first one I just see dismiss button -- so what??", "how did it alert when all of alerts are disabled?? are those default alerts??", and "if I enable them it says no channels -- why not just ask to configure channel first before enable, and take me to notifications page??".
  - **The alarm had no destination anywhere in the product.** The banner built a deep-link for exactly one alert type (`stuck_session`); everything else, including a HIGH secret-exfiltration finding, offered only Dismiss. Worse, the Security tab could not have shown that finding either: it calls `/api/security/threats`, a live re-scan of recent activity through the built-in signature catalog, while an ingested finding lands in `security_events` and is served by `/api/security-threats` — two similarly-named endpoints over completely different data, and nothing rendered the second. New `_cmBannerDestination` maps each alert to the screen that answers the question it raises, and a new **Recorded findings** panel reads the durable log with each row linking to the session that produced it. Dismiss is not a response to a security alert; it is the absence of one.
  - **Two enabled rules that could never fire.** `routes/alerts.py` keyed its cloud-to-local map on `cost_daily`/`agent_offline`; the Alerts tab has always POSTed `daily_spend`/`node_offline` (`alerts.js` EXAMPLE_RULES). Neither matched, so both fell through to the `.get(..., "anomaly")` default — and the monitor loop has no `anomaly` branch. The rules rendered green, said "never triggered", and permanently could not. The map is now explicit and total: every type routes to the in-process loop, to the daemon's `alert_evaluator`, or is refused with 422 rather than stored. `agent_down` and `session_cost` had been accepted by the POST validator for months with no branch at all; both are now implemented, and `agent_down` reads the latest DuckDB event instead of `_otel_last_received`, which is 0 on any install without `[otel]` — so the built-in monitor of the same name had never fired there either.
  - **Nothing bridged the two evaluators.** A node runs two: the dashboard's in-process loop over the fleet SQLite DB, and the daemon's `alert_evaluator` over DuckDB. The dashboard's rule POST wrote only SQLite, and `ingest_alert_rule` had never once been called outside the cloud pending-action relay — so a rule created from the Alerts tab on a no-cloud node was evaluated by nobody. Rules owned by the daemon's evaluator are now mirrored on create, update, and delete, preserving the cloud vocabulary the evaluator parses. The shared `alert_history` already dedupes by rule id inside the cooldown window, so no rule double-fires.
  - **The mirror had to be made honest, twice.** `ingest_alert_rule` returns `None`, and `local_store_via_daemon` also returns `None` when the proxy call *failed* — indistinguishable at the call site. Caught live: against a daemon on an older wheel (whose method allowlist predates the bridge) the first implementation reported success for a write that never happened, and fell back to opening a DuckDB **writer handle from the dashboard process** — the documented brick-lock hazard — where the row landed somewhere the daemon would never read. It now reads back before claiming success, and only touches the store directly when it *is* the daemon. Then the same upgrade window needed closing from the other side: dashboard and daemon ship in one wheel but restart independently, so `_repair_missing_mirrors` re-mirrors a stranded rule on the next rules GET, once per rule per process, and no-ops when the store is unreadable.
  - **Monitors that fire with no rule behind them are no longer invisible.** `heartbeat_silent`, `numbat_finding`, the cost anomaly and seven others call `_fire_alert` directly, bypassing every toggle on the page — which is the whole answer to "how did it alert when all of alerts are disabled". They are now declared in `BUILTIN_MONITORS`, served by `GET /api/alerts/builtins` (deliberately ungated: knowing what watches you is not a paid feature), and rendered as an **Always on** section.
  - **Enabling asked for nothing, then reported "no channels".** Toggling a rule on POSTed `channel_ids: []` silently and relabelled the row — which read as though enabling had *deleted* the Slack/Email that row advertised a second earlier. Those pills were decorative `_exampleChannels` samples and are gone. A prompt now asks before arming, offering destination setup or in-app-only delivery, and remembers the choice for the visit.
  - **Found while verifying, on the reporter's own node: the two dead rules were also invisible.** `renderRules` matches saved rules to the eight canonical rows by `alert_type`, and rules written before that column match none — so they rendered nowhere at all. Two enabled rules, unseen, unturn-off-able, and unable to fire. They now appear as **unrecognized** with a plain-language reason and a Remove button, identified by threshold rather than by their internal storage type.
  - **Verified:** live at 0.12.710 against the reporter's own store — the banner renders "Investigate", the findings panel lists their numbat finding (`chain.secret_manager_read_then_egress`) with its session, `/api/alerts/rules` reports their two rules as `evaluator: "none"` / `needs_recreate: true`, and ten always-on monitors render with a clean console. End-to-end routing confirmed by POST: `daily_spend` → dashboard evaluator, `node_offline` → dashboard, `error_rate` → daemon with the mirror reported truthfully, `cron_failure` → HTTP 422. Test parity matches the origin/main baseline exactly and lint is unchanged at 223 pre-existing findings. Four new test files pin the lists that were free to drift — every UI alert type must route somewhere, every routed target must have a real branch, every hardcoded `_fire_alert` must be declared, every builtin must have a destination — and each fails against the pre-fix code. drift-bot's findings were closed by patching the Factory blueprints (Remote Alerts v3→v5, Cloud Fleet Dashboard v2→v4) and adding REQ-CLOUD-RAAA-004/005, not merged past.

- **Release: approvals now page you where you actually are — per runtime — and Claude Code's own permission prompts can be answered from your phone instead of the terminal.**
  - **Why:** founder ask 2026-08-15, from the Approvals tab: "ideally it would be nice to receive approvals to enable for each agent runtime — eg claude code if permissions are missing agent won't proceed so I have to run /permissions and manually tick them — if that can come to my slack or whatsapp or via call it would be faster". Two real gaps behind that. (1) A parked approval notified **nobody**: `clawmetry/approvals.py` had zero send paths, and the Slack/Telegram/PagerDuty plumbing in `routes/alerts.py` was wired only to alerts. A self-hosted node's approval sat silently until someone looked at the tab — no faster than walking to the terminal, which is why the pre-execution gate bought nothing in practice. (2) The Claude Code gate only arms for tools a protection rule names; the thing that actually stalls a session is Claude Code deciding *on its own* that it needs permission, and that never reached the queue at all.
  - **Per-runtime routing.** New `clawmetry/approval_notify.py` fans a parked approval out to the channels configured for THAT runtime (`~/.clawmetry/approval_routes.json`; runtime rows fall back to `default`, unknown runtime → default, never silently dropped — the same scoping contract as `alert_rules.runtime` in 0.12.642). Credentials are the EXISTING alert-channel config, so a channel is set up once and both features use it. An empty channel list means "every channel that has credentials", so nobody has to configure twice to get paged at all. New `GET/PUT /api/approvals/routing` (+ `/test`, which delivers synchronously so it can report exactly which channels accepted), rendered as a **Where approvals go** panel on the Approvals tab.
  - **Channels, with their real capabilities.** Telegram, Slack, Discord, generic webhook, PagerDuty, WhatsApp (Meta Cloud API or Twilio), email (local SMTP), phone (Twilio voice) — added to the Notifications tab alongside the existing cards. Only **Telegram** can carry a decision back to a self-hosted node with no public endpoint, so it is the only one that gets real Approve/Deny buttons: `clawmetry/approval_inbound.py` long-polls `getUpdates` from the daemon and resolves the DuckDB row through the same first-click-wins `update_approval_decision` the dashboard button uses (presses from any chat other than the configured one are refused). Everything else carries a signed decision link to the node's own `/a/<id>` page, which works from a phone on the same LAN or Tailscale. The UI states this per channel rather than implying a button that does not exist: Slack interactive callbacks and WhatsApp button replies need a public HTTPS endpoint, and Twilio's press-1-to-approve needs a public TwiML callback — those stay cloud features, and the local call still *announces* the approval.
  - **Mirror mode: answer Claude Code's own prompts from your phone.** Claude Code fires a `PermissionRequest` hook at exactly the moment it would stop and ask you, and a hook may answer it `allow` / `deny` / `ask`. `claude_code_gate.py` now installs that hook when mirroring is on for `claude_code` (independent of the policy set — it works with zero protection rules), and the new `POST /api/hooks/claude-code/permissionrequest` receiver parks it in the same queue, pages the phone, and answers with your tap. **The fallback is the point:** no answer inside the window (default 3 min, configurable 1 min – 1 h) returns `ask`, which is Claude Code's normal prompt — so the worst case is exactly today's behaviour and nothing this endpoint does can leave a session more stuck than it already was. Same for mirroring off, an unentitled node, or an unreachable store. No double-parking with the PreToolUse gate: if that hook answered, Claude Code never reaches the permission stage.
  - **`GET /a/<id>` never decides.** Mail clients and chat apps prefetch links; a GET that approved would let a link preview run the command. The page renders, the human taps, a POST decides. Auth on both is a 128-bit HMAC (per-node secret, `0600`, constant-time compared, scoped to one approval id) — deliberately not loopback-only, because a phone is not loopback.
  - **Also fixed:** the alert-channel key allowlist existed in **three** hand-maintained copies (`_default_alerts_webhook_config`, `_save_alerts_webhook_config`, and the `/api/alert-channels` route), and a key missing from any one of them saved as a silent no-op — the documented cause of the Telegram card that never left "Connect". The route now derives its allowlist from the schema.
  - **Verified:** 29 new tests in `tests/test_approval_routing.py` covering routing resolution, the Telegram/Slack payload shapes, both config 400s, GET-never-decides, single-decision links, all four mirror verdicts, mirror install/uninstall isolation (`MIRROR_CMD_MARKER` contains `HOOK_CMD_MARKER` as a substring — the PreToolUse uninstaller must not claim the mirror entry), and the Telegram cross-chat refusal.
- **Removed: the Developer > Gateway screen. Nothing replaces it, because nothing was needed: the OpenClaw gateway token is auto-detected server-side.**
- **Release: the Quality tab stops making things up. Every runtime label it printed was manufactured, and its one failure signal was uncorrelated with reality — both are fixed, and no verdict now renders without the evidence behind it.**
  - **Why:** founder ask 2026-08-15, from a screenshot: the Quality tab attributed eight rough runs to "openclaw" on a machine that only runs Claude Code, and asked, reasonably, why any number on that screen should be trusted. Two independent fabrications were confirmed against the live DuckDB.
  - **The runtime label was a loop variable.** `sessions.agent_type` is hardcoded to `"openclaw"` for every row by `upsert_sessions` (`session.get("agent_type") or "openclaw"`), and nothing ever sets it — 61 of 61 claude_code sessions were stored that way. `query_outcomes` does not even SELECT the column, so `routes/quality.py` swept 17 hardcoded agent types and stamped each returned row with whichever one the loop was on. Only the openclaw pass ever returned rows, which made the other 16 iterations dead code and every session read "openclaw". The correct label, `metadata.runtime`, was one field away the whole time.
  - **The failure signal could not produce a true negative on most runtimes.** `_session_has_cognitive_loop` had exactly one false-positive guard — "a new tool or file appeared between these turns, so this is progress" — and it read tool use only from `data.message.content[*].type == "tool_use"`, the OpenClaw/Anthropic block-list shape. Family runtimes emit tool calls as separate `tool_call` events under `data.tool_calls[*]`, and that event type was not even in the classifier's scan set. Replaying the live classifier over a real 824-event session: 41 candidate turns, **0** with any tool use parsed. All eight flagged sessions had 39–63 real tool calls *inside the exact window the classifier examined*. What remained was a 0.85 Jaccard match on the last 200 characters of three assistant turns — which ordinary status narration ("standing by", "waiting on that merge") trips trivially.
  - **It was measurably noise.** Flagged sessions averaged a 3.59% real tool-error rate; "successes" averaged 2.96%. The six genuinely worst sessions by that rate were all labelled success, and no flagged session appeared among them. The same task title appeared in one week as `cognitive_loop` at $26.32 and as `success` at $49.46. `outcome_confidence` was a literal (0.8 flagged / 0.85 success), and `success` itself was the fallthrough default — 18 of 62 sessions were research chats with no tool calls collecting a free pass.
  - **What:** new `clawmetry/quality_signals.py` reads BOTH dialects into one normalized event, probes what a runtime actually emits, and enforces the contract that gives the tab its name: `Verdict.__post_init__` raises when the exhibit list is empty, so a claim without evidence cannot be constructed. Four signals (tool failure rate, identical-call thrash, edits never verified, runtime-reported errors) each declare the payload fields they need; a runtime that cannot supply them reports **not measurable** rather than a clean bill of health. New `clawmetry/quality_thresholds.py` calibrates against each runtime's own 30-day history on that install, with floors and a documented cold start, and the threshold's provenance travels into the evidence ("p90 of this install's own claude_code history (68 sessions)"). Confidence is derived from sample size and margin — it varies 0.43–0.95 on real data. Grading moved to ingest (`sync.py`, off the events already in hand) so the tab stays a pure DuckDB read. The grade is cost-weighted, because a plain mean let thirty cheap chats outvote the $82 run that spun on one file.
  - **The gate that would have caught it:** `tests/test_quality_runtime_conformance.py` ingests all 12 committed runtime fixtures and asserts that when the raw events contain tool calls, the reader parses them — with their inputs. Verified to fail when the original bug is reintroduced. It immediately found a second, independent blind spot: goose, n8n, opencode and qwen_code carry tool arguments under `arguments`, not `input`, so thrash and forward-progress detection would have been silently dead on all four while the capability probe still looked healthy. The contract tests caught a third, in the opposite dialect: Anthropic `tool_use` blocks ride inside `message` events and were being dropped.
  - **Verified:** against the live 62-session store — seven of the eight prior false positives now read clean, and the survivor is a true positive (five identical browser calls, two errored) whose evidence is on screen. The six genuinely-worst sessions now surface at 0.65–0.95 confidence. Playwright screenshot of the rendered tab shows correct per-runtime labels, evidence panels with real file paths, timestamps and error text, plain-English signal names, and a clean console. Related suite matches the origin/main baseline exactly (36 pre-existing dev-box failures, 0 introduced). Drive-by: `make lint-daemon-allowlist` was red on main — `query_replay_events` was missing, so session replay silently returned empty on daemon installs.
  - **The fabricated label is stopped at its source, not just on this tab.** `sessions.outcome` is also read by `/api/outcomes` (the Overview tile), the evals surface, and the cloud snapshot — fixing only the Quality tab would have left the same invented failures on other screens, and the founder's question was about trust, not about one tab. `classify_session`'s cognitive-loop branch now requires corroboration from an evidence-bearing signal (`tool_thrash` or `no_forward_progress`), delegating to `quality_signals` rather than re-deriving the checks so there stays exactly ONE reader of event payload shapes. The guard fails CLOSED: if the signals module is unavailable, no loop label, because failing open would restore the behaviour it exists to remove. Existing rows self-heal — `query_outcomes` re-classifies FAILURE labels stamped before the fix epoch, bounded by the same cap as the unlabeled path and self-limiting because reclassify stamps the row. Success rows are left alone: the old success branch was a conservative fallthrough that cannot have invented a failure, and re-reading events for every healthy session would be a large read for no correction. On the live store the eight sessions the old classifier called loops re-classify to seven successes plus one genuine loop; the second call takes 9ms.
  - **Three further false-positive classes found while reviewing the new detectors, each pinned by a test.** `no_forward_progress` only looked for verification BETWEEN the first and last edit of a file, which flags "edit a dozen times, then run the tests once" — one of the most ordinary ways anyone works; the window now runs to the end of the session, dropping the live flag count from 10 to 8 (both removed sessions genuinely did verify, just at the end). `tool_thrash` counted failures over the 12-item exhibit slice while reporting `identical_calls` over every repeat, putting two numbers on different denominators inside a single evidence block. And a session that simply had not been graded yet was described to the user as having "too little activity to judge" — a fact about us reported as a fact about their work.
  - **Honest limits:** benign-error filtering is a Pro feature, so an OSS install computes higher error rates than a Pro one on identical data — the tab now states which filter produced its numbers. The hosted dashboard has no local DuckDB; rather than reporting an empty week for a healthy machine, it now says the grade is computed locally. Sessions past the deep-scan cap are screened, never reported rough without evidence, and the counts are in the payload.
...
- **What:** wiring a runtime to report its own permission prompts is now one line — `clawmetry hook attention --runtime <name>` — instead of a multi-line command. It reads whatever the runtime hands it and cannot interrupt your agent: it always succeeds silently, so a misconfigured hook costs you precision, never a turn.
- **Fix:** Codex sessions were not picking up the folder they ran in. Codex records it in a place the reader did not look, so those sessions showed no project name. Found by checking each runtime's real captured session rather than assuming, which also confirmed OpenClaw, Claude Code, Qwen Code and Antigravity do report it, and that GitHub Copilot and PicoClaw record nothing to read.
- **Verified:** on the machine that exposed the check-in bug, with the same stalled service, the page went from "Can't tell right now" to "Nothing needs you right now, 2 agents working". Every runtime's location extraction is now measured against its real captured session rather than a hand-written example.

### Feature: see which agent is waiting on you, across every runtime (2026-08-16)
- **Why:** ClawMetry could say what an agent did and what it cost, but not the thing people actually open a dashboard to find out — is anything stuck waiting for me? An agent spinning through tool calls burns money and gets noticed; an agent blocked on a permission prompt burns nothing, silently never finishes, and can eat an afternoon before anyone spots it. Nothing detected the second case.
- **What:** the Overview page now opens with a plain answer. Most of the time it is one calm line — "Nothing needs you right now, 6 agents working" — and that reassurance is the point. When something is waiting it lists each agent, what it is waiting on, and which project and branch it is working in, with one click through to that session. The same badge appears on the sessions list so the two screens use one vocabulary.
- **What:** each row says how confident the answer is. When a runtime tells us directly that it opened a prompt, the row reads "Waiting for you". When it was worked out from an agent that went quiet mid-task, it reads "Maybe waiting" and says so. Two further states exist on purpose: a runtime that never asks for permission says so rather than reporting nothing waiting, and when the machine has gone quiet the page says it cannot tell instead of claiming all-clear. An all-clear you cannot trust is worse than no answer.
- **What:** sessions now record the folder and git branch they ran in, on every runtime that writes them down. Session lists identify a project by name instead of an opaque id, and cost per repository or per branch becomes a query rather than a new pipeline.
- **What:** Claude Code reports its prompts directly through the permission hook ClawMetry already installs. Every other runtime can be wired the same way with one line of its own config; `docs/NEEDS_YOU.md` carries the exact snippet for Qwen Code, Gemini CLI, GitHub Copilot, Antigravity and Aider, and states plainly which of them have been confirmed against a real install and which are read from vendor documentation.
- **What:** seeing that an agent is blocked is free on every plan and every runtime. Answering it remotely stays a paid capability.
- **Verified:** against real Claude Code transcripts from three projects; a real permission payload end to end from hook to rendered strip; every documented wiring snippet run verbatim against a live server; all screen states captured in both light and dark themes; and the schema migration exercised against a pre-change database with its existing rows intact.
