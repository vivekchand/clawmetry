...
### Added: GET /api/license endpoint surfaces self-hosted license metadata (2026-06-01)
- New read-only HTTP route in `routes/entitlement.py` (on the existing `bp_entitlement`) that exposes the same payload metadata `clawmetry license` prints on the CLI: `tier`, `nodes`, `sub`, `exp`, `days_left`, and a `status` of `active` / `expired` / `invalid` / `none`. Lets the dashboard render a license-status badge without shelling out to the CLI, while keeping the resolved entitlement (what the install is allowed to do) on `/api/entitlement`. The two endpoints are complementary: `/api/entitlement` answers "what features and runtimes are unlocked", `/api/license` answers "what license file is installed and how much time is left on it".
- Safety: the handler delegates to `clawmetry.license.current_license_info()` (which verifies the signature against the embedded Ed25519 public key) and never returns the raw key bytes or signature segment, so an unprivileged dashboard caller cannot extract the activated token. A tampered or forged file reports `valid: false, status: "invalid"` rather than 200/OK so the UI can prompt for reactivation. Side-effect-free and never-raise per the always-on-surface rule: any exception falls back to `{installed: false, status: "error"}`.
- Tests: 8 new cases in `tests/test_routes_license.py` covering no-file, active token, enterprise tier, expired, garbage file, forged signature, no-leak invariant on the token, and a monkey-patched read failure to confirm the never-crash fallback. Hermetic; each test mints its own Ed25519 keypair and patches the embedded public key, so nothing depends on the real production signing key. Grace mode unaffected; no enforcement is added. (#TBD)
