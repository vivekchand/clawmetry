...
- **Release: the desktop app's open reports now verify certificates, so they actually arrive from a packaged bundle. Carries #5025 to PyPI and the bundles.**
  - **Who this reaches:** anyone running the desktop app. The reports added one release ago could not leave a packaged bundle at all, so this is the release that makes that feature real rather than one that tunes it.
  - **What was wrong:** the report was sent without a certificate-verification context. Inside a packaged app that always fails, because the TLS library looks for its trust store at paths that exist on a build machine and not inside the bundle. It is the same failure that broke the onboarding "Send code" button in August, which is why the onboarding pane already carries a fix for it. The report should have reused that fix.
  - **Why nobody would have reported it:** these reports swallow their own errors on purpose, so that a network problem can never slow down or visibly break a launch. That means this failure had no traceback and no symptom. The only evidence was an endpoint receiving nothing, from exactly the installs the early report exists to observe.
  - **Also fixed by the same change:** the anonymous install ping has always been sent the same way, so it too failed on any interpreter without a working trust store, silently. It now takes the same path.
  - **Verified before release:** reproduced by sending the previous release's report at production from an interpreter with no trust store, where it reported success and nothing arrived. The same interpreter lands the report after the fix, with live runtime detection, and the verification rows were deleted afterwards so the live counts start at zero.

### Fix: the desktop open reports verify certificates, and so does the install ping (#5025) (2026-08-20)
- **Why:** found while verifying the previous release against production rather than trusting a green pipeline. The published wheel's report claimed success and no row appeared. With debug logging the cause was immediate: `CERTIFICATE_VERIFY_FAILED`, because the request went out with no verification context. A packaged bundle has no trust store at the paths the default context searches, so every HTTPS call from inside it fails. `desktop/onboarding.py` already carries a ladder built for exactly this (2026-08-12, the onboarding "Send code" button), and the new call did not reuse it. Every other outbound call in the shell is localhost HTTP, so this was the only offender, and it was the one that mattered: the early report is the only signal from installs whose bootstrap never completes.
- **What:** the shell reuses `onboarding._ssl_context()` for HTTPS, and `clawmetry/telemetry.py` grows the same ladder (the operating system trust store through `truststore`, then `certifi`'s bundle, then the default context), which also covers the install ping. Both rungs are best-effort imports that fall back to the previous behaviour, so no new dependency is added, and a plain HTTP sink is still given no context.
- **Verified:** 5 guards across both reports (a verifying context on HTTPS, none on HTTP, and the ladder degrading rather than raising when neither library can be imported), 3 of which were confirmed red against the parent commit by restoring its two files and re-running. Beyond the tests, the failure was reproduced against production and the fix confirmed to land there from the same interpreter.
