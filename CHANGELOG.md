...
- **Release: the post-trial paywall is no longer a lockout, and it quotes the price Stripe actually charges. Carrier `[RELEASE]` so #4873 reaches PyPI and the fleet; pairs with the cloud pin roll and clawmetry-landing#640.**
  ...
  - **Prices reconciled with Stripe.** The overlay quotes a price and then opens the Checkout that charges it, so a mismatch is a customer billed a number they were not shown. Starter annual `$190 → $90`, Pro annual `$390 → $190`; anchors now taken from `pricing.html`'s `data-wasmo`/`data-wasyr` rather than invented, monthly carries no anchor (because `/pricing` publishes none) and correctly hides the annual-only device line. The benefit list is now **tier-aware** — previously selecting Pro repainted the price but left Starter's benefits under it, so the Pro card sold Starter. `_selTier`/`_selInterval` moved into module state so the background entitlement poll can no longer reset the user's selection mid-repaint.
