...
- **Add Devin (`devin`) as the 22nd runtime, a paid runtime whose adapter ships in `clawmetry-pro`.**
  ...
  - **Cost is reported in Devin's unit.** Devin bills ACUs and credits and its store carries no USD, so `cost_usd` stays unset and the committed ACU/credit totals ride in session metadata. Deriving dollars from token counts would invent a number the user is not charged.
  ...
  - **Verified:** 23 pro adapter tests plus a Devin cost-conformance case (the shared parametrized one demands `cost_usd > 0`, which an ACU-billed runtime cannot honestly satisfy); full pro suite green; OSS count-sync clean at 22; all catalogue, probe, memory-catalog and public-surface guards green.
