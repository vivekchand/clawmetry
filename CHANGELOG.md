...
- **Add Devin (`devin`) as the 22nd runtime, a paid runtime whose adapter ships in `clawmetry-pro`.**
  ...
  - **Scope decision, deliberate:** Devin Cloud sessions are NOT ingested. They live behind `api.devin.ai/v3` and never touch disk, so reading them would mean the daemon making authenticated outbound calls to a vendor with a customer's enterprise key. That breaks the local-first, read-only contract and belongs in its own opt-in connector. The limit is stated in the adapter docstring and the fixture README so nobody assumes cloud coverage.
