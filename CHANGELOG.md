...
- **Add Devin (`devin`) as the 22nd runtime, a paid runtime whose adapter ships in `clawmetry-pro`.**
  ...
  - **Verified:** 24 pro adapter tests plus a Devin cost-conformance case (the shared parametrized one demands `cost_usd > 0`, which an ACU-billed runtime cannot honestly satisfy); full pro suite green; OSS count-sync clean at 22; all catalogue, probe, memory-catalog and public-surface guards green.
