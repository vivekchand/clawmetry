...
- **Add Devin (`devin`) as the 23rd runtime, a paid runtime whose adapter ships in `clawmetry-pro`.**
  - **Why:** founder ask 2026-08-17. Devin is two products that share a name: Devin Cloud (app.devin.ai, hosted, API-only) and the Devin CLI (cli.devin.ai, internally "chisel"), a local terminal agent that keeps every session on the user's own machine. The CLI half fits the existing local-store model exactly, so it is what ships here.
  - **Scope decision, deliberate:** neither Devin Cloud nor Devin Desktop is ingested. Devin Desktop (the IDE formerly called Windsurf) was investigated 2026-08-17/18 with real Cascade turns: it writes zero transcript bytes to disk, so no local adapter is possible and the call was to claim nothing. `devin` here means the CLI and its own session store. Devin Cloud sessions are NOT ingested either. They live behind `api.devin.ai/v3` and never touch disk, so reading them would mean the daemon making authenticated outbound calls to a vendor with a customer's enterprise key. That breaks the local-first, read-only contract and belongs in its own opt-in connector. The limit is stated in the adapter docstring and the fixture README so nobody assumes cloud coverage.
  - **What (OSS wiring; `DevinAdapter` ships in clawmetry-pro 0.7.9):** family-adapter spec plus lite detection paths in `sync.py` (`$XDG_DATA_HOME/devin/cli/sessions.db`, with the vendor installer's legacy `cognition`/`chisel` namespaces and a `CLAWMETRY_DEVIN_DB` override), entitlements (paid runtime, label, landing path), all six runtime prefix sets, runtime probe, Memory/Skills catalog, all six `app.js` runtime maps (labels, prefixes, caps `SESSIONS/EVENTS/COST`, flow tile, icon), README grid link, docs and i18n counts via `scripts/sync_runtime_count.py` (22 to 23).
  - **The hook lane was already live and being dropped.** The numbat installer writes `devin-permission-request` and `devin-post-tool` hooks into `~/.config/devin/config.json`, so findings already arrive with `source_agent="devin"`. `numbat_ingest` snake-cased that to a `devin` runtime, which then failed the `ALL_RUNTIMES` check and went nowhere. Adding the catalogue entry connects that existing pipe; explicit aliases were added alongside it.
  - **Memory and Skills paths are the CLI's own answer, not a guess:** `devin skills paths` and `devin rules paths` print them (`~/.config/devin/skills`, `~/.agents/skills`, `.devin/skills`, `.agents/skills`, `.windsurf/rules`). `.cursor/rules`, which Devin also imports, is deliberately left with the Cursor entry so one file is never attributed to two runtimes.
  - **Cost, corrected by a real capture.** The first draft of this adapter reported ACUs, because `ChatMessageMetadata` declares `committed_acu_cost` and `committed_credit_cost`. A real two-session store showed both absent on all 101 nodes: the CLI computes ACUs live for `/usage` and `/session-stats` and never writes them. So no ACU number is reported. What Devin does persist per assistant turn is `metadata.metrics` (input, output and cache-read tokens, time-to-first-token, duration), and that is what surfaces. `cost_usd` stays unset either way: Devin bills ACUs, so a token-derived dollar figure is a number the user is not charged.
  - **New guard, revert-proven:** `tests/test_runtime_prefix_sets_in_lockstep.py` asserts every catalogue runtime appears in all seven Python prefix sets and both `app.js` runtime maps, plus its label, landing path and presence probe. A session id carries its runtime as a prefix while `agent_type` stays `openclaw`, so a missed set is silent: rows land in DuckDB and get bucketed as OpenClaw. FLYWHEEL 2a records that `qm` shipped missing from `_CM_RT_PREFIXES` exactly that way; this is the guard that would have caught it. Dropping `devin` from the JS map turns it red.
  - **Also found only because a real store existed:** Devin rewrites its entire system prefix as fresh `role="system"` nodes on every turn (24 of the 31 nodes in a two-turn session), which the first draft rendered into the transcript and counted as messages; and every assistant message is journaled twice as sibling nodes, which the active-chain walk already deduped. Both are now pinned by tests.
  - **Verified:** 24 pro adapter tests plus a Devin cost-conformance case (the shared parametrized one demands `cost_usd > 0`, which an ACU-billed runtime cannot honestly satisfy); full pro suite green; OSS count-sync clean at 23; all catalogue, probe, memory-catalog and public-surface guards green.
