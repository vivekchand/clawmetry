## [Unreleased]
- **Removed:** The "Rate limits" left-nav tab. It rendered "No API usage data yet" for ~95% of users in OSS (data feed is opt-in: OTLP exporter on the gateway or the `clawmetry track` interceptor) and was hard-disabled by design in cloud (rate-limit state is per-node, doesn't ride the snapshot, so no cloud user could ever see data). Per-provider spend already lives on the Cost tab; 429s surface in Brain + Reliability. The `/api/rate-limits` endpoint stays for power users scripting against it. (#1971)
