...
- **Release: ship the Sessions-tab render fix to end users.**
  - **Why:** carrier `[RELEASE]` for #4675. The Sessions tab is blank for ANY user with at least one session: the row loop's `function(t)` parameter shadows the global i18n `t()`, and the Score button inside the loop calls `t('transcripts.score_btn', ...)`, so the first row throws and the catch paints "Failed to load transcripts". The API is unaffected; only the render dies.
  - **What:** no new code; ships #4675 (param renamed to `tx`, plus an auto-discovering guard test that fails CI if any callback shadowing `t` calls i18n inside its body).
  - **Verified:** guard revert-proven (red on pre-fix main at app.js:16031, green after); fix injected into the live 0.12.667 dashboard on the founder's Windows box rendered all 9 session rows where the tab was blank. Post-release: confirm the fleet auto-updates and the Sessions tab lists sessions.
