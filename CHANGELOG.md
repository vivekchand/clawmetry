...
- **Release: choosing Self-host in desktop onboarding now actually starts the 7-day trial.**
  - **Why:** carrier `[RELEASE]` so #4694 reaches PyPI and the fleet. Live regression on a Windows desktop install (v0.12.673): GitHub sign-in plus the Self-host choice left the machine on the OSS tier with every paid runtime locked and no account linked. The desktop runs `clawmetry connect --key ... --defer-sync` as a non-interactive subprocess; the ownership-OTP gate exempts the cloud path (`--start-sync-now`) but not this one, and `_verify_key_ownership` exits 1 without a tty, so the sign-in died before writing config, minting the trial, or provisioning Pro. Two more defects hid behind it: a plain successful connect deletes the nocloud marker and queues a full cloud backfill (signing in would flip a self-host install to cloud sync), and with the marker present connect refuses non-interactively with exit 0, so the caller reports "signed in" while nothing happened.
  - **What:** ships #4694. New `clawmetry connect --keep-local` flag exposing the keep-local sign-in seam (previously reachable only inside the interactive onboard wizard): nocloud marker kept, `enable_cloud()` never called, no cloud backfill, trial minted via the license rail (which deliberately works local-only). The OTP exemption now covers the machine-generated `--defer-sync` and `--keep-local` invocations. Keep-local with `--defer-sync` also skips the dashboard spawn and browser open, since the desktop supervisor owns the daemon and a second dashboard would race it for the DuckDB writer lock. The desktop Self-host path now passes `--keep-local --defer-sync`.
  - **Verified:** regression tests revert-proven (`test_defer_sync_skips_ownership_otp` red on unfixed code, green after; `test_keep_local_skips_otp_and_never_reenables_cloud` asserts marker kept and no `enable_cloud` call); live replay on the affected machine confirmed the fixed command passes the OTP gate non-interactively and keeps the marker, where the shipped build exited at the tty check.
